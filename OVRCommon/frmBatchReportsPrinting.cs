...
{
    public partial class BatchReportsPrintingForm : UIForm
    {
        ...
        public List<OVRScheduleNodeInfo> m_lstBatchReportParamters;
        ...
        //以下代码，不用，不用和公共模块进行交互，仅仅需要更为直接的交互。
        ...
        public BatchReportsPrintingForm()
        {
            InitializeComponent();
            ...
            this.chbGenerateDoc.Checked = false;
        ...
        public bool QueryReportInfo(OVRReportInfo oReportInfo)
        {
            ...
            if (drSelRows.Length < 1)
            {
                ...
                DevComponents.DotNetBar.MessageBoxEx.Show(strText + strTemplateName + ".mrt", strCaption, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
        ...
        public void DoReport(OVRReportAction eAction, OVRReportInfo oReportInfo)
        {
            ...
            if (!VarValueValidating())
            {
                ...
                string strCaption = LocalizationRecourceManager.GetString(strSectionName, "GenRptError");
                DevComponents.DotNetBar.MessageBoxEx.Show(strText, strCaption, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void GetSystemConfiguration()
        {
            ...
            catch (System.Exception ex)
            {
                ...
                DevComponents.DotNetBar.MessageBoxEx.Show(ex.Message, strCaption, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void IntiCmbNodeType()
        {

            DataTable tbNodeTypes = new DataTable("NodeTypes");
            tbNodeTypes.Columns.Add(new DataColumn("F_NodeTypeID", typeof(int)));
            tbNodeTypes.Columns.Add(new DataColumn("F_NodeTypeName", typeof(string)));

            // -4表示未知, -3表示Sport，-2表示Discipline，-1表示Event，0表示Phase， 1表示Match
            DataRow drLst = tbNodeTypes.NewRow();
            drLst["F_NodeTypeID"] = -3;
            drLst["F_NodeTypeName"] = "Sport";
            tbNodeTypes.Rows.Add(drLst);

            drLst = tbNodeTypes.NewRow();
            drLst["F_NodeTypeID"] = -2;
            drLst["F_NodeTypeName"] = "Discipline";
            tbNodeTypes.Rows.Add(drLst);

            drLst = tbNodeTypes.NewRow();
            drLst["F_NodeTypeID"] = -1;
            drLst["F_NodeTypeName"] = "Event";
            tbNodeTypes.Rows.Add(drLst);

            drLst = tbNodeTypes.NewRow();
            drLst["F_NodeTypeID"] = 0;
            drLst["F_NodeTypeName"] = "Phase";
            tbNodeTypes.Rows.Add(drLst);

            drLst = tbNodeTypes.NewRow();
            drLst["F_NodeTypeID"] = 1;
            drLst["F_NodeTypeName"] = "Match";
            tbNodeTypes.Rows.Add(drLst);

            m_dsDataSet.Tables.Add(tbNodeTypes);

            cbNoteType.DataSource = m_dsDataSet.Tables["NodeTypes"];
            cbNoteType.DisplayMember = "F_NodeTypeName";
            cbNoteType.ValueMember = "F_NodeTypeID";
        }
        ...
        private void LoadTemplates()
        {
            if (!m_bTplPathExist)
            {
                ...
                DevComponents.DotNetBar.MessageBoxEx.Show(strCaption + ": \n" + m_strTplPath, strCaption, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        private void PrintToPdf(string strRptName, string strVersion)
        {
            ...
            if (!m_bPrtPathExist)
            {
                ...
                DevComponents.DotNetBar.MessageBoxEx.Show(strMsg, "", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            try
            {
                ...
                //zjy 2013-05-03 批量的生成报表，应该成功就不用弹出成功提示！
                //strTemp = LocalizationRecourceManager.GetString(strSectionName, "RptPdfExportSucceed");
                //DevComponents.DotNetBar.MessageBoxEx.Show(strTemp + "\n\r" + strRptFullName, null, MessageBoxButtons.OK);
            }
            catch (System.Exception ex)
            {
                strTemp = LocalizationRecourceManager.GetString(strSectionName, "RptPdfExportFailed");
                DevComponents.DotNetBar.MessageBoxEx.Show(ex.Message, strTemp, MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void btnBatchPrintToPdf_Click(object sender, EventArgs e)
        {
            emScheduleNodeType emSelectedNodeType = (emScheduleNodeType)cbNoteType.SelectedValue;

            if (m_lstBatchReportParamters == null) return;
            if (m_lstBatchReportParamters.Count > 0)
            {
                ...
                foreach (OVRScheduleNodeInfo oneScheduleNodeInfo in m_lstBatchReportParamters)
                {
                    if (oneScheduleNodeInfo.emNodeType == emSelectedNodeType)
                    {
                        //给参数列表赋值
                        SetReportContext("DisciplineID", oneScheduleNodeInfo.DisciplineID.ToString());
                        SetReportContext("EventID", oneScheduleNodeInfo.EventID.ToString());
                        SetReportContext("PhaseID", oneScheduleNodeInfo.PhaseID.ToString());
                        SetReportContext("MatchID", oneScheduleNodeInfo.MatchID.ToString());
                        ...
                        PrintToPdf(tbRscCode.Text + "_" + tbRptType.Text + "_" + tbVersion.Text, tbVersion.Text);
                    }
                    else
                    {
                        continue;
            ...
            DevComponents.DotNetBar.MessageBoxEx.Show(this, "Batch Create Reports Success!");
            this.Cursor = System.Windows.Forms.Cursors.Arrow;
    ...
    public enum emScheduleNodeType
    {
        emUnknown = -4, // -4表示未知, -3表示Sport，-2表示Discipline，-1表示Event，0表示Phase， 1表示Match
        emSport = -3,
        emDiscipline = -2,
        emEvent = -1,
        emPhase = 0,
    ...
    //OVRScheduleNodeInfo 
    public class OVRScheduleNodeInfo
    {
        private emScheduleNodeType m_emNodeType;
        private int m_iSportID;
        private int m_iDisciplineID;
        private int m_iEventID;
        private int m_iPhaseID;
        private int m_iMatchID;

        public OVRScheduleNodeInfo()
        {
            m_iSportID = -1;
            m_iDisciplineID = -1;
            m_iEventID = -1;
        ...
        public OVRScheduleNodeInfo(emScheduleNodeType emNodeType, int iSportID, int iDisciplineID, int iEventID, int iPhaseID, int iMatchID)
        {
        ...
        public emScheduleNodeType emNodeType
        {
        ...
        public int SportID
        {
            get { return m_iSportID; }
            set { m_iSportID = value; }
        }

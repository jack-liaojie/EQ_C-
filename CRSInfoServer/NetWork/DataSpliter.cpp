...
void CDataSpliter::SetSplitMark(LPBYTE lpBeginMark, int nBeginMarkSize)
{
	RemoveSplitMark();
	if (nBeginMarkSize > 0)
	{
...
void CDataSpliter::ProcessDataNoEnd()
{
	// Not enough valid data
	int nDataSize = GetDataSize();
	if (nDataSize < (m_nBeginMarkSize + 4))
		return;				

	LPBYTE lpCurPos = m_lpDataBeginPos;
	while ( lpCurPos < m_lpDataEndPos )
	{
		if (IsBeginMark(lpCurPos)) // Find begin mark
		{
			if (m_lpDataEndPos < (lpCurPos + m_nBeginMarkSize + 4))
				break;
				
			UINT nCountedFrameSize = *((DWORD*)(lpCurPos + m_nBeginMarkSize)); // the frame size is 4 bytes value
			
			LPBYTE lpFrameBeginPos = NULL;
			UINT nFrameSize = 0;
			if (m_bNeedSplitMark)
			{
				lpFrameBeginPos = lpCurPos;
				nFrameSize = m_nBeginMarkSize + 4 + nCountedFrameSize;
			}
			else
			{
				lpFrameBeginPos = lpCurPos + m_nBeginMarkSize + 4;
				nFrameSize = nCountedFrameSize;
			}

			if (m_lpDataEndPos < (lpFrameBeginPos + nFrameSize)) // Not completed pack
				break;

			// One Pack
			DoCallBackFunc(lpFrameBeginPos, nFrameSize);

			// Record the processed position, and adjust the lpCurPos			
			lpCurPos = lpFrameBeginPos + nFrameSize;
		}
		else
			lpCurPos++;

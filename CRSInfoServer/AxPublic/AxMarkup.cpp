// Markup.cpp: implementation of the CAxMarkup class.
//
// Markup Release 11.0
// Copyright (C) 2009 First Objective Software, Inc. All rights reserved
// Go to www.firstobject.com for the latest CAxMarkup and EDOM documentation
...
void x_AddResult( MCD_STR& strResult, MCD_CSTR pszID, MCD_CSTR pszVal = NULL, int nResultCode = 0, int n = -1, int n2 = -1 )
{
	...
	{
		// Use a temporary CAxMarkup object but keep strResult in a string to minimize memory footprint
		CAxMarkup mResult( strResult );
		if ( nResultCode & MRC_MODIFY )
			mResult.FindElem( pszID );
		else
			mResult.AddElem( pszID, MCD_T(""), CAxMarkup::MNF_WITHNOLINES );
		if ( pszVal.pcsz )
		{
...
int TextEncoding::PerformConversion( void* pTo, MCD_CSTR pszToEncoding/*=NULL*/ )
{
	...
	if ( nFromCP == MCD_ACP )
	{
		...
		while ( nALenRemaining )
		{
			...
			if ( nToCP == MCD_UTF8 )
				CAxMarkup::EncodeCharUTF8( (int)wcChar, pU, nToLen );
			else if ( nToCP == MCD_UTF16 )
				CAxMarkup::EncodeCharUTF16( (int)wcChar, (unsigned short*)pU, nToLen );
			else // UTF32
			{
				if ( pU )
	...
	else if ( nToCP == MCD_ACP )
	{
		...
		{
			if ( nFromCP == MCD_UTF8 )
				nUChar = CAxMarkup::DecodeCharUTF8( pU.p8, pUEnd );
			else if ( nFromCP == MCD_UTF16 )
				nUChar = CAxMarkup::DecodeCharUTF16( pU.p16, (const unsigned short*)pUEnd );
			else // UTF32
				nUChar = *(pU.p32)++;
			if ( nUChar == -1 )
				nCharLen = -2;
	...
	if ( nFromCP == MCD_UTF32 )
	{
		...
		if ( nToCP == MCD_UTF8 )
		{
			char* p8 = (char*)pTo;
			while ( p32 != p32End )
				CAxMarkup::EncodeCharUTF8( *p32++, p8, nToLen );
		}
		else if ( nToCP == MCD_UTF16 )
		{
			unsigned short* p16 = (unsigned short*)pTo;
			while ( p32 != p32End )
				CAxMarkup::EncodeCharUTF16( (int)*p32++, p16, nToLen );
		}
		else // to ANSI
		{
	...
	else if ( nFromCP == MCD_UTF16 )
	{
		...
		if ( nToCP == MCD_UTF32 )
		{
			...
			while ( p16 != p16End )
			{
				nUChar = CAxMarkup::DecodeCharUTF16( p16, p16End );
				if ( nUChar == -1 )
					nUChar = '?';
				if ( p32 )
		...
		else if ( nToCP == MCD_UTF8 )
		{
			...
			while ( p16 != p16End )
			{
				...
				if ( nUChar == -1 )
					nUChar = '?';
				CAxMarkup::EncodeCharUTF8( nUChar, p8, nToLen );
			}
		}
		else // to ANSI
		{
	...
	else if ( nToCP == MCD_UTF16  ) // to UTF-16 from UTF-8/ANSI
	{
		...
		if ( nFromCP == MCD_UTF8 )
		{
			...
			while ( p8 != p8End )
			{
				nUChar = CAxMarkup::DecodeCharUTF8( p8, p8End );
				if ( nUChar == -1 )
					nUChar = '?';
				if ( p16 )
...
bool TextEncoding::FindRaggedEnd( int& nTruncBeforeBytes )
{
	...
	if ( nCP == MCD_UTF16 )
	{
		...
		if ( CAxMarkup::DecodeCharUTF16(pUTF16Last,&pUTF16Buffer[m_nFromLen]) == -1 )
			nTruncBeforeBytes = 2;
	}
	else // UTF-8, SBCS DBCS
	{
		if ( nCP == MCD_UTF8 )
		{
			...
			while ( nLast > 0 && CAxMarkup::DecodeCharUTF8(pUTF8Last,pUTF8End) == -1 )
...
bool x_EndianSwapRequired( int nDocFlags )
{
	...
	if ( cFirstByte ) // LE
	{
		if ( nDocFlags & CAxMarkup::MDF_UTF16BEFILE )
			return true;
	}
	else if ( nDocFlags & CAxMarkup::MDF_UTF16LEFILE )
		return true;
	return false;
}
...
struct TokenPos
{
	...
	{
		int nLen = Length();
		if ( m_nTokenFlags & CAxMarkup::MDF_IGNORECASE )
...
void ElemStack::PushTagAndCount( TokenPos& token )
{
	...
	if ( iTop != iPar )
	{
		...
		{
			nSlot = CalcSlot( pTagName, token.Length(), (token.m_nTokenFlags & CAxMarkup::MDF_IGNORECASE)?true:false );
...
// In file mode the file stays open between CAxMarkup calls and is stored in m_pFilePos
//
struct FilePos
{
	FilePos()
...
{
	{ "\xef\xbb\xbf", 3, MCD_T("UTF-8"), CAxMarkup::MDF_UTF8PREAMBLE },
	{ "\xff\xfe", 2, MCD_T("UTF-16LE"), CAxMarkup::MDF_UTF16LEFILE },
	{ "\xfe\xff", 2, MCD_T("UTF-16BE"), CAxMarkup::MDF_UTF16BEFILE },
	{ NULL,0,NULL,0 }
};
...
bool FilePos::FileAtTop()
{
	...
	if ( ((m_nDocFlags & CAxMarkup::MDF_READFILE) && m_nFileByteOffset < (MCD_INTFILEOFFSET)m_nOpFileByteLen + 5 )
			|| ((m_nDocFlags & CAxMarkup::MDF_WRITEFILE) && m_nFileByteOffset < 5) )
		return true;
	return false;
}
...
bool FilePos::FileOpen( MCD_CSTR_FILENAME szFileName )
{
	...
	if ( m_nDocFlags & CAxMarkup::MDF_APPENDFILE )
		pMode = MCD_T_FILENAME("ab");
	else if ( m_nDocFlags & CAxMarkup::MDF_WRITEFILE )
	...
	if ( m_nDocFlags & CAxMarkup::MDF_READFILE )
	{
		// Get file length
		...
		if ( nReadTop )
		{
			...
			if ( bSuccess )
			{
				...
				int nBomCheck = 0;
				m_nDocFlags &= ~( CAxMarkup::MDF_UTF16LEFILE | CAxMarkup::MDF_UTF8PREAMBLE );
...
#endif // not WCHAR
					m_strEncoding = CAxMarkup::GetDeclaredEncoding( strDeclCheck );
				}
...
bool FilePos::FileReadText( MCD_STR& strDoc )
{
	...
	if ( m_nDocFlags & (CAxMarkup::MDF_UTF16LEFILE | CAxMarkup::MDF_UTF16BEFILE) )
	{
	...
	else // single or multibyte file (i.e. not UTF-16)
	{
		...
		if ( MCD_STRISEMPTY(m_strEncoding) )
		{
			int nNonASCII;
			bool bErrorAtEnd;
			if ( CAxMarkup::DetectUTF8(pBuffer,m_nOpFileByteLen,&nNonASCII,&bErrorAtEnd) || (bCheckRaggedEnd && bErrorAtEnd) )
			{
				m_strEncoding = MCD_T("UTF-8");
		...
		else // load directly into string
		{
			...
			if ( bAssumeUnknownIsNative )
			{
				...
				int nNonASCII;
				bool bErrorAtEnd;
				bool bIsUTF8 = CAxMarkup::DetectUTF8( pBuffer, m_nOpFileByteLen, &nNonASCII, &bErrorAtEnd ) || (bCheckRaggedEnd && bErrorAtEnd);
...
bool FilePos::FileClose()
{
	if ( m_fp )
	{
		if ( fclose(m_fp) )
			FileErrorAddResult();
		m_fp = NULL;
		m_nDocFlags &= ~(CAxMarkup::MDF_WRITEFILE|CAxMarkup::MDF_READFILE|CAxMarkup::MDF_APPENDFILE);
		return true;
	}
	return false;
}
...
bool FilePos::FileFlush( int nWriteStrLen/*=-1*/, bool bFflush/*=false*/ )
{
	...
	if ( nWriteStrLen )
	{
		...
		{
			m_strEncoding = CAxMarkup::GetDeclaredEncoding( strBuffer );
			if ( MCD_STRISEMPTY(m_strEncoding) )
				m_strEncoding = MCD_T("UTF-8");
		}
	...
	while ( 1 )
	{
		...
		if ( ! cD )
		{
			...
			if ( ! cD )
			{
				...
				if ( nNodeType != CAxMarkup::MNT_WHITESPACE && nNodeType != CAxMarkup::MNT_TEXT )
				{
					MCD_PCSZ pType = MCD_T("tag");
					if ( (nParseFlags & PD_DOCTYPE) || nNodeType == CAxMarkup::MNT_DOCUMENT_TYPE )
						pType = MCD_T("document_type");
					else if ( nNodeType == CAxMarkup::MNT_ELEMENT )
						pType = MCD_T("start_tag");
					else if ( nNodeType == 0 )
						pType = MCD_T("end_tag");
					else if ( nNodeType == CAxMarkup::MNT_CDATA_SECTION )
						pType = MCD_T("cdata_section");
					else if ( nNodeType == CAxMarkup::MNT_PROCESSING_INSTRUCTION )
						pType = MCD_T("processing_instruction");
					else if ( nNodeType == CAxMarkup::MNT_COMMENT )
						pType = MCD_T("comment");
					nNodeType = -1;
		...
		if ( pFindEnd )
		{
			if ( cD == '>' && ! (nParseFlags & (PD_INQUOTE_S|PD_INQUOTE_D)) )
			{
				...
				{
					pFindEnd = NULL;
					if ( nNodeType == CAxMarkup::MNT_ELEMENT && cDminus1 == '/' )
					{
			...
			else if ( cD == '<' && (nNodeType == CAxMarkup::MNT_TEXT || nNodeType == -1) )
			{
				m_nNext = (int)(pD - m_pDocText);
				break;
			}
			...
			else if ( nNodeType & CAxMarkup::MNT_DOCUMENT_TYPE )
			{
		...
		else if ( nParseFlags )
		{
			if ( nParseFlags & PD_TEXTORWS )
			{
				...
				{
					m_nNext = (int)(pD - m_pDocText);
					nNodeType = CAxMarkup::MNT_WHITESPACE;
					break;
				}
				else if ( ! MCD_PSZCHR(MCD_T(" \t\n\r"),(MCD_CHAR)cD) )
				{
					nParseFlags ^= PD_TEXTORWS;
					FINDNODETYPE( MCD_T("<"), CAxMarkup::MNT_TEXT )
				}
			}
			else if ( nParseFlags & PD_OPENTAG )
			{
				...
				if ( cD > 0x60 || ( cD > 0x40 && cD < 0x5b ) || cD == 0x5f || cD == 0x3a )
					FINDNODETYPENAME( MCD_T(">"), CAxMarkup::MNT_ELEMENT, 0 )
				else if ( cD == '/' )
					FINDNODETYPENAME( MCD_T(">"), 0, 1 )
				else if ( cD == '!' )
					nParseFlags |= PD_BANG;
				else if ( cD == '?' )
					FINDNODETYPENAME( MCD_T("?>"), CAxMarkup::MNT_PROCESSING_INSTRUCTION, 1 )
				else
			...
			else if ( nParseFlags & PD_BANG )
			{
				...
				else if ( nParseFlags & PD_DOCTYPE )
				{
					if ( MCD_PSZCHR(MCD_T("EAN"),(MCD_CHAR)cD) ) // <!ELEMENT ATTLIST ENTITY NOTATION
						FINDNODETYPE( MCD_T(">"), CAxMarkup::MNT_DOCUMENT_TYPE )
					else
			...
			else if ( nParseFlags & PD_DASH )
			{
				nParseFlags ^= PD_DASH;
				if ( cD == '-' )
					FINDNODETYPE( MCD_T("-->"), CAxMarkup::MNT_COMMENT )
				else
			...
			else if ( nParseFlags & PD_BRACKET )
			{
				nParseFlags ^= PD_BRACKET;
				if ( cD == 'C' )
					FINDNODETYPE( MCD_T("]]>"), CAxMarkup::MNT_CDATA_SECTION )
				else
			...
			else if ( nParseFlags & PD_DOCTYPE )
			{
				...
				{
					m_nNext = (int)(pD - m_pDocText) + 1;
					nNodeType = CAxMarkup::MNT_DOCUMENT_TYPE;
					break;
				}
			}
		}
...
void CAxMarkup::operator=( const CAxMarkup& markup )
{
...
bool CAxMarkup::SetDoc( MCD_PCSZ pDoc )
{
...
bool CAxMarkup::SetDoc( const MCD_STR& strDoc )
{
...
MCD_STR CAxMarkup::GetError() const
{
...
bool CAxMarkup::Load( MCD_CSTR_FILENAME szFileName )
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
...
bool CAxMarkup::ReadTextFile( MCD_CSTR_FILENAME szFileName, MCD_STR& strDoc, MCD_STR* pstrResult, int* pnDocFlags, MCD_STR* pstrEncoding )
{
...
bool CAxMarkup::Save( MCD_CSTR_FILENAME szFileName )
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
...
bool CAxMarkup::WriteTextFile( MCD_CSTR_FILENAME szFileName, const MCD_STR& strDoc, MCD_STR* pstrResult, int* pnDocFlags, MCD_STR* pstrEncoding )
{
...
bool CAxMarkup::FindElem( MCD_CSTR szName )
{
	if ( m_nDocFlags & MDF_WRITEFILE )
		return false;
...
bool CAxMarkup::FindChildElem( MCD_CSTR szName )
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
...
MCD_STR CAxMarkup::EscapeText( MCD_CSTR szText, int nFlags )
{
...
MCD_STR CAxMarkup::UnescapeText( MCD_CSTR szText, int nTextLength /*=-1*/ )
{
...
bool CAxMarkup::DetectUTF8( const char* pText, int nTextLen, int* pnNonASCII/*=NULL*/, bool* bErrorAtEnd/*=NULL*/ )
{
...
int CAxMarkup::DecodeCharUTF8( const char*& pszUTF8, const char* pszUTF8End/*=NULL*/ )
{
...
void CAxMarkup::EncodeCharUTF16( int nUChar, unsigned short* pwszUTF16, int& nUTF16Len )
{
...
int CAxMarkup::DecodeCharUTF16( const unsigned short*& pwszUTF16, const unsigned short* pszUTF16End/*=NULL*/ )
{
...
void CAxMarkup::EncodeCharUTF8( int nUChar, char* pszUTF8, int& nUTF8Len )
{
...
int CAxMarkup::UTF16To8( char* pszUTF8, const unsigned short* pwszUTF16, int nUTF8Count )
{
...
int CAxMarkup::UTF8To16( unsigned short* pwszUTF16, const char* pszUTF8, int nUTF8Count )
{
...
MCD_STR CAxMarkup::UTF8ToA( MCD_CSTR pszUTF8, int* pnFailed/*=NULL*/ )
{
...
MCD_STR CAxMarkup::AToUTF8( MCD_CSTR pszANSI )
{
...
MCD_STR CAxMarkup::GetDeclaredEncoding( MCD_CSTR szDoc )
{
...
int CAxMarkup::FindNode( int nType )
{
...
MCD_STR CAxMarkup::GetTagName() const
{
...
MCD_STR CAxMarkup::GetAttribName( int n ) const
{
...
bool CAxMarkup::SavePos( MCD_CSTR szPosName /*=""*/, int nMap /*=0*/ )
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
...
bool CAxMarkup::RestorePos( MCD_CSTR szPosName /*=""*/, int nMap /*=0*/ )
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
...
bool CAxMarkup::SetMapSize( int nSize, int nMap /*=0*/ )
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
...
bool CAxMarkup::RemoveChildElem()
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
...
//////////////////////////////////////////////////////////////////////
// CAxMarkup private methods
//
void CAxMarkup::x_InitMarkup()
{
	// Only called from CAxMarkup constructors
	m_pFilePos = NULL;
...
int CAxMarkup::x_GetParent( int i )
{
	return ELEM(i).iElemParent;
}

void CAxMarkup::x_SetPos( int iPosParent, int iPos, int iPosChild )
{
...
void CAxMarkup::x_SetDebugState()
{
...
bool CAxMarkup::x_AllocElemPos( int nNewSize /*=0*/ )
{
...
int CAxMarkup::x_ParseElem( int iPosParent, TokenPos& token )
{
...
int CAxMarkup::x_FindElem( int iPosParent, int iPos, MCD_PCSZ pPath ) const
{
...
MCD_STR CAxMarkup::x_GetPath( int iPos ) const
{
	MCD_STR strPath;
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
	{
...
MCD_STR CAxMarkup::x_GetTagName( int iPos ) const
{
...
MCD_STR CAxMarkup::x_GetAttrib( int iPos, MCD_PCSZ pAttrib ) const
{
...
bool CAxMarkup::x_SetAttrib( int iPos, MCD_PCSZ pAttrib, int nValue, int nFlags /*=0*/ )
{
...
bool CAxMarkup::x_SetAttrib( int iPos, MCD_PCSZ pAttrib, MCD_PCSZ pValue, int nFlags /*=0*/ )
{
	if ( m_nDocFlags & MDF_READFILE )
		return false;
	int nNodeStart = 0;
	...
	int nReplace = 0;
	int nInsertAt;
	MCD_STR strEscapedValue = EscapeText( pValue, nFlags );
...
bool CAxMarkup::x_CreateNode( MCD_STR& strNode, int nNodeType, MCD_PCSZ pText )
{
...
MCD_STR CAxMarkup::x_EncodeCDATASection( MCD_PCSZ szData )
{
...
bool CAxMarkup::x_SetData( int iPos, int nValue )
{
	// Convert integer to string
	MCD_CHAR szVal[25];
	MCD_SPRINTF( MCD_SSZ(szVal), MCD_T("%d"), nValue );
...
bool CAxMarkup::x_SetData( int iPos, MCD_PCSZ szData, int nFlags )
{
	if ( m_nDocFlags & MDF_READFILE )
		return false;
	MCD_STR strInsert;
...
MCD_STR CAxMarkup::x_GetData( int iPos )
{
	if ( iPos == m_iPos && m_nNodeLength )
	{
...
MCD_STR CAxMarkup::x_GetElemContent( int iPos ) const
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return MCD_T("");
	ElemPos* pElem = &ELEM(iPos);
...
bool CAxMarkup::x_SetElemContent( MCD_PCSZ szContent )
{
	MCD_STRCLEAR(m_strResult);
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
...
void CAxMarkup::x_DocChange( int nLeft, int nReplace, const MCD_STR& strInsert )
{
	x_StrInsertReplace( m_strDoc, nLeft, nReplace, strInsert );
}

void CAxMarkup::x_Adjust( int iPos, int nShift, bool bAfterPos /*=false*/ )
{
...
int CAxMarkup::x_InsertNew( int iPosParent, int& iPosRel, NodePos& node )
{
...
bool CAxMarkup::x_AddElem( MCD_PCSZ pName, int nValue, int nFlags )
{
	// Convert integer to string
	MCD_CHAR szVal[25];
	MCD_SPRINTF( MCD_SSZ(szVal), MCD_T("%d"), nValue );
...
bool CAxMarkup::x_AddElem( MCD_PCSZ pName, MCD_PCSZ pValue, int nFlags )
{
	if ( m_nDocFlags & MDF_READFILE )
		return false;
	if ( nFlags & MNF_CHILD )
...
MCD_STR CAxMarkup::x_GetSubDoc( int iPos ) const
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return MCD_T("");
	if ( iPos )
...
bool CAxMarkup::x_AddSubDoc( MCD_PCSZ pSubDoc, int nFlags )
{
	if ( m_nDocFlags & (MDF_READFILE|MDF_WRITEFILE) )
		return false;
	MCD_STRCLEAR(m_strResult);
...
int CAxMarkup::x_RemoveElem( int iPos )
{
...
void CAxMarkup::x_LinkElem( int iPosParent, int iPosBefore, int iPos )
{
...
int CAxMarkup::x_UnlinkElem( int iPos )
{
...
int CAxMarkup::x_UnlinkPrevElem( int iPosParent, int iPosBefore, int iPos )
{
...
int CAxMarkup::x_ReleasePos( int iPos )
{
...
int CAxMarkup::x_ReleaseSubDoc( int iPos )
{
...
void CAxMarkup::x_CheckSavedPos()
{
...
void CAxMarkup::x_AdjustForNode( int iPosParent, int iPos, int nShift )
{
...
bool CAxMarkup::x_AddNode( int nNodeType, MCD_PCSZ pText, int nNodeFlags )
{
	if ( m_nDocFlags & MDF_READFILE )
		return false;
...
void CAxMarkup::x_RemoveNode( int iPosParent, int& iPos, int& nNodeType, int& nNodeOffset, int& nNodeLength )
{
	int iPosPrev = iPos;

#include "StdAfx.h"
#include "AxLogSystem.h"

#define MAX_MSG_LINE_COUNT			1000
#define WM_USER_ADD_MSG				WM_USER+500

CAxLogSystem g_oLog;

CAxLogSystem::CAxLogSystem(void)
{
...
BOOL CAxLogSystem::StartLogService(CEdit *pEdit)
{
	if (m_strFileName.IsEmpty())
	{
		...
		m_strFileName = strModule.TrimRight(_T('\\')) + _T('\\') + _T("ServerNetwork") + _T(".log");
...
BOOL CAxLogSystem::SetLogFile(CString strFilePath, BOOL bTruncate)
{
	UINT nOpenFlag;
	if (bTruncate)
...
void CAxLogSystem::OutPutMsg(CString strMsg)
{
...
CString CAxLogSystem::FormatLogMsg(CString strMsg)
{
	CString strFmtMsg;
...
LRESULT CAxLogSystem::OnAddMsg(WPARAM wParam, LPARAM lParam)
{
...
//////////////////////////////////////////////////////////////////////////
// Thread control
void CAxLogSystem::DestroyWorkThread()
{
...
BOOL CAxLogSystem::CreateWorkThread()
{
	...
	m_pThread = AfxBeginThread(CAxLogSystem::ThreadProc, this);
	if (!m_pThread)
		return FALSE;
...
UINT CAxLogSystem::ThreadProc(LPVOID pParam)
{
	MSG msg;
	ZeroMemory(&msg, sizeof(MSG));
	PeekMessage(&msg, NULL, WM_USER, WM_USER, PM_NOREMOVE);

	CAxLogSystem *pLogFunc = (CAxLogSystem *)pParam;

...
// character clAxses.
class CDigiColonDotChar : public CAxDigiChar
{
public:
	CDigiColonDotChar();
...
class CDigi7Segment : public CAxDigiChar
{
public:
	CDigi7Segment();
	void SetElementData(WORD wSegmData, int iDispStyle);
...
class CDigi14Segment : public CAxDigiChar
{
public:
	CDigi14Segment();
	void SetElementData(WORD wSegmData, int iDispStyle);
...
/////////////////////////////////////////////////////////////////////////////
// CAxDigiSegment
CAxDigiSegment::CAxDigiSegment()
{
	m_paPoints	= NULL;
	m_paTypes	= NULL;
}

CAxDigiSegment::CAxDigiSegment(const CAxDigiSegment& Segment)
{
...
void CAxDigiSegment::DefPoints(const POINT* lpaPoints, const BYTE* lpaTypes, int nCount)
{
	FreeSegment();
...
CAxDigiSegment CAxDigiSegment::operator=(const CAxDigiSegment &Segment)
{
	CPoint *pNewPoints;
	BYTE * pNewTypes;
...
void CAxDigiSegment::FreeSegment()
{
	delete[] m_paPoints;
	delete[] m_paTypes;
}

void CAxDigiSegment::Draw(CDC *pDC, const CAxDoubleRect &DrawPlace, int iWidth) const
{
	int i, nBez,b;
	CPoint * paPoints;
	double daContr[4];
...
CAxDigiChar::CAxDigiChar(const CAxDigiChar& DigiChar)
{
...
void CAxDigiChar::Draw(CDC *pDC, const CAxDoubleRect &DrawPlace, CPen *pOffPen, CPen *pOnPen,
					 CBrush *pOffBrush, CBrush *pOnBrush, CRgn* region) 
{
	...
	for (iSeg = 0; iSeg < m_SegmentArray.GetSize(); iSeg++)
	{
		...
		if (!m_bNoOff || (m_wSegmData & SegMAxk))
		{
			pDC->BeginPath();
...
void CAxDigiChar::SetElementData(WORD wSegmData, int iDispStyle)
{
	m_wSegmData = wSegmData;
	m_bNoOff = (iDispStyle & CAxStaDigiClock::DS_NO_OFF) > 0;
}

int CAxDigiChar::GetNormWidth() const
{
	return m_Width;
}
...
CAxDigiChar CAxDigiChar::operator=(const CAxDigiChar &DigiChar)
{
	m_NSegments		  = DigiChar.m_NSegments;
	m_OffColor		  = DigiChar.m_OffColor;
	m_OnColor		  = DigiChar.m_OnColor;
...
void CDigi7Segment::SetElementData(WORD wSegmData, int iDispStyle)
{
	...
	for (i = 0; i < m_NSegments; i++)
	{
		...
		{
		case 0:
		case 6: if (iDispStyle & CAxStaDigiClock::DS_SMOOTH)
				{
				...
				break;
		case 1:
		case 4:if (iDispStyle & CAxStaDigiClock::DS_SMOOTH)
			   {
			   ...
			   break;
		case 2:
		case 5: if (iDispStyle & CAxStaDigiClock::DS_SMOOTH)
				{
				...
				break;
		case 3: if (iDispStyle & CAxStaDigiClock::DS_SMOOTH)
				{
		...
		case 2: if (!(iDispStyle & CAxStaDigiClock::DS_SMOOTH))
				{
		...
		case 5: lpTmpSegPoints = new CPoint[nCount]; 
			for (p = 0; p < nCount; p++)
				if (iDispStyle & CAxStaDigiClock::DS_SMOOTH)
	...
	CAxDigiChar::SetElementData(wSegmData, iDispStyle);
}
...
CAxStaDigiClock::CAxStaDigiClock(): m_strText(_T("?"))
{
...
CAxStaDigiClock::~CAxStaDigiClock()
{
	m_CharArray.RemoveAll();
}

void CAxStaDigiClock::SetText(LPCTSTR lpszText)
{
...
CString CAxStaDigiClock::GetText()
{
	return m_strText;
}

void CAxStaDigiClock::Format(LPCTSTR lpszFormat, ...)
{
	CString str;
...
void CAxStaDigiClock::Format(UINT nFormatID, ...)
{
	CString str;
	CString strFormat;
...
void CAxStaDigiClock::SetColor(COLORREF OffColor, COLORREF OnColor)
{
...
COLORREF CAxStaDigiClock::SetBkColor(COLORREF BackColor /* = BLACK */)
{
...
void CAxStaDigiClock::SetTransparent(BOOL bSet /* = TRUE */)
{
	m_bTransparent = bSet;
	if (m_bTransparent)
...
void CAxStaDigiClock::SetDrawImmediately(BOOL Enable /* = TRUE*/)
{
	m_bImmediateUpdate = Enable;
}

BOOL CAxStaDigiClock::ModifyDigiStyle(DWORD dwRemove, DWORD dwAdd)
{
...
CAxDigiChar * CAxStaDigiClock::DefineChar(TCHAR cChar)
{
	int iIndex;
	CAxDigiChar * pDChar = NULL;
...
void CAxStaDigiClock::BuildString()
{
	CAxDigiChar * pDChar;
...
BEGIN_MESSAGE_MAP(CAxStaDigiClock, CStatic)
	//{{AFX_MSG_MAP(CAxStaDigiClock)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
/////////////////////////////////////////////////////////////////////////////
// CAxStaDigiClock message handlers

void CAxStaDigiClock::OnPaint() 
{
	CRect rect;
	CAxDoubleRect CharRect;
	int r, g, b;
	GetClientRect(&rect);
	COLORREF	PenColor;
	...
	CAxMemDC MemDC(&dc, &rect, m_bTransparent);
...
BOOL CAxStaDigiClock::OnErAxeBkgnd(CDC* /*pDC*/) 
{
	// Don't erAxe the background to avoid flickering
	// Background is painted in CMemDC::CMemDC(); with FillSolidRect();
	return FALSE;
}

void CAxStaDigiClock::DoInvalidate()
{
	if (m_bTransparent)
	{

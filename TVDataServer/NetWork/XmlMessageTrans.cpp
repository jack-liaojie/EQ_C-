...
void CXmlMessageReceiver::SetRecvWndAndMsg(CTVUpdateSettingDlg *pParentDlg, UINT nMsgID)
{ 
	if (pParentDlg && pParentDlg->IsKindOf(RUNTIME_CLASS(CTVUpdateSettingDlg)))
	{
	...
	// Init DataSpliter
	BYTE baBeginMark[] = {0x01, 0x00};
	BYTE baEndMark[] = {0x04, 0x00};
	m_Spliter.SetSplitMark(baBeginMark, sizeof(baBeginMark), baEndMark, sizeof(baEndMark));
...
void CXmlMessageReceiver::OnTranscvDisconnected(CNetworkSocket* pSocket)
{
	...
	if (m_pParentDlg)
	{
		CTVUpdateSettingDlg *pDlg = m_pParentDlg;
		if (pDlg->m_strServerIP == strAddr && pDlg->m_strServerPort == AxCommonInt2String(nPort))
		{
			...
			if (g_WndPopupNotifier.m_pWndParent && g_WndPopupNotifier.m_pWndParent->IsWindowEnabled())
			{
				g_WndPopupNotifier.Show(_T("TVG Connection broken!"));
			}
		}
	}

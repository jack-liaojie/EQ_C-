...
// CTaskbarNotifier
CTaskbarNotifier g_WndPopupNotifier;
...
int CTaskbarNotifier::Create(CWnd *pWndParent)
{
	...
	return CreateEx(WS_EX_TOPMOST,strWndClass,NULL,WS_POPUP,0,0,0,0,pWndParent->m_hWnd,NULL);
}
...
BOOL CTaskbarNotifier::SetSkin(UINT nBitmapID,short red,short green,short blue)
{
	...
	m_rcText.SetRect(10, 10, bm.bmWidth-10, bm.bmHeight-10);
...
void CTaskbarNotifier::Show( LPCTSTR szCaption,DWORD dwTimeToStay/*=3000*/,DWORD dwTimeToShow/*=400*/,DWORD dwTimeToHide/*=500*/,int nIncrement/*=1*/ )
{
	...
	switch (m_nAnimStatus)
	{
		case IDT_HIDDEN:
			...
			else //if (bTaskbarOnBottom)
			{
				...
				m_dwDelayBetweenShowEvents=m_dwTimeToShow/(m_nSkinHeight/m_nIncrement);
				m_dwDelayBetweenHideEvents=m_dwTimeToHide/(m_nSkinHeight/m_nIncrement);
				m_nStartPosX=rcDesktop.right-m_nSkinWidth - 5;
...
void CTaskbarNotifier::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	m_pWndParent->ShowWindow(SW_NORMAL);
	m_pWndParent->SetForegroundWindow();
...
void CTaskbarNotifier::OnPaint()
{
	...
	dc.DrawText(m_strCaption,-1,m_rcText,DT_CENTER | DT_VCENTER | DT_WORDBREAK | DT_END_ELLIPSIS);

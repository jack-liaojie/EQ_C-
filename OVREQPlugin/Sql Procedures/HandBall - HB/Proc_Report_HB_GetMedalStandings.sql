...
/****** Object:  StoredProcedure [dbo].[Proc_Report_HB_GetMedalStandings]    Script Date: 08/29/2012 15:38:16 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_HB_GetMedalStandings]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_HB_GetMedalStandings]
GO
...
--名    称：[Proc_Report_HB_GetMedalStandings]
...
CREATE PROCEDURE [dbo].[Proc_Report_HB_GetMedalStandings](
												@DisciplineID		INT,
												@LanguageCode		CHAR(3)
)
As
...
SET NOCOUNT ON 
    ...
    SELECT DISTINCT D.F_DelegationID, D.F_DelegationCode, E.F_DelegationLongName, 0 AS F_MenG, 0 AS F_MenS, 0 AS F_MenB, 0 AS F_WomenG, 0 AS F_WomenS, 0 AS F_WomenB FROM TS_Event_Result AS A LEFT JOIN TR_Register AS B ON A.F_RegisterID = B.F_RegisterID
    ...
    WHERE C.F_DisciplineID = @DisciplineID AND A.F_RegisterID IS NOT NULL AND A.F_MedalID IS NOT NULL
    ...
    UPDATE #Tmp_Medal SET F_FinishCount = @Count

...
/****** Object:  StoredProcedure [dbo].[Proc_Report_HB_GetGroupResults]    Script Date: 08/29/2012 16:52:51 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_HB_GetGroupResults]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_HB_GetGroupResults]
GO
...
----存储过程名称：[Proc_Report_HB_GetGroupResults]
----功		  能：得到一个小项的小组的比赛成绩。
----作		  者：邓年彩
----日		  期: 2011-6-9

CREATE PROCEDURE [dbo].[Proc_Report_HB_GetGroupResults]
                     (
	                   @EventID       INT,
                       @LanguageCode  CHAR(3)
                      )
...
SET NOCOUNT ON
	...
	SELECT P.F_PhaseID
		...
		, ISNULL(CASE M.F_MatchStatusID
			...
			ELSE CONVERT(NVARCHAR(20), DATEPART(DAY, M.F_MatchDate)) 

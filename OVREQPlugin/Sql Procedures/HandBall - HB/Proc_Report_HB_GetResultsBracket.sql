...
/****** Object:  StoredProcedure [dbo].[Proc_Report_HB_GetResultsBracket]    Script Date: 08/29/2012 16:54:14 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_HB_GetResultsBracket]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_HB_GetResultsBracket]
GO
...
--名    称：[Proc_Report_HB_GetResultsBracket]
...
CREATE PROCEDURE [dbo].[Proc_Report_HB_GetResultsBracket](
                       @EventID         INT,
...
SET NOCOUNT ON 
    ...
    INSERT INTO #Tmp_Table(F_EventID, F_MatchID, F_MatchName, F_MatchNum, F_MatchDate, F_PhaseName,F_RoundCode,F_OrderInRound,F_MatchStatusID)
             SELECT F.F_EventID, A.F_MatchID, B.F_MatchShortName, A.F_MatchNum, UPPER(LEFT(DATENAME(WEEKDAY, A.F_MatchDate), 3)) + ' ' + UPPER(LEFT(CONVERT (NVARCHAR(100),A.F_MatchDate, 113), 7)), 
      ...
      UPDATE #Tmp_Table SET F_BPlayerID = B.F_RegisterID, F_BNOC =  D.F_DelegationCode, F_BDes = D.F_DelegationCode
       ...
       FETCH NEXT FROM MatchIDCursor INTO @MatchID,@MatchStatusID
		...
		BEGIN
			...
			BEGIN
				exec [Proc_HB_GetMatchResult] @MatchID , @OutPut OUTPUT
			...
			BEGIN
				exec [Proc_HB_GetMatchResult] @SouceMatchID , @OutPut OUTPUT

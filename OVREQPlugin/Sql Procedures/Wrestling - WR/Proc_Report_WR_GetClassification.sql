IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_WR_GetClassification]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_WR_GetClassification]
GO
...
--名    称: [Proc_Report_WR_GetClassification]
--描    述: 获取一个项目的所有名次.
--创 建 人: 邓年彩
--日    期: 2010年10月25日 星期一
--修改记录：
...
CREATE PROCEDURE [dbo].[Proc_Report_WR_GetClassification]
	@EventID						INT,
	@PhaseID						INT,
	@MatchID						INT,
	@LanguageCode					CHAR(3)
...
SET NOCOUNT ON
	...
	LEFT JOIN TC_Delegation_Des as DD
		ON DD.F_DelegationID=D.F_DelegationID and DD.F_LanguageCode=@LanguageCode

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_TVG_WR_GetMatchPlayers]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_TVG_WR_GetMatchPlayers]
GO
...
--名    称: [Proc_TVG_WR_GetMatchPlayers]
...
CREATE PROCEDURE [dbo].[Proc_TVG_WR_GetMatchPlayers]
	@MatchID						INT,
...
SET NOCOUNT ON
	...
	SELECT
			...
			,N'[Image]'+D.F_DelegationCode AS [NOC_Red]
			,RD1.F_TvLongName AS [Name_Red]
			,N'[Image]Card_Red' AS [Color_Red]
			,N'[Image]'+D2.F_DelegationCode AS [Noc_Blue]
			,RD2.F_TvLongName as [Name_Blue]
			,N'[Image]Card_Blue' AS [Color_Blue]
			,RD1.F_TvShortName AS [Name_Red_Short]
			,RD2.F_TvShortName AS [Name_Blue_Short]
	from TS_Match AS M

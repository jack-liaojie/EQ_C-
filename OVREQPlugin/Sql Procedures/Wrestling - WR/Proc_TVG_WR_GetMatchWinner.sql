IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_TVG_WR_GetMatchWinner]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_TVG_WR_GetMatchWinner]
GO
...
--名    称: [Proc_TVG_WR_GetMatchWinner]
...
CREATE PROCEDURE [dbo].[Proc_TVG_WR_GetMatchWinner]
	@MatchID						INT,
...
SET NOCOUNT ON
	...
	WHERE M.F_MatchID = @MatchID
		...
		select 
				...
				,case when MR.F_CompetitionPosition=1 then N'[Image]Card_Red'
					when MR.F_CompetitionPosition=2 then N'[Image]Card_Blue'
					else N'' end AS Color

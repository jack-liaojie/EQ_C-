...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_WR_GetEventListByDisciplineCode]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_WR_GetEventListByDisciplineCode]
GO
/****** Object:  StoredProcedure [dbo].[Proc_WR_GetEventListByDisciplineID]    Script Date: 09/18/2009 11:17:10 ******/
SET ANSI_NULLS ON
GO
...
--名    称：[Proc_WR_GetEventListByDisciplineCode]
...
CREATE PROCEDURE [dbo].[Proc_WR_GetEventListByDisciplineCode](
												@LanguageCode		CHAR(3)
)
As
Begin
SET NOCOUNT ON 
	...
	SELECT @DisciplineID = D.F_DisciplineID FROM TS_Discipline AS D WHERE D.F_DisciplineCode = N'WR'

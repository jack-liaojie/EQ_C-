IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_WR_GetWeighInList_OtherInfo]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_WR_GetWeighInList_OtherInfo]
GO
...
--名    称: [Proc_Report_WR_GetWeighInList_OtherInfo]
--描    述: 获取体重称量表的其他信息.
--创 建 人: 邓年彩
--日    期: 2010年11月8日 星期一
--修改记录：
...
CREATE PROCEDURE [dbo].[Proc_Report_WR_GetWeighInList_OtherInfo]
	@EventID						INT,
	@LanguageCode					CHAR(3)
AS
BEGIN
SET NOCOUNT ON
	...
	SELECT REPLACE(REPLACE(ED.F_EventLongName, N'Women''s ', N''), N'Men''s ', N'') AS [WeightCategory]
		...
		, dbo.Func_Report_WR_GetDateTime(E.F_OpenDate, 6, 'ENG') AS [Date]
		, N'7:00' AS [Time]
		, (

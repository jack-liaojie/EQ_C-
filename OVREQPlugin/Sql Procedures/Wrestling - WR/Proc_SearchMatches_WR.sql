IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_SearchMatches_WR]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_SearchMatches_WR]
...
Create PROCEDURE [dbo].[Proc_SearchMatches_WR]
...
SET NOCOUNT ON
	...
	SELECT M.F_RaceNum AS [R.N.]
		...
		, C1.F_NOC AS [Red]
		, C2.F_NOC AS [Blue]
		, case when E.F_PlayerRegTypeID=1 then  dbo.[Func_Report_WR_GetResultByMatchID](M.F_MatchID, 1, 'ENG') 
			else dbo.[Func_Report_WR_GetResultByMatchID](M.F_MatchID, 1, 'ENG') end AS [Result]

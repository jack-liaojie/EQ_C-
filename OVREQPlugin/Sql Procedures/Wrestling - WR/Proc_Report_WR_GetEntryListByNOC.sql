IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_WR_GetEntryListByNOC]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_WR_GetEntryListByNOC]
GO
...
--名    称: [Proc_Report_WR_GetEntryListByNOC]
--描    述: 摔跤项目获取报表 Entry List by NOC 的主要数据.
--创 建 人: 宁顺泽
--日    期: 2010年9月25日 星期六
--修改记录：
/*			
...
CREATE PROCEDURE [dbo].[Proc_Report_WR_GetEntryListByNOC]
	@DisciplineID						INT,
	@DelegationID						INT,
	@LanguageCode						CHAR(3)
AS
...
SET NOCOUNT ON
	...
	SELECT D.F_DelegationCode AS [NOCCode]
		...
		, REPLACE(REPLACE(REPLACE(ED.F_EventLongName,N'Men''s GRECO-ROMAN',N'GR'),N'Women''s FREESTYLE',N'FW'),N'Men''s FREESTYLE',N'FS') AS [Event]
		, X.F_EventCode
		, dbo.Func_Report_WR_GetDateTime(R.F_Birth_Date, 1, 'ENG') AS [DateOfBirth]

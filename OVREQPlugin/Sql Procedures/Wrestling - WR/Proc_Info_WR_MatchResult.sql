IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Info_WR_MatchResult]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Info_WR_MatchResult]
GO
...
CREATE PROCEDURE [dbo].[Proc_Info_WR_MatchResult]
		@MatchID			AS INT
AS
BEGIN
...
SET NOCOUNT ON
	...
	CREATE TABLE #tmp_Duel(	
								Match				  NVARCHAR(100),
								DelegationName_R	  NVARCHAR(100),
								AthleteName_R		  NVARCHAR(100),
								DelegationName_B	  NVARCHAR(100),
								AthleteName_B	      NVARCHAR(100),
								GameTime			  nvarchar(100),
								Score_R 				  NVARCHAR(100),
								Score_B 				  NVARCHAR(100),
								Winner				  NVARCHAR(100),
								[Status]			  NVARCHAR(100)
							)
							
	insert into #tmp_Duel(Match,DelegationName_R,
							AthleteName_R,DelegationName_B,
							AthleteName_B,GameTime,
							Score_R,Score_B,Winner,[Status])
	select 
		...
		,Rd2.F_LongName
		,@GameTime
		,dbo.[Func_Report_WR_GetResultByIDs](M.F_MatchID, 1, 1,1)
		+N'|'
		+dbo.[Func_Report_WR_GetResultByIDs](M.F_MatchID, 1, 2,1)
		+N'|'
		+dbo.[Func_Report_WR_GetResultByIDs](M.F_MatchID, 1, 3,1)
		,dbo.[Func_Report_WR_GetResultByIDs](M.F_MatchID, 2, 1,1)
		+N'|'
		+dbo.[Func_Report_WR_GetResultByIDs](M.F_MatchID, 2, 2,1)
		+N'|'
		+dbo.[Func_Report_WR_GetResultByIDs](M.F_MatchID, 2, 3,1)
	...
	SET		@Content = (SELECT [Row].[Match],  [Row].[DelegationName_R], [Row].[AthleteName_R]
							, [Row].[DelegationName_B], [Row].[AthleteName_B], [Row].[GameTime], [Row].Score_R,Row.Score_B, [Row].[Winner], [Row].[Status]
   ...
   --SELECT cast( @Content AS XML )
	...
	SET @FileName =	@Discipline + @Gender + @Event + N'000' + N'.C73.CHI.1.0'

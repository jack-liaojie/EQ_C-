IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_WR_GetGeneralInfo]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_WR_GetGeneralInfo]
GO
...
--名    称: [Proc_Report_WR_GetGeneralInfo]
--描    述: 摔跤项目报表获取基本信息, 如 Discipline, Event, EventUnit, Venue, Weekday, Date, StartTime, EndTime, AsOfDate, CreateTime 等等.  
--创 建 人: 宁顺泽
--日    期: 2011年10月17日 星期1
--修改记录：
/*			
...
CREATE PROCEDURE [dbo].[Proc_Report_WR_GetGeneralInfo]
	@DisciplineID					INT,
	@EventID						INT,
	@PhaseID						INT,
	@MatchID						INT, 
	...
	*/
	...
	BEGIN
		SELECT @Discipline AS [Discipline]
			...
			, @Venue AS [Venue]
			, dbo.[Func_Report_WR_GetDateTime](@Date, 2, @LanguageCode) AS [Weekday]
			, dbo.[Func_Report_WR_GetDateTime](@Date, 1, @LanguageCode) AS [Date]
			, dbo.[Func_Report_WR_GetDateTime](@StartTime, 4, @LanguageCode) AS [StartTime]
			, dbo.[Func_Report_WR_GetDateTime](@EndTime, 4, @LanguageCode) AS [EndTime]
			, dbo.[Func_Report_WR_GetDateTime](GETDATE(), 1, @LanguageCode) AS [AsOfDate]
			, dbo.[Func_Report_WR_GetDateTime](GETDATE(), 3, @LanguageCode) AS [CreatedTime]
			, CONVERT(DECIMAL(10, 2), 9) AS [X_Portrait]
			, CONVERT(DECIMAL(10, 2), 9) AS [X_Landscape]
...
-- Just for test
EXEC [Proc_Report_WR_GetGeneralInfo] NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'ENG'

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_DataExchange_SH_M0003]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_DataExchange_SH_M0003]
GO
...
----存储过程名称：[Proc_DataExchange_SH_M0003]
----功		  能：射击过程成绩（资格赛）
----作		  者：穆学峰
----日		  期: 2010-11-29 
----修改	记录:

---实时成绩（男子飞碟多向，双向,多双向；女子飞碟多向，双向；男女混合双多向150靶
---资格赛）
-----------------	@Lang; @RSC; @Discipline; @Event; @Phase; @Unit; 
-----------------	@Gender; @Venue; @Date; @DisciplineID; @EventID; @PhaseID;
-----------------   @MatchID; @SessionID; @CourtID

CREATE PROCEDURE [dbo].[Proc_DataExchange_SH_M0003]
		@MatchID			INT,
--		@Auto				INT,
...
EXEC [Proc_DataExchange_SH_M0003]  2188, 'ENG'
EXEC [Proc_DataExchange_SH_M0003]  1, 'ENG'

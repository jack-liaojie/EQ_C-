IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_DataExchange_SH_M0007]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_DataExchange_SH_M0007]
GO
...
SET NOCOUNT ON
	...
	SELECT F_RegisterID, F_Stage, F_SubTotal, [1], [2], [3], [4], [5], [6] FROM 
	(
	...
	SET @OutputHeader = (
					SELECT	[Message].[Version],
							...
							ISNULL(Row.Memo,'') Memo,
						
							Row1.[Group],
							ISNULL(Row1.Result1,'') Result1,
							ISNULL(Row1.Result2,'') Result2,
							ISNULL(Row1.Result3,'') Result3,
							ISNULL(Row1.Result4,'') Result4,
							ISNULL(Row1.Result5,'') Result5,
							ISNULL(Row1.Result6,'') Result6,
							ISNULL(Row1.Result ,'') Result
							
							
							FROM dbo.Func_DataExchange_GetMessageHeader ('SH', '1.0', 'VRS-001', 'M0007', @MatchID, @LanguageCode, 0) [Message]
...
EXEC [Proc_DataExchange_SH_M0007]  328, 'ENG'

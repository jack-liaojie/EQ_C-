...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_SH_GetMatchResult]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_SH_GetMatchResult]
GO
...
----Modified by:
----
CREATE PROCEDURE [dbo].[Proc_Report_SH_GetMatchResult] (	
	@MatchID					INT,
	@LanguageCode				CHAR(3),
	@OrderbyFp					INT = 0
)	
...
SET NOCOUNT ON
	...
	BEGIN
		...
		SELECT A.F_CompetitionPosition, A.F_RegisterID,  
				...
				dbo.Func_SH_GetChineseDate(E.F_Birth_Date), F_IRMID,
				CAST(CAST(A.F_Points/10.0 AS Decimal(10,1) ) AS NVARCHAR(10)), 
				A.F_Rank, [dbo].[Func_SH_GetBay](F_CompetitionPositionDes1)
		...
		-- 25m 速射决赛 用整型表示			  
		IF @RegType = 1 AND @SexCode = 'M' AND @EventInfo = '25RF'
		BEGIN
			UPDATE  #TMP SET 
	...
	BEGIN
		...
		BEGIN
			...
			WHILE @@FETCH_STATUS = 0
			BEGIN
				INSERT #T_QResult(REG_ID, CP, S1, S2, S3, S4, S5, S6, S7, S8, S9, S10, S11, S12, SCOUNT)
				SELECT REG_ID ,	CP ,S1 ,S2 ,S3 ,S4 ,S5 ,S6 ,S7 ,S8 ,S9 ,S10 ,S11 ,S12 ,	SCOUNT 
				FROM dbo.[Func_SH_GetMatchQualifiedlResult] (@MatchID)
...
--		OR (@MatchCode = '01' AND @EventCode IN('007', '009', '011', '013', '105', '107', '109') )
	...
	BEGIN
		IF LEFT(@EventInfo+'00',2) = '10' AND @PhaseCode = '9' AND @MatchCode = '00'
		BEGIN
			UPDATE #TMP SET Remark1 = 'QS-off: ' + CAST(B.F_Points/10 AS NVARCHAR(10))
			FROM #TMP A
		...
		IF @RegType = 1 AND @SexCode = 'M' AND @EventInfo = '25RF' AND @PhaseCode = '9' AND @MatchCode = '00'
		BEGIN
			UPDATE #TMP SET Remark1 = 'QS-off: ' + CAST(B.F_Points/10 AS NVARCHAR(10))
			FROM #TMP A
			LEFT JOIN (SELECT * FROM TS_Match_Result WHERE F_MatchID = @ShootOff_MatchID) B ON A.RegisterID = B.F_RegisterID
		...
		IF @RegType = 1 AND @SexCode = 'M' AND @EventInfo IN('25P', '50R3P', '50RP', '50P') AND @PhaseCode = '9' AND @MatchCode = '01'
		BEGIN
			UPDATE #TMP SET Remark1 = 'QS-off: ' + CAST(B.F_Points/10 AS NVARCHAR(10))
			FROM #TMP A
			LEFT JOIN (SELECT * FROM TS_Match_Result WHERE F_MatchID = @ShootOff_MatchID) B ON A.RegisterID = B.F_RegisterID
		...
		IF @RegType = 1 AND @SexCode = 'M' AND @EventInfo = '25RF' AND @PhaseCode = '9' AND @MatchCode = '02'
		BEGIN
			UPDATE #TMP SET Remark1 = 'QS-off: ' + CAST(B.F_Points/10 AS NVARCHAR(10))
			FROM #TMP A
			LEFT JOIN (SELECT * FROM TS_Match_Result WHERE F_MatchID = @ShootOff_MatchID) B ON A.RegisterID = B.F_RegisterID
...
-- EXEC Proc_Report_SH_GetMatchResult 3216,'ENG'
-- EXEC Proc_Report_SH_GetMatchResult 265,'ENG',1

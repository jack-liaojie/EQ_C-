IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_TE_GetMatchResults]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_TE_GetMatchResults]
GO
...
----存储过程名称：[Proc_Report_TE_GetMatchResults]
...
CREATE PROCEDURE [dbo].[Proc_Report_TE_GetMatchResults] (	
	@MatchID					INT,
	@LanguageCode				CHAR(3),
	@Position					INT = 0
)	
...
--select * from TS_Match_Result
	...
	SELECT A.F_MatchID, A.F_CompetitionPosition, B.F_PrintLongName AS CompetitorLongName, B.F_PrintShortName AS CompetitiorShortName,
	      D.F_DelegationCode AS NOC, A.F_Rank, A.F_Points AS [Sets]
		, NULL AS Srv, CAST(NULL AS NVARCHAR(10)) AS Set1 , CAST(NULL AS NVARCHAR(10)) AS Set2, CAST(NULL AS NVARCHAR(10)) AS Set3, CAST(NULL AS NVARCHAR(10)) AS Set4, CAST(NULL AS NVARCHAR(10)) AS Set5, F_Points AS Score, E.F_IRMCode AS IRM
	...
	UPDATE A SET A.Set1 = B.Set1, A.Set2 = B.Set2, A.Set3 = B.Set3, A.Set4 = B.Set4, A.Set5 = B.Set5 FROM #Temp_Results AS A LEFT JOIN
		(SELECT F_MatchID, F_CompetitionPosition, Set1, Set2, Set3, Set4, Set5 FROM
			(SELECT A.F_MatchID, 'Set' + CAST(A.F_MatchSplitID AS NVARCHAR(10)) AS F_SetPoints, B.F_CompetitionPosition, CAST( B.F_Points AS NVARCHAR(10)) AS F_Points
	...
	--抢七比分
	SELECT F_MatchID, F_CompetitionPosition, Set1, Set2, Set3, Set4, Set5 INTO #Temp_Results_1 FROM
			(SELECT A.F_MatchID, 'Set' + CAST(A.F_MatchSplitID AS NVARCHAR(10)) AS F_SetPoints, B.F_CompetitionPosition, CAST( B.F_SplitPoints AS NVARCHAR(10)) AS F_SplitPoints
			FROM TS_Match_Split_Info AS A LEFT JOIN TS_Match_Split_Result AS B ON A.F_MatchID = B.F_MatchID AND A.F_MatchSplitID = B.F_MatchSplitID
				WHERE  A.F_MatchID = @MatchID AND A.F_FatherMatchSplitID = 0 AND B.F_Rank = 2 ) AS A 
    ...
    UPDATE A SET A.Set1 = A.Set1 + (CASE WHEN B.Set1 IS NULL THEN '' ELSE '('+ B.Set1 + ')' END), 
        ...
        FROM #Temp_Results AS A LEFT JOIN  #Temp_Results_1 AS B ON A.F_MatchID = B.F_MatchID AND A.F_CompetitionPosition = B.F_CompetitionPosition
	...
	IF @Position = 0 
	BEGIN
		SELECT (CASE F_Rank WHEN 1 THEN 'W' ELSE '' END) AS Result, NOC, CompetitorLongName,CompetitiorShortName, Srv, [Sets], Set1, Set2, Set3, Set4, Set5, Score, IRM FROM #Temp_Results
	END
	ELSE
	BEGIN
		SELECT (CASE F_Rank WHEN 1 THEN 'W' ELSE '' END) AS Result, NOC, CompetitorLongName,CompetitiorShortName, Srv, [Sets], Set1, Set2, Set3, Set4, Set5, Score , IRM FROM #Temp_Results WHERE F_CompetitionPosition = @Position
	END

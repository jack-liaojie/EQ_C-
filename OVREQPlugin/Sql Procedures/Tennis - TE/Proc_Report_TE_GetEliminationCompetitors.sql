...
/****** Object:  StoredProcedure [dbo].[Proc_Report_TE_GetEliminationCompetitors]    Script Date: 06/14/2011 02:37:00 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_TE_GetEliminationCompetitors]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_TE_GetEliminationCompetitors]
GO
...
CREATE PROCEDURE [dbo].[Proc_Report_TE_GetEliminationCompetitors](
			     @EventID	    INT,
				 @PhaseCode		char(10),
                 @LanguageCode  char(3)
)
...
SET NOCOUNT ON 
    ...
    SELECT @PhaseID = A.F_PhaseID, @PhaseName = B.F_PhaseLongName FROM TS_Phase AS A LEFT JOIN TS_Phase_Des AS B ON A.F_PhaseID = B.F_PhaseID AND B.F_LanguageCode = @LanguageCode WHERE A.F_EventID = @EventID AND A.F_PhaseCode = @PhaseCode
	...
	INSERT  INTO #table_report_Competitors(phase) VALUES (@phasename)
	...
	INSERT INTO #table_Competitors (F_SportID, F_DisciplineID, F_EventID, F_PhaseID, F_MatchID, F_MatchOrder, F_CompetitionPosition, F_RegisterID,
		...
		SELECT  F.F_SportID, E.F_DisciplineID, B.F_EventID, B.F_PhaseID, A.F_MatchID, A.F_Order, D.F_CompetitionPosition, D.F_RegisterID,
			...
			FROM TS_Match AS A LEFT JOIN TS_Phase AS B ON A.F_PhaseID = B.F_PhaseID 
			      LEFT JOIN TS_Match_Des AS C ON C.F_MatchID = A.F_MatchID AND C.F_LanguageCode = @LanguageCode
			      RIGHT JOIN TS_Match_Result AS D ON A.F_MatchID = D.F_MatchID
			      LEFT JOIN TS_Event AS E ON B.F_EventID = E.F_EventID
			      LEFT JOIN TS_Discipline AS F ON E.F_DisciplineID = F.F_DisciplineID
				 WHERE B.F_PhaseID = @PhaseID AND E.F_EventID = @EventID
	...
	UPDATE #table_Competitors SET F_RegisterName = B.F_PrintLongName FROM #table_Competitors AS A LEFT JOIN TR_Register_Des AS B ON A.F_RegisterID = B.F_RegisterID AND B.F_LanguageCode = @LanguageCode
	UPDATE #table_Competitors SET F_RegisterName = '(轮空)' WHERE F_RegisterID = -1
	UPDATE #table_Competitors SET F_FederationName = C.F_DelegationLongName FROM #table_Competitors AS A LEFT JOIN TR_Register AS B ON A.F_RegisterID = B.F_RegisterID 
			LEFT JOIN TC_Delegation_Des AS C ON B.F_DelegationID = C.F_DelegationID AND C.F_LanguageCode = @LanguageCode

	-- 用来存放位置号
	UPDATE #table_Competitors SET F_InscripNum = F_InscriptionNum from #table_Competitors as A 
				left join tr_inscription as B on A.F_RegisterID = B.F_RegisterID and A.F_EventId = B.F_EventID
					 
	-- 种子号
	UPDATE #table_Competitors SET F_Seed = B.F_Seed from #table_Competitors as A 
				left join tr_inscription as B on A.F_RegisterID = B.F_RegisterID and A.F_EventId = B.F_EventID

	UPDATE #table_Competitors SET F_RegisterName = '(' + cast(F_Seed as nvarchar(100)) + ')' + F_RegisterName where F_Seed is not null
...
--	DELETE FROM #table_Competitors WHERE F_RegisterName = 'BYE'
	...
	DECLARE @InscriNum AS INT
	DECLARE @ROW AS INT
	SET @ROW = 1
	DECLARE Competitor_CURSOR CURSOR FOR SELECT F_PhaseName, F_RegisterName, F_FederationName, F_InscripNum FROM #table_Competitors 
...
--	exec Proc_Report_TE_GetEliminationCompetitors 537,'8to4','CHN'
GO


--exec Proc_Report_TE_GetEliminationCompetitors 1, '4', 'chn'
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_TE_GetTeamMatchResult]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_TE_GetTeamMatchResult]
GO
...
CREATE PROCEDURE [dbo].[Proc_Report_TE_GetTeamMatchResult] (	
	@PhaseID			INT,
...
SET NOCOUNT ON
		...
		create table #table_round_match(
			...
			F_SplitStatus       INT,
			F_SplitCode         NVARCHAR(10),
			F_SplitName         NVARCHAR(100),
			F_HSplitPlayer		NVARCHAR(300),
			F_ASplitPlayer		NVARCHAR(300),
			F_SplitPoints       NVARCHAR(20),
			...
			F_HSplitIRMID       INT,
			F_VSplitIRMID       INT,
			F_SplitIRM          NVARCHAR(100),
			F_SetPoints			NVARCHAR(100)
		)

		INSERT INTO #table_round_match(F_PhaseOrder, F_PhaseLongName, F_PhaseShortName, F_EventName, F_RoundName,  F_Order, F_MatchNum, F_MatchID, F_PhaseID, F_SportName, F_DisciplineName
		                               , F_MatchSplitID, F_SplitCode, F_SplitStatus)
		   SELECT  B.F_Order, E.F_PhaseLongName, E.F_PhaseShortName, D.F_EventLongName, F.F_RoundLongName,  A.F_Order, A.F_RaceNum, A.F_MatchID, A.F_PhaseID,
		           GD.F_DisciplineLongName, SD.F_SportLongName, TT.F_MatchSplitID, TT.F_MatchSplitCode, TT.F_MatchSplitStatusID      
		      FROM TS_Match_Split_Info as TT LEFT JOIN TS_Match AS A  ON TT.F_MatchID = A.F_MatchID AND TT.F_FatherMatchSplitID = 0
		     ...
		     AND (TT.F_MatchSplitStatusID IS NOT NULL OR TT.F_MatchSplitStatusID <> 0)
		            Order By B.F_Order, A.F_MatchID, CAST( TT.F_MatchSplitCode AS INT)
		...
		UPDATE #table_round_match SET F_SplitName = CASE F_SplitCode WHEN '1' THEN '单打' WHEN'2' THEN '单打' WHEN '3' THEN '双打' END 
		
		UPDATE #table_round_match SET F_HSplitPlayer = C.F_PrintLongName, F_HSplitIRMID = B.F_IRMID
		     FROM #table_round_match  AS A LEFT JOIN TS_Match_Split_Result AS  B ON A.F_MatchID = B.F_MatchID AND A.F_MatchSplitID = B.F_MatchSplitID 
		          LEFT JOIN TR_Register_Des AS C ON B.F_RegisterID = C.F_RegisterID  AND C.F_LanguageCode = @LanguageCode
		          WHERE B.F_CompetitionPosition = 1
                     
	    UPDATE #table_round_match SET F_ASplitPlayer = C.F_PrintLongName, F_VSplitIRMID = B.F_IRMID
		     FROM #table_round_match  AS A LEFT JOIN TS_Match_Split_Result AS  B ON A.F_MatchID = B.F_MatchID AND A.F_MatchSplitID = B.F_MatchSplitID 
		          LEFT JOIN TR_Register_Des AS C ON B.F_RegisterID = C.F_RegisterID  AND C.F_LanguageCode = @LanguageCode
		          WHERE B.F_CompetitionPosition = 2
		
	    UPDATE #table_round_match SET F_SplitIRM = F_HSplitPlayer + B.F_IRMLongName 
                 FROM #table_round_match AS A LEFT JOIN TC_IRM_Des AS B ON A.F_HSplitIRMID = B.F_IRMID AND B.F_LanguageCode = @LanguageCode
                     WHERE F_HSplitIRMID IS NOT NULL
                     
        UPDATE #table_round_match SET F_SplitIRM = F_ASplitPlayer + B.F_IRMLongName 
                 FROM #table_round_match AS A LEFT JOIN TC_IRM_Des AS B ON A.F_VSplitIRMID = B.F_IRMID AND B.F_LanguageCode = @LanguageCode
                     WHERE F_VSplitIRMID IS NOT NULL
                     
		UPDATE #table_round_match SET F_SplitPoints = CAST( B.F_Points AS NVARCHAR(10)) + ':' + CAST( C.F_Points AS NVARCHAR(10)) 
		    FROM #table_round_match AS A 
		...
		update #table_round_match set F_SetPoints = dbo.Fun_TE_GetMatchSplitsPointsDes(F_MatchID,1,F_MatchSplitID) from #table_round_match
		
		--delete from #table_round_match where F_HomePlayer is null
		--delete from #table_round_match where F_AwayPlayer is null
		delete from #table_round_match WHERE F_SplitStatus = 0
		
		select * from #table_round_match
	
SET NOCOUNT OFF
END

--	select * from ts_phase
-- Proc_Report_TE_GetMatchInfoByRound 249,1

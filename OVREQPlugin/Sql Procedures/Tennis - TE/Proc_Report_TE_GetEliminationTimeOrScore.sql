...
/****** Object:  StoredProcedure [dbo].[Proc_Report_TE_GetEliminationTimeOrScore]    Script Date: 06/14/2011 07:05:11 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_TE_GetEliminationTimeOrScore]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_TE_GetEliminationTimeOrScore]
GO
...
CREATE PROCEDURE [dbo].[Proc_Report_TE_GetEliminationTimeOrScore](
				 @EventID		INT,
...
SET NOCOUNT ON 
	...
	INSERT INTO #table_Competitors (F_SportID, F_DisciplineID, F_EventID, F_PhaseID, F_MatchID, F_MatchNum, F_MatchOrder, F_MatchStatusID, F_CompetitionPosition, F_RegisterID,
		...
		SELECT  G.F_SportID, F.F_DisciplineID, B.F_EventID, B.F_PhaseID, A.F_MatchID, A.F_MatchNum, A.F_Order, A.F_MatchStatusID, D.F_CompetitionPosition, D.F_RegisterID,
			...
			FROM TS_Match AS A LEFT JOIN TS_Phase AS B ON A.F_PhaseID = B.F_PhaseID 
			    ...
			    LEFT JOIN ts_session AS E ON E.F_SessionID = A.F_SessionID  			      
				LEFT JOIN TS_Event AS F ON B.F_EventID = F.F_EventID
			    LEFT JOIN TS_Discipline AS G ON F.F_DisciplineID = G.F_DisciplineID
				WHERE B.F_PhaseCode = @PhaseCode AND B.F_EventID = @EventID
...
--	DELETE FROM #table_Competitors WHERE F_RegisterName = 'BYE'
	...
	SELECT A.F_MatchNum, A.F_MatchName, A.F_RegisterID, B.F_RegisterID, A.F_RegisterName AS Home, B.F_RegisterName AS Away, 
		...
		FROM #table_Competitors AS A INNER JOIN #table_Competitors AS B ON A.F_MatchID = B.F_MatchID 
			AND A.F_CompetitionPosition = 1 AND B.F_CompetitionPosition = 2 

	update #table_Match_Competitors set F_IrmName = dbo.fun_Report_TE_GetMatchIRM(F_MatchID, '', '', 'CHN')
    
    IF(@EventType <> 3)
    BEGIN
        UPDATE #table_Match_Competitors SET F_Score = dbo.Fun_TE_GetMatchSplitsPointsDes(F_MatchID,2, 0)
    END
    ELSE
    BEGIN
        UPDATE #table_Match_Competitors SET F_Score = dbo.Fun_Report_TE_GetMatchResultDes_ForTeam(F_MatchID)
    END
...
--exec [Proc_Report_TE_GetEliminationTimeOrScore] 5,'3', 'chn'

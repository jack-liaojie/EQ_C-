IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_BK_GetNOCListByDiscipline]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_BK_GetNOCListByDiscipline]
GO
...
--名    称：[Proc_Report_BK_GetNOCListByDiscipline]
...
CREATE PROCEDURE [dbo].[Proc_Report_BK_GetNOCListByDiscipline](
												@DisciplineID	    INT,
                                                @DelegationID       INT,
												@LanguageCode		CHAR(3)
)

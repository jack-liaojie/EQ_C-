IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_BK_GetTeamColours]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_BK_GetTeamColours]
GO
...
--名    称：[Proc_Report_BK_GetTeamColours]
--描    述：得到代表团下各个队的服饰颜色
--参数说明： 
--说    明：
--创 建 人：杨佳鹏
...
CREATE PROCEDURE [dbo].[Proc_Report_BK_GetTeamColours]
(
               @DelegationID        INT,
			   @LanguageCode		CHAR(3)
)
...
SET NOCOUNT ON 
    ...
    BEGIN
			select a.F_RegisterID,b.F_UniformID,b.F_Shirt,b.F_Shorts,b.F_Socks,
				   ...
				   CASE WHEN @LanguageCode = 'CHN'
				   THEN
					   '第'+CONVERT(NVARCHAR(100),ROW_NUMBER() OVER(PARTITION BY a.F_RegisterID ORDER BY b.F_Order ))+'套'
				   ELSE
			...
			left join TR_Uniform as b on a.F_RegisterID=b.F_RegisterID

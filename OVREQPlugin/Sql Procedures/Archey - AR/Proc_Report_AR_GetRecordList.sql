IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_AR_GetRecordList]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_AR_GetRecordList]
GO
...
--名    称: [Proc_Report_AR_GetRecordList]
...
CREATE PROCEDURE [dbo].[Proc_Report_AR_GetRecordList]
	@EventID					INT,
	@RegTypeID					INT,
	@LanguageCode				CHAR(3)
AS
...
SET NOCOUNT ON
    ...
    WHERE  NER.F_IsNewCreated =0 AND  E.F_SexCode = @SexCode AND E.F_PlayerRegTypeID = @RegTypeID
	END
...
/*
EXEC [Proc_Report_AR_GetRecordList] 1,1,'CHN'
*/
GO

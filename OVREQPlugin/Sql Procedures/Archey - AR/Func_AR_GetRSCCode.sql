IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Func_AR_GetRSCCode]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].Func_AR_GetRSCCode
GO
...
----名    称: [Func_AR_GetRSCCode]
----描    述: 射箭项目报表获取 RSC Code, 即 DISCIPLINE(2), SEX(1), EVENT(3), PHASE(1), Match(2) 9位等连接起来的字符串. 
----参数说明: 
----说    明: 
----创 建 人: 崔凯	
----日   期：2012年09月12日
...
CREATE FUNCTION [dbo].[Func_AR_GetRSCCode]
(
	@DisciplineID					INT,
	@EventID						INT,
	@PhaseID						INT,
...
-- Just for test
EXEC [Func_AR_GetRSCCode] NULL, NULL, NULL, NULL, NULL, NULL, N'ALL'
EXEC [Func_AR_GetRSCCode] 1, 1, 1, 1, 1, NULL, N'C73A'

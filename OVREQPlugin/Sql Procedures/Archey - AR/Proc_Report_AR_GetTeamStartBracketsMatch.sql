IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_AR_GetTeamStartBracketsMatch]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_AR_GetTeamStartBracketsMatch]
GO
...
--名    称：[Proc_Report_AR_GetTeamStartBracketsMatch]
...
CREATE PROCEDURE [dbo].[Proc_Report_AR_GetTeamStartBracketsMatch](
                       @EventID         INT,
                       @PhaseCode       NVARCHAR(10),   ---如果@PhaseCode == '-1'，返回所有Match信息
                       @MatchCode		NVARCHAR(10),   ---如果@MatchCode == '-1'，返回所有Match信息
...
SET NOCOUNT ON 
    ...
    INSERT INTO #Tmp_Table(F_MatchID, F_MatchCode,F_APlayerID, F_APos, F_AMatchFrom, F_AMatchFromCourt, F_AMatchFromDate, F_BPlayerID, F_BPos, 
    ...
    SELECT A.F_MatchID,A.F_MatchCode
    ...
    UPDATE #Tmp_Table SET F_APrintLN = 'BYE' WHERE F_APlayerID = -1
    UPDATE #Tmp_Table SET F_BPrintLN = 'BYE' WHERE F_BPlayerID = -1
	...
	UPDATE #Tmp_Table SET F_AMembers1 = RD.F_PrintLongName FROM #Tmp_Table AS A 
	...
	UPDATE #Tmp_Table SET F_AMembers2 = RD.F_PrintLongName FROM #Tmp_Table AS A 
		LEFT JOIN TR_Register_Member AS RM ON RM.F_RegisterID = A.F_APlayerID AND RM.F_Order = 2
		LEFT JOIN TR_Register_Des AS RD ON RD.F_RegisterID = RM.F_MemberRegisterID AND RD.F_LanguageCode = @LanguageCode 
	UPDATE #Tmp_Table SET F_AMembers3 = RD.F_PrintLongName FROM #Tmp_Table AS A 
		LEFT JOIN TR_Register_Member AS RM ON RM.F_RegisterID = A.F_APlayerID AND RM.F_Order = 3
		LEFT JOIN TR_Register_Des AS RD ON RD.F_RegisterID = RM.F_MemberRegisterID AND RD.F_LanguageCode = @LanguageCode 
	
	UPDATE #Tmp_Table SET F_BMembers1 = RD.F_PrintLongName FROM #Tmp_Table AS A 
		LEFT JOIN TR_Register_Member AS RM ON RM.F_RegisterID = A.F_BPlayerID AND RM.F_Order = 1
		LEFT JOIN TR_Register_Des AS RD ON RD.F_RegisterID = RM.F_MemberRegisterID AND RD.F_LanguageCode = @LanguageCode 
	UPDATE #Tmp_Table SET F_BMembers2 = RD.F_PrintLongName FROM #Tmp_Table AS A 
		LEFT JOIN TR_Register_Member AS RM ON RM.F_RegisterID = A.F_BPlayerID AND RM.F_Order = 2
		LEFT JOIN TR_Register_Des AS RD ON RD.F_RegisterID = RM.F_MemberRegisterID AND RD.F_LanguageCode = @LanguageCode 
	UPDATE #Tmp_Table SET F_BMembers3 = RD.F_PrintLongName FROM #Tmp_Table AS A 
	...
	SELECT  *  FROM #Tmp_Table WHERE @MatchCode=-1 OR  (@MatchCode !=-1 AND F_MatchCode = @MatchCode)
...
/*
exec Proc_Report_AR_GetTeamStartBracketsMatch 3,'1','01',@LanguageCode
*/
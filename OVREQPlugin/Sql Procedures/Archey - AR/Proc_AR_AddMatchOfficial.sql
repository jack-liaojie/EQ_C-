IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_AR_AddMatchOfficial]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_AR_AddMatchOfficial]
GO
...
CREATE PROCEDURE [dbo].[Proc_AR_AddMatchOfficial](
												@MatchID		    INT,
                                                @RegisterID         INT,
                                                @FunctionID         INT, 
...
SET NOCOUNT ON 
    ...
    VALUES (@pMatchID, @ServantNum, @RegisterID, @FunctionID)

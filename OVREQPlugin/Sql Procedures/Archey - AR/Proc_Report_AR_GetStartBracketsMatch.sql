IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_AR_GetStartBracketsMatch]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_AR_GetStartBracketsMatch]
GO
...
--名    称：[Proc_Report_AR_GetStartBracketsMatch]
...
SET NOCOUNT ON 
    ...
    ,F_ABackNumber = TRI.F_InscriptionNum, F_AQRank = QR.F_Rank, F_AScore = QR.F_Points,F_ARecord = '',F_ARank= X.F_Rank
    ...
    ,F_BBackNumber = TRI.F_InscriptionNum, F_BQRank = QR.F_Rank, F_BScore = QR.F_Points ,F_BRecord = '',F_BRank= X.F_Rank
    FROM #Tmp_Table AS A 
...
/*
exec Proc_Report_AR_GetStartBracketsMatch 1,'7','1'
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_AR_UpdatePlayerShootOffArrows]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_AR_UpdatePlayerShootOffArrows]
GO
...
--名    称: [Proc_AR_UpdatePlayerShootOffArrows]
--描    述: 射箭项目,更新某人一次箭靶信息
--参数说明: 
--说    明: 
--创 建 人: 崔凯
...
CREATE PROCEDURE [dbo].[Proc_AR_UpdatePlayerShootOffArrows]
	@MatchID				INT,
...
SET NOCOUNT ON
	...
	BEGIN
		...
		SET @IsX = 1
	END
	UPDATE TS_Match_Split_Result SET F_Points = @Ring, F_SplitInfo1 = (CASE WHEN @Ring ='10' OR UPPER(@Ring) = 'X' THEN 1 ELSE 0 END), F_SplitInfo2 =@IsX
...
/*
exec Proc_AR_UpdatePlayerShootOffArrows 1,1,1,'1',9,0,OUTPUT
*/

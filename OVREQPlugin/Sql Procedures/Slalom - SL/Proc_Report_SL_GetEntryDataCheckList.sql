IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_SL_GetEntryDataCheckList]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_SL_GetEntryDataCheckList]
GO
...
--名    称: [Proc_Report_SL_GetEntryDataCheckList]
--描    述: 激流回旋报表 C38 (Entry Data Checklist), 获取主要信息.
--参数说明: 
--说    明: 
--创 建 人: 邓年彩
--日    期: 2010年1月13日
...
CREATE PROCEDURE [dbo].[Proc_Report_SL_GetEntryDataCheckList]
	@DisciplineID				INT,
	@DelegationID				INT,
	@NOCID						CHAR(3),
	@LanguageCode				CHAR(3)
...
EXEC [Proc_Report_SL_GetEntryDataCheckList] 67, -1, '-1', 'ENG'
EXEC [Proc_Report_SL_GetEntryDataCheckList] 67, -1, 'CHN', 'ENG'

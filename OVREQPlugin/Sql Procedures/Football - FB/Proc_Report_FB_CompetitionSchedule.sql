IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_FB_CompetitionSchedule]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_FB_CompetitionSchedule]
GO
...
----日		  期: 2010-03-20
  ...
  --	杨佳鹏 2011-03-08
  --	增加字段长度，以防被截断
create procedure [dbo].[Proc_Report_FB_CompetitionSchedule]
( 
   @EventID int,
   @LanguageCode char(3)
)
...
begin
  ...
  set language English
    ...
    select d.F_Date,
    CASE WHEN @LanguageCode='CHN' THEN ' 休息日' ELSE 'REST DAY' END
    ...
    select dbo.[Func_Report_FB_GetDateTime](F_MatchDate,7,@LanguageCode) as 'MatchDate',F_MatchNum AS 'MatchNumber',F_VenueLongName as 'Venue',F_StartTime as 'Kick-off',
...
--exec Proc_Report_FB_CompetitionSchedule 1,'ENG'
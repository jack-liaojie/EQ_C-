IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Func_Report_FB_GetDateTime]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[Func_Report_FB_GetDateTime]
GO
...
--名    称: [Func_Report_FB_GetDateTime]
...
/*			
			日期					修改人		修改内容
			2010年4月13日 星期二	邓年彩		函数对月份和星期进行转化.
			2010年5月11日 星期二	邓年彩		添加 type = 5(Weekday + Date) 的情况.
			2010年5月12日 星期三	邓年彩		type = 1 时, 中文的日期有改变.
*/



CREATE FUNCTION [dbo].[Func_Report_FB_GetDateTime]
(
	@DateTime				DateTime,
	@Type					INT,
	@LanguageCode			CHAR(3)
...
-- Just for test
select dbo.[Func_Report_KR_GetDateTime](GETDATE(), 1, 'ENG')
select dbo.[Func_Report_KR_GetDateTime](GETDATE(), 2, 'ENG')
select dbo.[Func_Report_KR_GetDateTime](GETDATE(), 3, 'ENG')
select dbo.[Func_Report_KR_GetDateTime](GETDATE(), 4, 'ENG')

select dbo.[Func_Report_KR_GetDateTime](GETDATE(), 1, 'CHN')
select dbo.[Func_Report_KR_GetDateTime](GETDATE(), 2, 'CHN')
select dbo.[Func_Report_KR_GetDateTime](GETDATE(), 3, 'CHN')
select dbo.[Func_Report_KR_GetDateTime](GETDATE(), 4, 'CHN')

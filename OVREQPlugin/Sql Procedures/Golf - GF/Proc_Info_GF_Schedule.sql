IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Info_GF_Schedule]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Info_GF_Schedule]
GO
...
CREATE PROCEDURE [dbo].[Proc_Info_GF_Schedule]
		@DateID			INT
AS
BEGIN
...
SET NOCOUNT ON
	...
	SET @DisciplineCode = N'GF'
	SET @OutputDiscCode = N'GO'
	...
	BEGIN
		INSERT INTO #Temp_Schedule ([Code], [Name], [StartDate], [StartTime], [VenueCode], [Order], [F_MatchID],
		 [F_PhaseID], [F_EventID], [F_DisciplineID], [F_MatchOrder], [F_PhaseOrder], [F_EventOrder], 
		 [PhaseCode], [MatchCode], [IsTeamEvent])
		SELECT @OutputDiscCode + Gender.F_GenderCode + 
		       [dbo].[Func_GF_GetOutputEventCode](Gender.F_GenderCode, E.F_EventCode) + 
		       P.F_PhaseCode + M.F_MatchCode AS [Code],
		       case when ISNULL(PS.F_PhaseLongName,'') = ISNULL(MS.F_MatchLongName,'')
					then DS.F_DisciplineLongName + ES.F_EventLongName + ISNULL(PS.F_PhaseLongName,'')
		       else
					DS.F_DisciplineLongName + ES.F_EventLongName + PS.F_PhaseLongName + ISNULL(MS.F_MatchLongName,'') 
					end AS [Name],
			   ...
			   , P.F_PhaseCode, M.F_MatchCode, [dbo].[Func_GF_GetIsTeamEvent](Gender.F_GenderCode, E.F_EventCode) 
    ...
    DELETE FROM #Temp_Schedule WHERE IsTeamEvent = 0
	...
	INSERT INTO #Temp_Detail (Code, [Type1], [Type2], [FileName],[PhaseCode],[MatchCode])
		...
		WHERE PhaseCode = '1' AND IsTeamEvent = 0
	INSERT INTO #Temp_Detail (Code, [Type1], [Type2], [FileName],[PhaseCode],[MatchCode])
		SELECT Code, 2 AS [Type1], 1 AS [Type2], NULL, [PhaseCode],[MatchCode] FROM #Temp_Schedule 
		WHERE PhaseCode = '1' AND IsTeamEvent = 1
 
	INSERT INTO #Temp_Detail (Code, [Type1], [Type2], [FileName],[PhaseCode],[MatchCode])
		SELECT Code, 4 AS [Type1], 0 AS [Type2], NULL, [PhaseCode],[MatchCode] FROM #Temp_Schedule
		WHERE IsTeamEvent = 0
    
    --select * from #Temp_Detail

	UPDATE #Temp_Detail SET [Type] = N'C51A' WHERE [Type1] = 0

	UPDATE #Temp_Detail SET [Type] = N'C73A' WHERE [Type1] = 1 AND [Type2] = 0
	UPDATE #Temp_Detail SET [Type] = N'C73B' WHERE [Type1] = 1 AND [Type2] = 1

	UPDATE #Temp_Detail SET [Type] = N'C92A' WHERE [PhaseCode] = '1' AND [Type1] = 2 AND [Type2] = 0
	UPDATE #Temp_Detail SET [Type] = N'C92B' WHERE [PhaseCode] = '1' AND [Type1] = 2 AND [Type2] = 1

	UPDATE #Temp_Detail SET [Type] = N'C84B' WHERE [Type1] = 4 AND [Type2] = 0
	...
	--SELECT * FROM #Temp_Schedule
/*	
	UPDATE #Temp_Detail SET [Type] = N'C51A' WHERE [Type] = 'C51A'
	UPDATE #Temp_Detail SET [Type] = N'C73' WHERE [Type] = 'C73A'
	UPDATE #Temp_Detail SET [Type] = N'C73' WHERE [Type] = 'C73B'
	UPDATE #Temp_Detail SET [Type] = N'C92' WHERE [Type] = 'C92A'
	UPDATE #Temp_Detail SET [Type] = N'C92' WHERE [Type] = 'C92B'
*/	
	
	UPDATE #Temp_Schedule SET [Order] = B.DisplayPos
	FROM #Temp_Schedule AS A 
	LEFT JOIN (SELECT ROW_NUMBER() OVER(ORDER BY F_EventOrder, F_MatchOrder) AS DisplayPos,Code

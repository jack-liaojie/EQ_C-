IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_GF_GetMatchHoleInfo]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_GF_GetMatchHoleInfo]
GO
...
----存储过程名称：[Proc_GF_GetMatchHoleInfo]
----功		  能：得到一场比赛的洞的信息
----作		  者：张翠霞 
----日		  期: 2010-09-27

CREATE PROCEDURE [dbo].[Proc_GF_GetMatchHoleInfo] (	
	@MatchID					INT
)	
AS
BEGIN
...
SET NOCOUNT ON
	...
	BEGIN
		...
		SET @SplitSQL = 'Update #TableHole 
			...
			'				
			FROM TS_Match_Split_Info
			WHERE F_Order = ' + @SplitName + ' AND (F_FatherMatchSplitID = 0 OR F_FatherMatchSplitID IS NULL) AND F_MatchID = ' + CAST(@MatchID AS NVARCHAR(10))

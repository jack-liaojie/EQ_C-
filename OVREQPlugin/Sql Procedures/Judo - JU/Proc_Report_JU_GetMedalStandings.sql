IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_JU_GetMedalStandings]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_JU_GetMedalStandings]
GO
...
--名    称: [Proc_Report_JU_GetMedalStandings]
--描    述: 柔道项目报表获取奖牌榜(C95) 
--创 建 人: 邓年彩
--日    期: 2010年10月28日 星期四
--修改记录：
...
CREATE PROCEDURE [dbo].[Proc_Report_JU_GetMedalStandings]
	@DisciplineID						INT,
	@LanguageCode						CHAR(3)
AS
BEGIN

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_JU_GetOneEventMatches]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_JU_GetOneEventMatches]
GO
...
--名    称: [Proc_Report_JU_GetOneEventMatches]
...
CREATE PROCEDURE [dbo].[Proc_Report_JU_GetOneEventMatches]
	@EventID				INT,
	@PhaseID				INT,
	@MatchID				INT,
	@LanguageCode			CHAR(3),
...
SET NOCOUNT ON
	...
	SELECT M.F_MatchID
		...
		, [Result] = CASE @ReportType 
			WHEN N'C75' THEN dbo.[Func_Report_JU_GetResultByMatchID](M.F_MatchID, 1, @LanguageCode)
			WHEN N'Z75' THEN dbo.[Func_Report_JU_GetResultByMatchID](M.F_MatchID, 1, @LanguageCode)
			WHEN N'C51A' THEN dbo.[Func_Report_JU_GetResultByMatchID](M.F_MatchID, 1, @LanguageCode)
			ELSE N''
		END
	FROM TS_Match AS M

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_TVG_JU_Schedule]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_TVG_JU_Schedule]
GO
...
CREATE PROCEDURE [dbo].[Proc_TVG_JU_Schedule]
	@SessionID							INT
AS
BEGIN
SET NOCOUNT ON
	...
	insert into #Temp_Table(F_PhaseID)
		select F_PhaseID from 
			...
			where F_SessionID=@SessionID and C.F_CourtCode=N'F25JU02'
		group by F_PhaseID

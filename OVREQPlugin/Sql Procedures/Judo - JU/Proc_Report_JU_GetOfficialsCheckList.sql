IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_JU_GetOfficialsCheckList]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_JU_GetOfficialsCheckList]
GO
...
--名    称：[Proc_Report_JU_GetOfficialsCheckList]
...
CREATE PROCEDURE [dbo].[Proc_Report_JU_GetOfficialsCheckList](
												@DisciplineID		INT,
												@LanguageCode		CHAR(3)
...
SET NOCOUNT ON 
    ...
    INSERT INTO #Tmp_Table (F_DelegationID, F_RegisterID, F_NOC
		...
		SELECT			
			...
			, B.F_FirstName
			, A.F_SexCode
			, dbo.[Func_Report_JU_GetDateTime](A.F_Birth_Date, 1, 'ENG')
			, A.F_RegisterCode
			, A.F_RegisterNum
			, B.F_PrintLongName
			, B.F_PrintShortName

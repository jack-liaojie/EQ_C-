IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_JU_GetOfficialCommunication]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_JU_GetOfficialCommunication]
GO
...
--名    称：[Proc_Report_JU_GetOfficialCommunication]
...
CREATE PROCEDURE [dbo].[Proc_Report_JU_GetOfficialCommunication]
(
	@DisciplineID		INT,
	@NewsID				INT,
	@Item				INT = -1
...
SET NOCOUNT ON 
	...
	SELECT F_NewsItem
		...
		, F_Text
		, F_Issued_by
		, dbo.[Func_Report_JU_GetDateTime](F_Date, 1, 'ENG') AS F_Date
		, dbo.Fun_GetWeekdayName(F_Date, 'ENG') AS [Weekday_ENG]
		, dbo.Fun_GetWeekdayName(F_Date, 'CHN') AS [Weekday_CHN]
		, dbo.[Func_Report_JU_GetDateTime](F_Date, 4, 'ENG') AS F_Time

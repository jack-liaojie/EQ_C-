IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_JU_GetContestOrder]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_JU_GetContestOrder]
GO
...
--名    称: [Proc_Report_JU_GetContestOrder]
...
CREATE PROCEDURE [dbo].[Proc_Report_JU_GetContestOrder]
	@SessionID						INT,
	@LanguageCode					CHAR(3)
AS
BEGIN
SET NOCOUNT ON
	...
	SELECT S.F_SessionNumber AS [Session]
		...
		, REPLACE(REPLACE(ED.F_EventLongName, N'Women''s ', N''), N'Men''s ', N'') AS [WeightCategory]
	...
	LEFT JOIN
					...
					WHEN MR.F_SourceMatchID IS NOT NULL AND MR.F_SourceMatchRank IS NOT NULL
						THEN CASE @LanguageCode
							...
							ELSE CASE MR.F_SourceMatchRank 
								WHEN 1 THEN N'WINNER OF ' + SM.F_RaceNum
								WHEN 2 THEN N'LOSER OF ' + SM.F_RaceNum
							END
						END
		...
		END,N'') as F2
				...
				then 
					case when FD2.F_FunctionShortName=N'Judge' then TRD2.F_Bib
						when FD3.F_FunctionShortName=N'Judge' then TRD3.F_Bib
					end
			when FD3.F_FunctionShortName=N'Judge' then TRD3.F_Bib			
		END ,N'') as F3

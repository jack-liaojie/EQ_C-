IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_JU_GetCompetitionSchedule]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_JU_GetCompetitionSchedule]
GO
...
--名    称: [Proc_Report_JU_GetCompetitionSchedule]
...
CREATE PROCEDURE [dbo].[Proc_Report_JU_GetCompetitionSchedule]
	@DisciplineID						INT,
	@LanguageCode						CHAR(3)
AS
BEGIN
SET NOCOUNT ON
	...
	SELECT E.F_EventID
		...
		, case when CONVERT(DATETIME, CONVERT(NVARCHAR(20), M.F_StartTime, 114), 114)<>N'10:00' then N'16:00' 
			else CONVERT(DATETIME, CONVERT(NVARCHAR(20), M.F_StartTime, 114), 114) end
	FROM TS_Match AS M

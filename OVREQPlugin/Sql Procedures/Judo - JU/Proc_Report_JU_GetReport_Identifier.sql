IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_JU_GetReport_Identifier]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_JU_GetReport_Identifier]
GO
...
--名    称: [Proc_Report_JU_GetReport_Identifier]
--描    述: 柔道项目报表获取 Report Identifier 的组成元素, 如 Discipline Code(2), Gender(1), Event Code(3), Phase(1), EventUnit(5) 等等. 
--创 建 人: 邓年彩
--日    期: 2010年9月15日 星期三
--修改记录：
/*			
...
CREATE PROCEDURE [dbo].[Proc_Report_JU_GetReport_Identifier]
	@DisciplineID					INT,
	@EventID						INT,
	@PhaseID						INT,
	@MatchID						INT,

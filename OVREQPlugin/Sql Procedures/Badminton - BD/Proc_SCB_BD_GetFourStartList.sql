IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_SCB_BD_GetFourStartList]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_SCB_BD_GetFourStartList]
GO
...
----存储过程名称：[Proc_SCB_BD_GetFourStartList]
----功		  能：获取SCB_BD4场对阵信息
----作		  者：王强
----日		  期: 2011-06-14

CREATE PROCEDURE [dbo].[Proc_SCB_BD_GetFourStartList]
				@SessionID INT,
				@CourtID INT
AS
BEGIN
...
SET NOCOUNT ON
	...
	SELECT CourtNO, PlayerNameA_ENG, PlayerNameA_CHN, PlayerNameB_ENG, PlayerNameB_CHN, NOC_A, NOC_B, ScoreA, ScoreB, EventCode,
			GameScoreA1,GameScoreA2,GameScoreA3,GameScoreB1,GameScoreB2,GameScoreB3, SetScoreA, SetScoreB, CourtName_ENG,CourtName_CHN,
				 ...
				 CourtName_ENG + '（' + CourtName_CHN + '）' AS CourtName_All

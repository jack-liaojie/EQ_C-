IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_BD_EntryByEvent]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_BD_EntryByEvent]
GO
...
--名    称: [Proc_Report_BD_EntryByEvent]
...
/*			
			...
			2011年3月9日        李燕        增加F_InscriptionNum字段
*/


CREATE PROCEDURE [dbo].[Proc_Report_BD_EntryByEvent]
	@DisciplineID				INT,
...
SET NOCOUNT ON
	...
	SELECT X.F_EventID AS [EventID]
			, [NOC] = CASE @GroupType WHEN 2 THEN R.F_NOC ELSE dbo.Fun_BDTT_GetPlayerNOCName(X.F_RegisterID) END
			...
			, X.F_InscriptionNum AS [TechOrder]
		...
		ORDER BY D.F_DelegationCode, X.F_InscriptionNum 

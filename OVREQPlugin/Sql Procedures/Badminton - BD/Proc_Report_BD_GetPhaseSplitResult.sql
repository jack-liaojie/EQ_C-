...
/****** Object:  StoredProcedure [dbo].[Proc_Report_BD_GetPhaseSplitResult]    Script Date: 05/03/2011 17:47:54 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_BD_GetPhaseSplitResult]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_BD_GetPhaseSplitResult]
GO
...
----存储过程名称：[Proc_Report_BD_GetPhaseSplitResult]
----功		  能：得到该Phase下的所有Match的比分等信息
----作		  者：李燕
----日		  期: 2011-03-16
----修 改 记  录：
/*
                  2011-05-03   李燕   增加SetPoint
*/


CREATE PROCEDURE [dbo].[Proc_Report_BD_GetPhaseSplitResult] 
                   (	
					@PhaseID			    INT,
					@LanguageCode           CHAR(3)		
                   )	
...
SET NOCOUNT ON
       ...
       ,[dbo].[Fun_Report_BD_GetMatchResultDes](A.F_MatchID, 2, 0 )
        FROM #Temp_Phase AS TP LEFT JOIN TS_Match_Split_Info AS MSI  ON TP.F_MatchID = MSI.F_MatchID
        LEFT JOIN TS_Match AS A ON MSI.F_MatchID = A.F_MatchID
        ...
        WHERE MSI.F_FatherMatchSplitID = 0 
       ...
       UPDATE #Temp_Table SET F_AGame2 = C.F_Points, F_BGame2 = D.F_Points, F_AGame2IRMID = C.F_IRMID, F_BGame2IRMID = D.F_IRMID  
        ...
        UPDATE #Temp_Table SET F_BSplitIRM = (CASE WHEN A.F_ASplitIRMID IS NULL THEN '' ELSE '(' + B.F_IRMCODE + ')' END) FROM #Temp_Table AS A LEFT JOIN TC_IRM AS B ON A.F_BSplitIRMID = B.F_IRMID
        ...
        UPDATE #Temp_table SET F_SplitScoreDes = ISNULL(CAST(F_SplitAScore AS NVARCHAR(10)), '') +  F_ASplitIRM +' : ' + ISNULL(CAST(F_SplitBScore AS NVARCHAR(10)), '') + F_BSplitIRM
        ...
        UPDATE #Temp_Table SET F_Game2Des = ISNULL(CAST(F_AGame2 AS NVARCHAR(10)), '') + F_AGame1IRM + ' : ' + ISNULL(CAST(F_BGame2 AS NVARCHAR(10)), '') + F_BGame1IRM
        UPDATE #Temp_Table SET F_Game3Des = ISNULL(CAST(F_AGame3 AS NVARCHAR(10)), '') + F_AGame1IRM + ' : ' + ISNULL(CAST(F_BGame3 AS NVARCHAR(10)), '') + F_BGame1IRM
        ...
        SELECT * FROM #Temp_Table order by f_matchid, F_SplitID

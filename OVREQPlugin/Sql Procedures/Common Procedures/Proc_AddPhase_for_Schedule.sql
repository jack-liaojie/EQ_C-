if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[proc_AddPhase_for_Schedule]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[proc_AddPhase_for_Schedule]
GO
...
----名   称：[proc_AddPhase_for_Schedule]
...
SET NOCOUNT ON
	...
	BEGIN TRANSACTION --设定事务
		...
		INSERT INTO TS_PHASE (F_EventID, F_FatherPhaseID, F_PhaseCode, F_OpenDate, F_CloseDate, F_PhaseStatusID, F_PhaseNodeType, F_Order, F_PhaseIsPool, F_PhaseHasPools, F_PhaseType, F_PhaseSize, F_PhaseRankSize, F_PhaseIsQual, F_PhaseInfo)
    		VALUES (@EventID, @FatherPhaseID, @PhaseCode, @OpenDate, @CloseDate, @PhaseStatusID, @PhaseNodeType, @Order, @IsPool, @HasPools, @PhaseType, @PhaseSize, @PhaseRankSize, @PhaseIsQual, @PhaseInfo)

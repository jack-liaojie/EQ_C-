...
/****** Object:  StoredProcedure [dbo].[Proc_Export_Athlete]    Script Date: 02/10/2011 16:13:40 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Export_Athlete]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Export_Athlete]
GO
...
----存储过程名称：[Proc_Export_Athlete]
----功		  能：运动员基础信息(M0111)，仅仅是单个运动员，不包含任何的团体和双打组合的信息，不包含任何报项相关的信息。
----作		  者：郑金勇
----日		  期: 2010-04-19 
----修改	记录:
...
-----------------2	2011-01-12	对于输入参数@LanguageCode的特殊处理
-----------------	当 @LanguageCode为'CHN'时要求在消息头里面的显示为 'CHI'.特殊处理！
...
CREATE PROCEDURE [dbo].[Proc_Export_Athlete]

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[proc_info_Records]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[proc_info_Records]
...
----存储过程名称：[proc_info_Records]
----功		  能：为Info系统服务，获取运动会破纪录信息
----作		  者：穆学峰 
----日		  期: 2009-11-11 

CREATE PROCEDURE [dbo].[proc_info_Records]
...
--SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	CREATE TABLE #table_tmp(
		...
		COUNTRY	Nvarchar(50),
		MARK	Nvarchar(100),
		RECORD_EVENT	Nchar(3),
		RECORD_EVENT_PARENT	Nchar(3),
		RECORD_PHASE	Nchar(1),
		RECORD_POOL	Nchar(2),
		RECORD_EVENT_UNIT	Nchar(5),
		IS_EXEQUO	Tinyint,
		FLAG_BROKEN	Tinyint		
 	)

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_GetMatchSplitTree]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_GetMatchSplitTree]
...
--名    称：[Proc_GetMatchSplitTree]
--描    述：展开一个Match下所有的Split的信息
--参数说明： 
--说    明：
--创 建 人：郑金勇
--日    期：2009年04月13日

CREATE PROCEDURE [dbo].[Proc_GetMatchSplitTree](
				 @MatchID			INT
)
As
Begin

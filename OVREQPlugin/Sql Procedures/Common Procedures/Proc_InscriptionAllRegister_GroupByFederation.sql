IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_InscriptionAllRegister_GroupByFederation]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_InscriptionAllRegister_GroupByFederation]
GO
/****** Object:  StoredProcedure [dbo].[Proc_InscriptionAllRegister_GroupByFederation]    Script Date: 11/16/2009 13:46:21 ******/
SET ANSI_NULLS ON
GO
...
--名    称: [Proc_InscriptionAllRegister_GroupByFederation]
...
CREATE PROCEDURE [dbo].[Proc_InscriptionAllRegister_GroupByFederation]
	@EventID				INT,
	@LanguageCode			CHAR(3),
...
SET NOCOUNT ON
	...
	INSERT INTO  #table_group(F_GroupID)
	  SELECT CAST(F_FederationID AS NVARCHAR(9)) FROM TS_ActiveFederation

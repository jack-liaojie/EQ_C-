IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_WL_RemoveMatchOfficial]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_WL_RemoveMatchOfficial]
GO
...
--名    称：[Proc_WL_RemoveMatchOfficial]
--描    述：删除Match下的官员
--参数说明： 
--说    明：
--创 建 人：崔凯
...
CREATE PROCEDURE [dbo].[Proc_WL_RemoveMatchOfficial](
												@MatchID		    INT,
                                                @RegisterID         INT,

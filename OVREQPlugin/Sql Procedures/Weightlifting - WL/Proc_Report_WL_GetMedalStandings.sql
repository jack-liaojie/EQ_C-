IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_WL_GetMedalStandings]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_WL_GetMedalStandings]
GO
...
--名    称: [Proc_Report_WL_GetMedalStandings]
--描    述: 项目报表获取奖牌榜(C95) 
--参数说明: 
--说    明: 
--创 建 人: 吴定昉
...
CREATE PROCEDURE [dbo].[Proc_Report_WL_GetMedalStandings]
	@DisciplineID						INT,
	@LanguageCode			CHAR(3)
AS
BEGIN
...
-- Just for test
EXEC [Proc_Report_WL_GetMedalStandings] 67,'eng'

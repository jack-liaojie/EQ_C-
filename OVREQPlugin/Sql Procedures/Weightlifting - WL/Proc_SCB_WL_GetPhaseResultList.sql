IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_SCB_WL_GetPhaseResultList]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_SCB_WL_GetPhaseResultList]
GO
...
----存储过程名称：[Proc_SCB_WL_GetPhaseResultList]
----功		  能：SCB得到当前比赛信息
----作		  者：崔凯
----日		  期: 2011-02-28 

CREATE PROCEDURE [dbo].[Proc_SCB_WL_GetPhaseResultList]
             @MatchID         INT
...
SET NOCOUNT ON
	...
	(
		...
		Flag					NVARCHAR(20),
		[Group_ENG]             NVARCHAR(30),
		[Group_CHN]             NVARCHAR(30),
	...
	(
	SELECT 
	  CASE WHEN (SNMR.F_PointsNumDes1 IS NOT NULL 
			 AND SNMR.F_PointsNumDes2 IS NOT NULL
			 AND SNMR.F_PointsNumDes3 IS NOT NULL
			 AND CJMR.F_PointsNumDes1 IS NOT NULL
			 AND CJMR.F_PointsNumDes2 IS NOT NULL
			 AND CJMR.F_PointsNumDes3 IS NOT NULL) OR ER.F_IRMID IS NOT NULL THEN ISNULL(CAST(ER.F_EventRank AS NVARCHAR),N'-')
			 ELSE CAST(ER.F_EventRank AS NVARCHAR) END
 	--,ROW_NUMBER() OVER(ORDER BY ISNULL(SNMR.F_CompetitionPosition,999))
 	--, ROW_NUMBER() OVER(ORDER BY ISNULL(I.F_Seed,999)) \
	...
	, PDE.F_PhaseLongName + N'  (' +  PDC.F_PhaseLongName +')'
	, PDC.F_PhaseLongName
	...
	, CASE WHEN ER.F_IRMID IS NOT NULL THEN IRM.F_IRMCODE ELSE ER.F_EventPointsCharDes4 END AS TOTAL
	
	,ROW_NUMBER() OVER(ORDER BY  ISNULL(I.F_InscriptionNum,999), ISNULL(I.F_Seed,999))
	...
	, CASE WHEN SNMR.F_MatchID =@MatchID AND (SNMR.F_RealScore=1 OR SNMR.F_RealScore=10) AND SNMR.F_Points =1 THEN 1 ELSE NULL END
	, CASE WHEN SNMR.F_MatchID =@MatchID AND (SNMR.F_RealScore=1 OR SNMR.F_RealScore=10) AND SNMR.F_Points =2 THEN 1 ELSE NULL END
	, CASE WHEN SNMR.F_MatchID =@MatchID AND (SNMR.F_RealScore=1 OR SNMR.F_RealScore=10) AND SNMR.F_Points =3 THEN 1 ELSE NULL END
	, CASE WHEN CJMR.F_MatchID =@MatchID AND (CJMR.F_RealScore=1 OR CJMR.F_RealScore=10) AND CJMR.F_Points =1 THEN 1 ELSE NULL END
	, CASE WHEN CJMR.F_MatchID =@MatchID AND (CJMR.F_RealScore=1 OR CJMR.F_RealScore=10) AND CJMR.F_Points =2 THEN 1 ELSE NULL END
	, CASE WHEN CJMR.F_MatchID =@MatchID AND (CJMR.F_RealScore=1 OR CJMR.F_RealScore=10) AND CJMR.F_Points =3 THEN 1 ELSE NULL END
	...
	-- 在排名表中插入基本信息
	INSERT #RankList 
	([RankID],[RName_ENG],[RName_CHN],RFlag, RNOC,RDateOfBirth,RBodyWeight,[RankResult],[EventRank],
	 RLotNo,RSnatch1stAttempt,RSnatch1stRes,RSnatch2ndAttempt,RSnatch2ndRes,RSnatch3rdAttempt,RSnatch3rdRes,RSnatchResult,
	 RCleanJerk1stAttempt,RCleanJerk1stRes,RCleanJerk2ndAttempt,RCleanJerk2ndRes,RCleanJerk3rdAttempt,RCleanJerk3rdRes,RCleanJerkResult,
	 RBest1,RBest2,RBest3,RBest4,RBest5,RBest6,[ResultID]) 
	(SELECT TOP 3 
	 I.F_InscriptionNum
	,RDE.F_SBLongName
	,RDC.F_SBLongName
	...
	, ER.F_EventRank
	, I.F_Seed
	, CASE WHEN SNMR.F_PointsCharDes1 IS NULL THEN '---'
	       WHEN SNMR.F_PointsCharDes1 IS NOT NULL AND dbo.Fun_WL_GetIsSuccess(SNMR.F_PointsNumDes1)=3 THEN '---'
	       ELSE SNMR.F_PointsCharDes1 END
	, CASE WHEN dbo.Fun_WL_GetIsSuccess(SNMR.F_PointsNumDes1)=3 THEN 1 ELSE dbo.Fun_WL_GetIsSuccess(SNMR.F_PointsNumDes1) END
	, CASE WHEN SNMR.F_PointsCharDes2 IS NULL THEN '---'
	       WHEN SNMR.F_PointsCharDes2 IS NOT NULL AND dbo.Fun_WL_GetIsSuccess(SNMR.F_PointsNumDes2)=3 THEN '---'
	       ELSE SNMR.F_PointsCharDes2 END
	, CASE WHEN dbo.Fun_WL_GetIsSuccess(SNMR.F_PointsNumDes2)=3 THEN 1 ELSE dbo.Fun_WL_GetIsSuccess(SNMR.F_PointsNumDes2) END
	, CASE WHEN SNMR.F_PointsCharDes3 IS NULL THEN '---'
	       WHEN SNMR.F_PointsCharDes3 IS NOT NULL AND dbo.Fun_WL_GetIsSuccess(SNMR.F_PointsNumDes3)=3 THEN '---'
	       ELSE SNMR.F_PointsCharDes3 END
	, CASE WHEN dbo.Fun_WL_GetIsSuccess(SNMR.F_PointsNumDes3)=3 THEN 1 ELSE dbo.Fun_WL_GetIsSuccess(SNMR.F_PointsNumDes3) END
	, CASE WHEN SNMR.F_PointsCharDes4 IS NULL THEN '---'
	       ELSE SNMR.F_PointsCharDes4 END 
	
	, CASE WHEN CJMR.F_PointsCharDes1 IS NULL THEN '---'
	       WHEN CJMR.F_PointsCharDes1 IS NOT NULL AND dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes1)=3 THEN '---'
	       ELSE CJMR.F_PointsCharDes1 END
	, CASE WHEN dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes1)=3 THEN 1 ELSE dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes1) END
	, CASE WHEN CJMR.F_PointsCharDes2 IS NULL THEN '---'
	       WHEN CJMR.F_PointsCharDes2 IS NOT NULL AND dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes2)=3 THEN '---'
	       ELSE CJMR.F_PointsCharDes2 END
	, CASE WHEN dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes2)=3 THEN 1 ELSE dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes2) END
	, CASE WHEN CJMR.F_PointsCharDes3 IS NULL THEN '---'
	       WHEN CJMR.F_PointsCharDes3 IS NOT NULL AND dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes3)=3 THEN '---'
	       ELSE CJMR.F_PointsCharDes3 END
	, CASE WHEN dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes3)=3 THEN 1 ELSE dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes3) END
	, CASE WHEN CJMR.F_PointsCharDes4 IS NULL THEN '---'
	       ELSE CJMR.F_PointsCharDes4 END
	...
	, case WHEN CJMR.F_PointsCharDes3 IS NOT NULL and CJMR.F_PointsNumDes3  IS NOT NULL 
			AND dbo.Fun_WL_GetIsSuccess(CJMR.F_PointsNumDes3) =1 AND CJMR.F_PointsCharDes3 = CJMR.F_PointsCharDes4 THEN 1
			ELSE NULL END AS Best6
	, ROW_NUMBER() OVER(ORDER BY ISNULL(ER.F_EventRank,999))
	FROM TS_Event_Result AS ER
	LEFT JOIN TS_Phase_Result AS PR ON PR.F_RegisterID = ER.F_RegisterID AND PR.F_PhaseID IN
		(SELECT F_PhaseID FROM TS_Phase AS P WHERE P.F_EventID = ER.F_EventID)
	LEFT JOIN TR_Register AS R ON ER.F_RegisterID = R.F_RegisterID  
	LEFT JOIN TR_Register_Des AS RDE ON R.F_RegisterID = RDE.F_RegisterID AND RDE.F_LanguageCode = 'ENG' 
	LEFT JOIN TR_Register_Des AS RDC ON R.F_RegisterID = RDC.F_RegisterID AND RDC.F_LanguageCode = 'CHN' 
	LEFT JOIN TR_Inscription AS I ON ER.F_RegisterID = I.F_RegisterID AND ER.F_EventID = I.F_EventID 
	LEFT JOIN ( SELECT A.* FROM TS_Match_Result AS A LEFT JOIN TS_Match AS B ON A.F_MatchID = B.F_MatchID WHERE B.F_MatchCode = '01' ) AS SNMR ON ER.F_RegisterID = SNMR.F_RegisterID 
	LEFT JOIN ( SELECT A.* FROM TS_Match_Result AS A LEFT JOIN TS_Match AS B ON A.F_MatchID = B.F_MatchID WHERE B.F_MatchCode = '02' ) AS CJMR ON ER.F_RegisterID = CJMR.F_RegisterID 	
	WHERE ER.F_EventID=@EventID AND PR.F_PhaseID != @PhaseID AND ER.F_EventPointsCharDes4 != N'') 
    
	SELECT * FROM #ResultList AS R LEFT JOIN #Records AS T ON T.ResultID = R.ResultID
	LEFT JOIN #RankList AS RA ON RA.ResultID = R.ResultID 
	ORDER by R.ResultID 
...
/*
exec Proc_SCB_WL_GetPhaseResultList 42
*/
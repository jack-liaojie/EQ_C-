IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_TVG_WL_GetMatchOfficial]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_TVG_WL_GetMatchOfficial]
GO
...
--名    称：[Proc_TVG_WL_GetMatchOfficial]
...
CREATE PROCEDURE [dbo].[Proc_TVG_WL_GetMatchOfficial](
			@MatchID		    INT
)
As
Begin
SET NOCOUNT ON 
	...
	FROM #Tmp_Table 
	WHERE F_FunctionCode NOT IN ('RRE','R1','R2','R3','RE','RES') AND F_FunctionCode IS NOT NULL

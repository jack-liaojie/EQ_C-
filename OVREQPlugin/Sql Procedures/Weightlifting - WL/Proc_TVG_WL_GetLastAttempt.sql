IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_TVG_WL_GetLastAttempt]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_TVG_WL_GetLastAttempt]
GO
...
----存储过程名称：[Proc_TVG_WL_GetLastAttempt]
----功		  能：SCB得到当前运动员信息
----作		  者：崔凯
----日		  期: 2011-02-28 

CREATE PROCEDURE [dbo].[Proc_TVG_WL_GetLastAttempt]
             @MatchID         INT,
             @RegisterID      INT
...
SET NOCOUNT ON
	...
	RDC.F_TvShortName AS LongName_CHN,
	...
	CASE WHEN ER.F_IRMID IS NOT NULL AND (IRM.F_IRMCode ='DNS' OR IRM.F_IRMCode ='DSQ') THEN '---'
	     ...
	     WHEN MR.F_PointsNumDes1 IS NULL THEN ''
	...
	CASE WHEN ER.F_IRMID IS NOT NULL AND (IRM.F_IRMCode ='DNS' OR IRM.F_IRMCode ='DSQ') THEN '---'
	     ...
	     WHEN MR.F_PointsNumDes2 IS NULL THEN ''
	...
	CASE WHEN ER.F_IRMID IS NOT NULL AND (IRM.F_IRMCode ='DNS' OR IRM.F_IRMCode ='DSQ') THEN NULL
	     WHEN ER.F_IRMID IS NOT NULL AND IRM.F_IRMCode ='DNF' AND MR.F_PointsNumDes2 IS NULL  AND dbo.Fun_WL_GetIsSuccess(MR.F_PointsNumDes2)<>1 THEN NULL
	...
	CASE WHEN ER.F_IRMID IS NOT NULL AND (IRM.F_IRMCode ='DNS' OR IRM.F_IRMCode ='DSQ') THEN '---'
	     ...
	     WHEN MR.F_PointsNumDes3 IS NULL THEN ''
	...
	CASE WHEN ER.F_IRMID IS NOT NULL AND (IRM.F_IRMCode ='DNS' OR IRM.F_IRMCode ='DSQ') THEN NULL
	     WHEN ER.F_IRMID IS NOT NULL AND IRM.F_IRMCode ='DNF' AND MR.F_PointsNumDes3 IS NULL AND dbo.Fun_WL_GetIsSuccess(MR.F_PointsNumDes3)<>1 THEN NULL
	...
	ORDER BY (CASE WHEN MR.F_RealScore = 0 OR MR.F_RealScore = 10 OR MR.F_RealScore = 20 THEN 0 ELSE ISNULL(MR.F_RealScore,3) END), ISNULL(MR.F_CompetitionPositionDes2,999)

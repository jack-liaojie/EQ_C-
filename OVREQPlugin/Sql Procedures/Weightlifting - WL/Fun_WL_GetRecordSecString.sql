IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Fun_WL_GetRecordSecString]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [dbo].[Fun_WL_GetRecordSecString]
GO
...
CREATE FUNCTION [dbo].[Fun_WL_GetRecordSecString]
								(
									@MatchID					INT,
...
BEGIN
	...
	DECLARE One_Cursor CURSOR FOR 
			SELECT CASE WHEN ER.F_SubEventCode =1 THEN '[S]'+RT.F_RecordTypeCode 
				WHEN ER.F_SubEventCode =2 THEN '[CJ]'+RT.F_RecordTypeCode 
				ELSE '[T]'+RT.F_RecordTypeCode END AS RecordType
				...
				AND RR.F_RegisterID = @CompetitionPosition AND F_Active = 1
	ELSE				
	DECLARE One_Cursor CURSOR FOR 

IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_WL_GetEventRecordList]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_WL_GetEventRecordList]
GO
...
--名    称: [Proc_Report_WL_GetEventRecordList]
...
CREATE PROCEDURE [dbo].[Proc_Report_WL_GetEventRecordList]
	@MatchID				INT,
	@LanguageCode			CHAR(3)
AS
BEGIN
SET NOCOUNT ON
    ...
    --WHERE RR.F_MatchID IN (SELECT F_MatchID FROM TS_Match WHERE F_PhaseID = @PhaseID)
	WHERE ER.F_EventID = @EventID AND RR.F_Equalled = 0
	ORDER BY [SubEventCode],[F_RecordID]

SET NOCOUNT OFF
END
/*
EXEC [Proc_Report_WL_GetEventRecordList] 57, 'ENG'
EXEC [Proc_Report_WL_GetEventRecordList] 7, 'CHN'
*/
GO

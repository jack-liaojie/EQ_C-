IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_TVG_WL_GetSchedule]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_TVG_WL_GetSchedule]
GO
...
CREATE PROCEDURE [dbo].[Proc_TVG_WL_GetSchedule](
...
SET NOCOUNT ON
	...
	SELECT dbo.[Func_SCB_GetDateTime](S.F_SessionDate, 1, 'ENG') AS [Date_ENG]
		...
		, ED1.F_EventShortName + ' ' + PD1.F_PhaseLongName AS [EventName]
...
END
/*
exec Proc_TVG_WL_GetSchedule 5
*/
GO

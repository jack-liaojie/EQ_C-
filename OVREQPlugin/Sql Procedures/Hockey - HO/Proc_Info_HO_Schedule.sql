IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Info_HO_Schedule]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Info_HO_Schedule]
GO
...
CREATE PROCEDURE [dbo].[Proc_Info_HO_Schedule]
		@DateID			INT
AS
BEGIN
...
SET NOCOUNT ON
    ...
    INSERT INTO #Temp_Reports(Code, [FileName], [Type])
		SELECT Code, Code + N'.C51.CHI.1.0', N'C51' FROM #Temp_Schedule
		
	INSERT INTO #Temp_Reports(Code, [FileName], [Type])
		SELECT Code, Code + N'.C73.CHI.1.0', N'C73' FROM #Temp_Schedule 
	...
	INSERT INTO #Temp_Reports(Code, [FileName], [Type])
		SELECT D.F_DisciplineCode + Gender.F_GenderCode + E.F_EventCode + N'000' AS [Code]
			   , D.F_DisciplineCode + Gender.F_GenderCode + E.F_EventCode + N'000' + N'.C92.CHI.1.0' AS [FileName]
			   , N'C92'
			   FROM TS_Event AS E LEFT JOIN TS_Event_Des AS ES ON E.F_EventID = ES.F_EventID AND ES.F_LanguageCode = @LanguageCode
					LEFT JOIN TS_Discipline AS D ON E.F_DisciplineID = D.F_DisciplineID 
					LEFT JOIN TS_Discipline_Des AS DS ON D.F_DisciplineID = DS.F_DisciplineID AND DS.F_LanguageCode = @LanguageCode

...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Proc_Report_HO_GetRSCCode]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Proc_Report_HO_GetRSCCode]
GO
...
CREATE PROCEDURE [dbo].[Proc_Report_HO_GetRSCCode]
	@DisciplineID INT,
	@EventID	  INT,
	@PhaseID	  INT,
	@MatchId	  INT,
...
SET NOCOUNT ON
	...
	UPDATE #table_tmp SET DISCIPLINE = @tmp_DisciplineCode, SEX = @tmp_SexCode, 
				...
				PHASE = @tmp_PhaseCode, EVENT_UNIT = right('00000' + @tmp_UnitCode, 5)
	...
	UPDATE #table_tmp SET POOL = (CASE C.F_PhaseComment WHEN 'A' THEN '01' WHEN 'B' THEN '02' WHEN 'C' THEN '03' WHEN 'D' THEN '04' WHEN 'E' THEN '05' WHEN 'F' THEN '06' ELSE '00' END) FROM  #table_tmp AS A LEFT JOIN TS_Phase AS B ON A.PHASE_ID = B.F_PhaseID
	LEFT JOIN TS_Phase_Des AS C ON B.F_PhaseID = C.F_PhaseID WHERE A.IS_POOL = 1 AND C.F_LanguageCode = 'ENG'
	...
	UPDATE #table_tmp SET [KEY] = DISCIPLINE + SEX + [EVENT] + EVENT_PARENT + PHASE + POOL + EVENT_UNIT + '.' + @MatchDate + @DelegationCode

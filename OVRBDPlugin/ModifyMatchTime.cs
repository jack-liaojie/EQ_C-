...
namespace AutoSports.OVRBDPlugin
{
    public partial class ModifyMatchTime : Office2007Form
    {
        ...
        GameCountType m_gameCntType;
        public ModifyMatchTime(Int32 nMatchID, Int32 nMatchType, GameCountType type)
        {
            InitializeComponent();
            m_gameCntType = type;
        ...
        private void ModifyMatchTime_Load(object sender, EventArgs e)
        {
            if (m_iMatchType == 3)
            {
                ...
                m_iMatchSplitID = -1;
                BDCommon.g_ManageDB.GetRegusList(m_iMatchID, cmb_Time_Regus);
            }
            else
            {
                m_iMatchSplitID = 0;
        ...
        {
            BDCommon.g_ManageDB.GetMatchTime(m_iMatchID, m_iMatchSplitID, dgvMatchTime, m_gameCntType);
        ...
        private void SetGridStyle(DataGridView dgv)
        {
            ...
            if (dgv == dgvMatchTime)
            {
                ...
                if (dgvMatchTime.Columns["Game6"] != null)
                {
                    dgvMatchTime.Columns["Game6"].ReadOnly = false;
                }
                if (dgvMatchTime.Columns["Game7"] != null)
                {
                    dgvMatchTime.Columns["Game7"].ReadOnly = false;
                }
        ...
        private void dgvMatchTime_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (CurCell != null)
            {
                ...
                if (strColumnName.CompareTo("Game6") == 0)
                {
                    iSplitID = GetFieldValue(dgvMatchTime, iRowIndex, "F_Game6ID");
                }
                if (strColumnName.CompareTo("Game7") == 0)
                {
                    iSplitID = GetFieldValue(dgvMatchTime, iRowIndex, "F_Game7ID");
                }

                try
                {
                    DateTime dtTemp = Convert.ToDateTime(strInputValue);
                    BDCommon.g_ManageDB.UpdateMatchTime(m_iMatchID, iSplitID, strInputValue);
                }
                catch
                {
                }

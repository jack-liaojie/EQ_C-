...
namespace AutoSports.OVRFBPlugin
{
    public partial class frmOVRFBTeamMemberEntry : Office2007Form
    {
        ...
        private int m_iHomeGKUniformID;
        private int m_iVisitGKUniformID;
        ...
        private DataTable dt_Uniform_GKhome = new DataTable();
        private DataTable dt_Uniform_GKVisit = new DataTable();
        ...
        private void dgvMember_Home_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //}
            else if (dgvMember_Home.Columns[iColIdx].Name.CompareTo("Status") == 0)
            {
                UpdateMemberStatus(1, ref dgvMember_Home, e);
            }
        ...
        private void dgvMember_Home_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            if (dgvMember_Home.Columns[e.ColumnIndex] is DGVCustomComboBoxColumn || e.RowIndex >= 0)
            {
                ...
                else if (dgvMember_Home.Columns[iColumnIndex].Name.CompareTo("Status") == 0)
                {
        ...
        private void dgvMember_Visit_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //}
            else if (dgvMember_Visit.Columns[iColIdx].Name.CompareTo("Status") == 0)
            {
                UpdateMemberStatus(2, ref dgvMember_Visit, e);
            }
        ...
        private void dgvMember_Visit_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            if (dgvMember_Visit.Columns[e.ColumnIndex] is DGVCustomComboBoxColumn || e.RowIndex >= 0)
            {
                ...
                else if (dgvMember_Visit.Columns[iColumnIndex].Name.CompareTo("Status") == 0)
                {
        ...
        private void cmbUniform_HomeGK_SelectionChangeCommitted(object sender, EventArgs e)
        {
            m_iHomeGKUniformID = GVAR.Str2Int(cmbUniform_HomeGK.SelectedValue);
            GVAR.g_ManageDB.UpdateMatchGKUniform(m_iMatchID, 1, m_iHomeGKUniformID);
        }
        ...
        private void InitUniformCmb()
        {
            ...
            GVAR.g_ManageDB.GetMatchGKUniform(m_iMatchID, 1, ref m_iHomeGKUniformID);
            GVAR.g_ManageDB.GetMatchGKUniform(m_iMatchID, 2, ref m_iVisitGKUniformID);

            GVAR.g_ManageDB.GetGKUniform(ref dt_Uniform_GKhome);
            GVAR.g_ManageDB.GetGKUniform(ref dt_Uniform_GKVisit);

            dt_Uniform_GKhome.Rows.Add(-1, "");
            dt_Uniform_GKVisit.Rows.Add(-1, "");

            cmbUniform_HomeGK.DisplayMember = "UniformName";
            cmbUniform_HomeGK.ValueMember = "UniformID";
            cmbUniform_HomeGK.DataSource = dt_Uniform_GKhome;

            cmbUniform_VisitGK.DisplayMember = "UniformName";
            cmbUniform_VisitGK.ValueMember = "UniformID";
            cmbUniform_VisitGK.DataSource = dt_Uniform_GKVisit;

            if (m_iHomeGKUniformID == 0)
            {
                cmbUniform_HomeGK.SelectedValue = -1;
            }
            else
            {
                cmbUniform_HomeGK.SelectedValue = m_iHomeGKUniformID;
            }

            if (m_iVisitGKUniformID == 0)
            {
                cmbUniform_VisitGK.SelectedValue = -1;
            }
            else
            {
                cmbUniform_VisitGK.SelectedValue = m_iVisitGKUniformID;
            }

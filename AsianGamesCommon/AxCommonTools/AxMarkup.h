// Markup.h: interface for the CAxMarkup class.
...
class AX_OVRCOMMON_EXP CAxMarkup
{
public:
	CAxMarkup() { x_InitMarkup(); SetDoc( NULL ); };
	CAxMarkup( MCD_CSTR szDoc ) { x_InitMarkup(); SetDoc( szDoc ); };
	CAxMarkup( int nFlags ) { x_InitMarkup(); SetDoc( NULL ); m_nDocFlags = nFlags; };
	CAxMarkup( const CAxMarkup& markup ) { x_InitMarkup(); *this = markup; };
	void operator=( const CAxMarkup& markup );
	~CAxMarkup();

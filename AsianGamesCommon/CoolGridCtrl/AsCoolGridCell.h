...
class AX_OVRCOMMON_EXP CAsCoolGridCell : public CAsCoolGridCellBase
{
    friend class CAsCoolGridCtrl;
    DECLARE_DYNCREATE(CAsCoolGridCell)

// Construction/Destruction
public:
    CAsCoolGridCell();
    virtual ~CAsCoolGridCell();

// Attributes
public:
    void operator=(const CAsCoolGridCell& cell);
    ...
    virtual void  SetGrid(CAsCoolGridCtrl* pGrid){ m_pCoolGrid = pGrid;     }                          
    ...
    virtual CAsCoolGridCtrl* GetGrid() const        { return m_pCoolGrid; }
...
protected:
    ...
    CAsCoolGridCtrl* m_pCoolGrid;   // Parent grid control
    CWnd*      m_pEditWnd;
};

// This class is for storing grid default values. It's a little heavy weight, so
// don't use it in bulk 
class AX_OVRCOMMON_EXP CAsCoolGridDefaultCell : public CAsCoolGridCell
{
    DECLARE_DYNCREATE(CAsCoolGridDefaultCell)

// Construction/Destruction
public:
    CAsCoolGridDefaultCell();
    virtual ~CAsCoolGridDefaultCell();
...
public:
	...
	virtual void     SetState(DWORD nState)             { m_nState = (nState & GVIS_READONLY) ? (m_nState|GVIS_READONLY) : (m_nState&~GVIS_READONLY); }  // GUAN : Validate the ReadOnly property

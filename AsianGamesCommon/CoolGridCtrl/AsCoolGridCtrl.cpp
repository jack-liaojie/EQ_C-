// AsCoolGridCtrl.cpp : implementation file
// MFC Grid Control v2.26

#include "stdafx.h"
#include "AsCoolGridMemDC.h"
...
IMPLEMENT_DYNCREATE(CAsCoolGridCtrl, CWnd)
...
UINT GetMouseScrollLines()
{
	...
	if (!GetVersionEx(&VersionInfo) || (VersionInfo.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS && VersionInfo.dwMinorVersion == 0))
	{
		HKEY hKey;
...
/////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl
CAsCoolGridCtrl::CAsCoolGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	RegisterWindowClass();
	...
	m_pRtcDefault		  = RUNTIME_CLASS(CAsCoolGridCell);
...
CAsCoolGridCtrl::~CAsCoolGridCtrl()
{
	DeleteAllItems();
...
BOOL CAsCoolGridCtrl::RegisterWindowClass()
{
	WNDCLASS wndcls;
	//HINSTANCE hInst = AfxGetInstanceHandle();
	//HINSTANCE hInst = AfxGetResourceHandle(); // When the Grid is Global variable, this function can cause ASSERT, because the AfxWinInit() has not been called. ( Guan changed )
...
BOOL CAsCoolGridCtrl::Initialise()
{
...
BOOL CAsCoolGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
...
void CAsCoolGridCtrl::SetupDefaultCells()
{
...
void CAsCoolGridCtrl::PreSubclassWindow()
{
...
LRESULT CAsCoolGridCtrl::SendMessageToParent(int nRow, int nCol, int nMessage) const
{
	if (!IsWindow(m_hWnd))
		return 0;
...
LRESULT CAsCoolGridCtrl::SendDisplayRequestToParent(GV_DISPINFO* pDisplayInfo) const
{
	if (!IsWindow(m_hWnd))
		return 0L;
...
LRESULT CAsCoolGridCtrl::SendCacheHintToParent(const CAsCoolGridCellRange& range) const
{
	if (!IsWindow(m_hWnd))
		return 0;
...
BEGIN_MESSAGE_MAP(CAsCoolGridCtrl, CWnd)
	//EFW - Added ON_WM_RBUTTONUP
	//{{AFX_MSG_MAP(CAsCoolGridCtrl)
	ON_WM_PAINT()
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
	ON_WM_SIZE()
...
/////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl message handlers

void CAsCoolGridCtrl::OnPaint()
{
	CPaintDC dc(this);      // device context for painting

	if (m_bDoubleBuffer)    // Use a memory DC to remove flicker
	{
		CAsCoolGridMemDC MemDC(&dc);
		OnDraw(&MemDC);
	}
...
BOOL CAsCoolGridCtrl::OnEraseBkgnd(CDC* /*pDC*/)
{
...
void CAsCoolGridCtrl::EraseBkgnd(CDC* pDC)
{
...
void CAsCoolGridCtrl::OnSize(UINT nType, int cx, int cy)
{  
...
UINT CAsCoolGridCtrl::OnGetDlgCode()
{
...
void CAsCoolGridCtrl::OnSysColorChange()
{
...
void CAsCoolGridCtrl::OnCaptureChanged(CWnd *pWnd)
{
...
void CAsCoolGridCtrl::OnSettingChange(UINT uFlags, LPCTSTR lpszSection)
{
...
void CAsCoolGridCtrl::OnTimer(UINT nIDEvent)
{
...
void CAsCoolGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	CAsCoolGridCellID next = m_idCurrentCell;
	...
	else if (nChar == VK_DOWN)
	{
		...
		iOrig = next.m_nRow;
		next.m_nRow++;
		while( next.m_nRow < GetRowCount())
		{
			if( GetRowHeight( next.m_nRow) > 0)
			{
				bFoundVisible = TRUE;
				break;
			}
	...
	else if (nChar == VK_UP)
	{
		...
		iOrig = next.m_nRow;
		next.m_nRow--;
		while( next.m_nRow >= m_nFixedRows)
		{
			if( GetRowHeight( next.m_nRow) > 0)
			{
				bFoundVisible = TRUE;
	...
	else if (nChar == VK_RIGHT || (nChar == VK_TAB && !IsSHIFTpressed()) )
	{
		...
		if (nChar == VK_TAB)
		{
			...
			if (next.m_nCol == (GetColumnCount()) && next.m_nRow < (GetRowCount() - 1))
			{
				next.m_nRow++;
				while( next.m_nRow < GetRowCount())
				{
		...
		while( next.m_nCol < GetColumnCount())
		{
	...
	else if (nChar == VK_LEFT || (nChar == VK_TAB && IsSHIFTpressed()) )
	{
		...
		if (nChar == VK_TAB)
		{
			if (next.m_nCol == (GetFixedColumnCount()-1) && next.m_nRow > GetFixedRowCount())
			{
				next.m_nRow--;
				while( next.m_nRow > GetFixedRowCount())
				{
					if( GetRowHeight(next.m_nRow) > 0)
					{
						bFoundVisible = TRUE;
	...
	{
		CAsCoolGridCellID idOldTopLeft = GetTopleftNonFixedCell();
		SendMessage(WM_VSCROLL, SB_PAGEDOWN, 0);
		bVertScrollAction = TRUE;
		CAsCoolGridCellID idNewTopLeft = GetTopleftNonFixedCell();

		int increment = idNewTopLeft.m_nRow - idOldTopLeft.m_nRow;
		if (increment)
		{
			next.m_nRow += increment;
			if (next.m_nRow >(GetRowCount() - 1))
				next.m_nRow = GetRowCount() - 1;
		}
		else
			next.m_nRow = GetRowCount() - 1;
	}
	else if (nChar == VK_PRIOR)
	{
		...
		{
			next.m_nRow += increment;
			if (next.m_nRow < m_nFixedRows) 
				next.m_nRow = m_nFixedRows;
		}
		else
			next.m_nRow = m_nFixedRows;
	}
	else if (nChar == VK_HOME)
	{
		...
		while( next.m_nCol < GetColumnCount() - 1)
		{
			if( GetColumnWidth( next.m_nCol) > 0)
				break;
			next.m_nCol++;
		}
		while( next.m_nRow < GetRowCount() - 1)
		{
			if( GetRowHeight( next.m_nRow) > 0)
				break;
			next.m_nRow++;
	...
	else if (nChar == VK_END)
	{
		...
		while( next.m_nRow > m_nFixedRows + 1)
		{
			if( GetRowHeight( next.m_nRow) > 0)
				break;
			next.m_nRow--;
	...
	{
		OnEditCell(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol, CPoint( -1, -1), VK_LBUTTON);
	}
	else
	{
		CWnd::OnKeyDown(nChar, nRepCnt, nFlags);
	...
	if (next != m_idCurrentCell)
	{
		...
		if (m_MouseMode == MOUSE_NOTHING)
		{
			...
			SendMessageToParent(next.m_nRow, next.m_nCol, GVN_SELCHANGING);
			OnSelecting(next);
			SendMessageToParent(next.m_nRow, next.m_nCol, GVN_SELCHANGED);
...
void CAsCoolGridCtrl::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	CWnd::OnKeyUp(nChar, nRepCnt, nFlags);
}

void CAsCoolGridCtrl::OnSysKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CAsCoolGridCtrl::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	// EFW - BUG FIX
	if (!IsCTRLpressed() && m_MouseMode == MOUSE_NOTHING && nChar != VK_ESCAPE)
	{
		if (!m_bHandleTabKey || (m_bHandleTabKey && nChar != VK_TAB))
			OnEditCell(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol, CPoint( -1, -1), nChar);
	}
...
// Added by KiteFly
LRESULT CAsCoolGridCtrl::OnImeChar(WPARAM wCharCode, LPARAM)
{
	// EFW - BUG FIX
	if (!IsCTRLpressed() && m_MouseMode == MOUSE_NOTHING && wCharCode != VK_ESCAPE) 
		OnEditCell(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol, CPoint( -1, -1), (UINT)wCharCode);
	return 0;
}
...
void CAsCoolGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	case VK_END:
		...
		OnEditCell(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol, CPoint( -1, -1), (UINT)pgvItem->lParam);
	}
...
void CAsCoolGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	EndEditing();
	...
	CAsCoolGridCellID idTopLeft = GetTopleftNonFixedCell();
	...
	case SB_LINERIGHT:
		if (scrollPos < m_nHScrollMax)
		{
			...
			while (idTopLeft.m_nCol < (GetColumnCount()-1)
				&& GetColumnWidth( idTopLeft.m_nCol) < 1 )
			{
				idTopLeft.m_nCol++;
			}
			int xScroll = GetColumnWidth(idTopLeft.m_nCol);
	...
	case SB_LINELEFT:
		if (scrollPos > 0 && idTopLeft.m_nCol > GetFixedColumnCount())
		{
			int iColToUse = idTopLeft.m_nCol-1;
			// may have contiguous hidden columns.  Blow by them
...
void CAsCoolGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	EndEditing();
	...
	case SB_LINEDOWN:
		if (scrollPos < m_nVScrollMax)
		{
			...
			while(  idTopLeft.m_nRow < (GetRowCount()-1)
				&& GetRowHeight( idTopLeft.m_nRow) < 1 )
			{
				idTopLeft.m_nRow++;
			}

			int yScroll = GetRowHeight(idTopLeft.m_nRow);
	...
	case SB_LINEUP:
		if (scrollPos > 0 && idTopLeft.m_nRow > GetFixedRowCount())
		{
			int iRowToUse = idTopLeft.m_nRow-1;
			// may have contiguous hidden rows.  Blow by them
...
/////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl implementation functions

void CAsCoolGridCtrl::OnDraw(CDC* pDC)
{
	if (!m_bAllowDraw)
		return;
	...
	CRect rect;
	int row, col;
	CAsCoolGridCellBase* pCell;

	int nFixedRowHeight = GetFixedRowHeight();
	int nFixedColWidth  = GetFixedColumnWidth();

	CAsCoolGridCellID idTopLeft = GetTopleftNonFixedCell();
	int minVisibleRow = idTopLeft.m_nRow,
		minVisibleCol = idTopLeft.m_nCol;

	CRect VisRect;
	CAsCoolGridCellRange VisCellRange = GetVisibleNonFixedCellRange(VisRect);
	...
	// draw top-left cells 0..m_nFixedRows-1, 0..m_nFixedCols-11
	rect.bottom = -1;
	...
	if (GetVirtualMode())
		SendCacheHintToParent(CAsCoolGridCellRange(-1,-1,-1,-1));
...
////////////////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl Cell selection stuff

// Is a given cell designation valid (ie within the bounds of our number
// of columns/rows)?
BOOL CAsCoolGridCtrl::IsValid(int nRow, int nCol) const
{
	return (nRow >= 0 && nRow < m_nRows && nCol >= 0 && nCol < m_nCols);
}

BOOL CAsCoolGridCtrl::IsValid(const CAsCoolGridCellID& cell) const
{
	return IsValid(cell.m_nRow, cell.m_nCol);
}

// Is a given cell range valid (ie within the bounds of our number
// of columns/rows)?
BOOL CAsCoolGridCtrl::IsValid(const CAsCoolGridCellRange& range) const
{
...
void CAsCoolGridCtrl::SetRedraw(BOOL bAllowDraw, BOOL bResetScrollBars /* = FALSE */)
{
...
// or the supplied dc)
BOOL CAsCoolGridCtrl::RedrawCell(const CAsCoolGridCellID& cell, CDC* pDC /* = NULL */)
{
	return RedrawCell(cell.m_nRow, cell.m_nCol, pDC);
}

BOOL CAsCoolGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = NULL */)
{
	BOOL bResult = TRUE;
	...
	if (pDC)
	{
		...
		{
			CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
			if (pCell)
...
BOOL CAsCoolGridCtrl::RedrawRow(int row)
{
	BOOL bResult = TRUE;
...
BOOL CAsCoolGridCtrl::RedrawColumn(int col)
{
	BOOL bResult = TRUE;
...
CAsCoolGridCellID CAsCoolGridCtrl::SetFocusCell(int nRow, int nCol)
{
	return SetFocusCell(CAsCoolGridCellID(nRow, nCol));
}

CAsCoolGridCellID CAsCoolGridCtrl::SetFocusCell(CAsCoolGridCellID cell)
{
	if (cell == m_idCurrentCell)
		return m_idCurrentCell;

	CAsCoolGridCellID idPrev = m_idCurrentCell;

	// EFW - Bug Fix - Force focus to be in a non-fixed cell
	if (cell.m_nRow != -1 && cell.m_nRow < GetFixedRowCount())
		cell.m_nRow = GetFixedRowCount();
	if (cell.m_nCol != -1 && cell.m_nCol < GetFixedColumnCount())
		cell.m_nCol = GetFixedColumnCount();

	m_idCurrentCell = cell;

	if (IsValid(idPrev))
	{
		SetItemState(idPrev.m_nRow, idPrev.m_nCol,
			GetItemState(idPrev.m_nRow, idPrev.m_nCol) & ~GVIS_FOCUSED);
		RedrawCell(idPrev); // comment to reduce flicker

		if (GetTrackFocusCell() && idPrev.m_nCol != m_idCurrentCell.m_nCol)
			for (int row = 0; row < m_nFixedRows; row++)
				RedrawCell(row, idPrev.m_nCol);
		if (GetTrackFocusCell() && idPrev.m_nRow != m_idCurrentCell.m_nRow)
			for (int col = 0; col < m_nFixedCols; col++)
				RedrawCell(idPrev.m_nRow, col);
	}

	if (IsValid(m_idCurrentCell))
	{
		SetItemState(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol,
			GetItemState(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol) | GVIS_FOCUSED);

		RedrawCell(m_idCurrentCell); // comment to reduce flicker

		if (GetTrackFocusCell() && idPrev.m_nCol != m_idCurrentCell.m_nCol)
			for (int row = 0; row < m_nFixedRows; row++)
				RedrawCell(row, m_idCurrentCell.m_nCol);
		if (GetTrackFocusCell() && idPrev.m_nRow != m_idCurrentCell.m_nRow)
			for (int col = 0; col < m_nFixedCols; col++)
				RedrawCell(m_idCurrentCell.m_nRow, col);      
	}
...
void CAsCoolGridCtrl::SetSelectedRange(const CAsCoolGridCellRange& Range,
...
void CAsCoolGridCtrl::SetSelectedRange(int nMinRow, int nMinCol, int nMaxRow, int nMaxCol,
									   BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
	if (!m_bEnableSelection)
		return;

	if (!AfxGetApp()) return; // Sometimes this can fail 
	...
	CAsCoolGridCellRange VisCellRange;
	if (IsWindow(GetSafeHwnd()))
	...
	if (bSelectCells)
	{
		...
		{
			DWORD key;
			CAsCoolGridCellID cell;
			m_SelectedCellMap.GetNextAssoc(pos, key, (CAsCoolGridCellID&)cell);

			// Reset the selection flag on the cell
			if (IsValid(cell))
			{
				// This will remove the cell from the m_SelectedCellMap map
				SetItemState(cell.m_nRow, cell.m_nCol,
					GetItemState(cell.m_nRow, cell.m_nCol) & ~GVIS_SELECTED);

				// If this is to be reselected, continue on past the redraw
				if (nMinRow <= cell.m_nRow && cell.m_nRow <= nMaxRow &&
					nMinCol <= cell.m_nCol && cell.m_nCol <= nMaxCol)
					continue;

				if ( (VisCellRange.IsValid() && VisCellRange.InRange( cell )) )
				{
					if (bForceRepaint && pDC)                    // Redraw NOW
						RedrawCell(cell.m_nRow, cell.m_nCol, pDC);
					else
			...
			nMinRow >= 0 && nMinCol >= 0 && nMaxRow >= 0 && nMaxCol >= 0)
		{
			for (pos = m_PrevSelectedCellMap.GetStartPosition(); pos != NULL; /* nothing */)
			{
				DWORD key;
				CAsCoolGridCellID cell;
				m_PrevSelectedCellMap.GetNextAssoc(pos, key, (CAsCoolGridCellID&)cell);

				if (!IsValid(cell))
					continue;

				int nState = GetItemState(cell.m_nRow, cell.m_nCol);

				// Set state as Selected. This will add the cell to m_SelectedCellMap
				SetItemState(cell.m_nRow, cell.m_nCol, nState | GVIS_SELECTED);
			...
			m_PrevSelectedCellMap.RemoveAll(); // -Guan added, Clear the preSlected cells because they have been added to m_SelectedCellMap
		}
	}
...
// selects all cells
void CAsCoolGridCtrl::SelectAllCells()
{
	if (!m_bEnableSelection)
		return;
...
// selects columns
void CAsCoolGridCtrl::SelectColumns(CAsCoolGridCellID currentCell, 
									BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	if (!m_bEnableSelection)
		return;

	//if (currentCell.m_nCol == m_idCurrentCell.m_nCol) return;
	...
	if (GetSingleColSelection())
		SetSelectedRange(GetFixedRowCount(), currentCell.m_nCol,
		GetRowCount()-1,    currentCell.m_nCol,
		bForceRedraw, bSelectCells);
	else
		SetSelectedRange(GetFixedRowCount(),
		min(m_SelectionStartCell.m_nCol, currentCell.m_nCol),
		GetRowCount()-1,
		max(m_SelectionStartCell.m_nCol, currentCell.m_nCol),
		bForceRedraw, bSelectCells);
}

// selects rows
void CAsCoolGridCtrl::SelectRows(CAsCoolGridCellID currentCell, 
								 BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	if (!m_bEnableSelection)
		return;
	...
	if (currentCell.m_nRow < m_nFixedRows)
		return;
	if (!IsValid(currentCell))
		return;

	if (GetSingleRowSelection())
		SetSelectedRange(currentCell.m_nRow, GetFixedColumnCount(),
		currentCell.m_nRow, GetColumnCount()-1, 
		bForceRedraw, bSelectCells);
	else
		SetSelectedRange(min(m_SelectionStartCell.m_nRow, currentCell.m_nRow),
		GetFixedColumnCount(),
		max(m_SelectionStartCell.m_nRow, currentCell.m_nRow),
		GetColumnCount()-1,
		bForceRedraw, bSelectCells);
}

// selects cells
void CAsCoolGridCtrl::SelectCells(CAsCoolGridCellID currentCell, 
								  BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	if (!m_bEnableSelection)
		return;

	int row = currentCell.m_nRow;
	int col = currentCell.m_nCol;
	if (row < m_nFixedRows || col < m_nFixedCols)
		return;
	if (!IsValid(currentCell))
		return;

	SetSelectedRange(min(m_SelectionStartCell.m_nRow, row),
		min(m_SelectionStartCell.m_nCol, col),
		__max(m_SelectionStartCell.m_nRow, row),
		__max(m_SelectionStartCell.m_nCol, col),
		bForceRedraw, bSelectCells);
}

// Called when mouse/keyboard selection is a-happening.
void CAsCoolGridCtrl::OnSelecting(const CAsCoolGridCellID& currentCell)
{
	if (!m_bEnableSelection)
		return;
...
void CAsCoolGridCtrl::ValidateAndModifyCellContents(int nRow, int nCol, LPCTSTR strText)
{
...
void CAsCoolGridCtrl::ClearCells(CAsCoolGridCellRange Selection)
{
...
void CAsCoolGridCtrl::CutSelectedText()
{
	if (!IsEditable())
		return;

	// GUAN fixed: Add GetCount() avoid grid cell is deleted during update
	for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL && m_SelectedCellMap.GetCount() > 0; )
	{
		DWORD key;
		CAsCoolGridCellID cell;
		m_SelectedCellMap.GetNextAssoc(pos, key, (CAsCoolGridCellID&)cell);
		ValidateAndModifyCellContents(cell.m_nRow, cell.m_nCol, _T(""));
	}
}

// Copies text from the selected cells to the clipboard
COleDataSource* CAsCoolGridCtrl::CopyTextFromGrid()
{
	USES_CONVERSION;

	CAsCoolGridCellRange Selection = GetSelectedCellRange();
	...
	for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
	{
		...
		//if (row != Selection.GetMaxRow())  -Guan remove this, because the last line should also be ended with "\r\n"
		str += _T("\r\n");

		// Guan Fixed, Make the DBCS and UNICODE string length correct
		CStringA strCh(str);
		sf.Write(strCh.GetBuffer(1), strCh.GetLength());
		strCh.ReleaseBuffer();
	}
...
BOOL CAsCoolGridCtrl::PasteTextToGrid(CAsCoolGridCellID cell, COleDataObject* pDataObject, 
									  BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	CAsCoolGridCellRange PasteRange(cell.m_nRow, cell.m_nCol,-1,-1);
	int nIndex;
	do
	{
		int nColumn = 0;
		...
		{
			// skip hidden columns
			int iColVis = cell.m_nCol + nColumn;
			...
			CAsCoolGridCellID TargetCell(iRowVis, iColVis);
			if (IsValid(TargetCell))
			{
				strCellText.TrimLeft();
				strCellText.TrimRight();

				ValidateAndModifyCellContents(TargetCell.m_nRow, TargetCell.m_nCol, strCellText);

				// Make sure cell is not selected to avoid data loss
				SetItemState(TargetCell.m_nRow, TargetCell.m_nCol,
					GetItemState(TargetCell.m_nRow, TargetCell.m_nCol) & ~GVIS_SELECTED);
...
// Start drag n drop
void CAsCoolGridCtrl::OnBeginDrag()
{
	if (!m_bAllowDragAndDrop)

		return;
	CAsCoolGridCellRange PrevSelRangeBak = GetSelectedCellRange();
	SetSelectedRange(PrevSelRangeBak, TRUE, TRUE);

	COleDataSource* pSource = CopyTextFromGrid();

	if (pSource)
	{
		SendMessageToParent(GetSelectedCellRange().GetTopLeft().m_nRow,
		...
		{
			CAsCoolGridCellRange CurSelRange = GetSelectedCellRange();
			for (int j = PrevSelRangeBak.GetMinCol(); j <= PrevSelRangeBak.GetMaxCol(); j++)
			{
				for (int i = PrevSelRangeBak.GetMinRow(); i <= PrevSelRangeBak.GetMaxRow(); i++)
				{
					if (!CurSelRange.InRange(i, j))
					{
						ValidateAndModifyCellContents(i, j, _T(""));
					}
				}
			}
		}
...
DROPEFFECT CAsCoolGridCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
	...
	CAsCoolGridCellID cell = GetCellFromPt(point, FALSE);

	// If not valid, set the previously drop-highlighted cell as no longer drop-highlighted
	if (!IsValid(cell))
	{
		OnDragLeave();
		m_LastDragOverCell = CAsCoolGridCellID(-1,-1);
		return DROPEFFECT_NONE;
	}
	...
	if (cell != m_LastDragOverCell)
	{
		...
		{
			UINT nState = GetItemState(m_LastDragOverCell.m_nRow, m_LastDragOverCell.m_nCol);
			SetItemState(m_LastDragOverCell.m_nRow, m_LastDragOverCell.m_nCol,
...
DROPEFFECT CAsCoolGridCtrl::OnDragEnter(COleDataObject* pDataObject, DWORD dwKeyState,
										CPoint point)
{
...
void CAsCoolGridCtrl::OnDragLeave()
{
	// Set the previously drop-highlighted cell as no longer drop-highlighted
	if (IsValid(m_LastDragOverCell))
	{
...
BOOL CAsCoolGridCtrl::OnDrop(COleDataObject* pDataObject, DROPEFFECT dropEffect, CPoint /*point*/) // -Guan Changed 08-10-24
{
	m_MouseMode = MOUSE_NOTHING;
	...
	OnDragLeave();
	SetFocusCell(m_LastDragOverCell);
	return PasteTextToGrid(m_LastDragOverCell, pDataObject);
}

void CAsCoolGridCtrl::OnEditCut()
{
	if (!IsEditable())
		return;
...
void CAsCoolGridCtrl::OnEditCopy()
{
	COleDataSource* pSource = CopyTextFromGrid();
	if (!pSource)
		return;
...
void CAsCoolGridCtrl::OnEditPaste()
{
	if (!IsEditable())
		return;

	CAsCoolGridCellRange cellRange = GetSelectedCellRange();

	// Get the top-left selected cell, or the Focus cell, or the topleft (non-fixed) cell
	CAsCoolGridCellID cell;
	if (cellRange.IsValid())
	{
		cell.m_nRow = cellRange.GetMinRow();
		cell.m_nCol = cellRange.GetMinCol();
	}
	else
	{
		cell = GetFocusCell();
	...
	if ( IsItemEditing(cell.m_nRow, cell.m_nCol) )
	{
		CAsCoolGridCellBase* pCell = GetCell(cell.m_nRow, cell.m_nCol);
		ASSERT(pCell);
		if (!pCell) return;
...
void CAsCoolGridCtrl::OnEditSelectAll()
{
	SendMessageToParent(m_LeftClickDownCell.m_nRow, m_LeftClickDownCell.m_nCol, GVN_SELCHANGING);
	SelectAllCells();
	SendMessageToParent(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol, GVN_SELCHANGED);
}

void CAsCoolGridCtrl::OnUpdateEditCopy(CCmdUI* pCmdUI)
{
...
void CAsCoolGridCtrl::OnUpdateEditCut(CCmdUI* pCmdUI)
{
...
void CAsCoolGridCtrl::OnUpdateEditPaste(CCmdUI* pCmdUI)
{
	CAsCoolGridCellID cell = GetFocusCell();
...
void CAsCoolGridCtrl::OnUpdateEditSelectAll(CCmdUI* pCmdUI)
{
...
BOOL CAsCoolGridCtrl::MouseOverRowResizeArea(CPoint& point)
{
	if (point.x >= GetFixedColumnWidth())
		return FALSE;

	CAsCoolGridCellID idCurrentCell = GetCellFromPt(point);
	CPoint start;
	if (!GetCellOrigin(idCurrentCell, &start))
		return FALSE;

	int endy = start.y + GetRowHeight(idCurrentCell.m_nRow);

	if ((point.y - start.y < m_nResizeCaptureRange && idCurrentCell.m_nRow != 0) ||
...
BOOL CAsCoolGridCtrl::MouseOverColumnResizeArea(CPoint& point)
{
	...
	int endx = start.x + GetColumnWidth(idCurrentCell.m_nCol);

	if ((point.x - start.x < m_nResizeCaptureRange && idCurrentCell.m_nCol != 0) ||
...
CAsCoolGridCellID CAsCoolGridCtrl::GetCellFromPt(CPoint point, BOOL bAllowFixedCellCheck /*=TRUE*/)
{
	CAsCoolGridCellID cellID; // return value
	...
	else    // in non-fixed col
	{
		...
		int col = idTopLeft.m_nCol; //m_nFixedCols;
	...
	else
	{
		...
		int row = idTopLeft.m_nRow; //m_nFixedRows;
...
////////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl cellrange functions

// Gets the first non-fixed cell ID
CAsCoolGridCellID CAsCoolGridCtrl::GetTopleftNonFixedCell(BOOL bForceRecalculation /*=FALSE*/)
{
	...
	m_idTopLeftCell.m_nCol = m_nFixedCols;
	int nRight = 0;
	while (nRight < nHorzScroll && m_idTopLeftCell.m_nCol < (GetColumnCount()-1))
		nRight += GetColumnWidth(m_idTopLeftCell.m_nCol++);

	m_idTopLeftCell.m_nRow = m_nFixedRows;
	int nTop = 0;
	while (nTop < nVertScroll && m_idTopLeftCell.m_nRow < (GetRowCount()-1))
		nTop += GetRowHeight(m_idTopLeftCell.m_nRow++);

	//TRACE2("TopLeft cell is row %d, col %d\n",m_idTopLeftCell.m_nRow, m_idTopLeftCell.m_nCol);
	return m_idTopLeftCell;
}

// This gets even partially visible cells
CAsCoolGridCellRange CAsCoolGridCtrl::GetVisibleNonFixedCellRange(LPRECT pRect /*=NULL*/, BOOL bForceRecalculation /*=FALSE*/)
{
	int i;
	CRect rect;
	GetClientRect(rect);

	CAsCoolGridCellID idTopLeft = GetTopleftNonFixedCell(bForceRecalculation);

	// calc bottom
	int bottom = GetFixedRowHeight();
	for (i = idTopLeft.m_nRow; i < GetRowCount(); i++)
	{
	...
	for (i = idTopLeft.m_nCol; i < GetColumnCount(); i++)
	{
	...
	return CAsCoolGridCellRange(idTopLeft.m_nRow, idTopLeft.m_nCol, maxVisibleRow, maxVisibleCol);
}



// used by ResetScrollBars() - This gets only fully visible cells
CAsCoolGridCellRange CAsCoolGridCtrl::GetUnobstructedNonFixedCellRange(BOOL bForceRecalculation /*=FALSE*/)
{
	CRect rect;
	GetClientRect(rect);
...
// If no selection, then the returned CAsCoolGridCellRange will be invalid
CAsCoolGridCellRange CAsCoolGridCtrl::GetSelectedCellRange() const
{
	CAsCoolGridCellRange Selection(GetRowCount(), GetColumnCount(), -1,-1);
	...
	for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
	{
		...
		Selection.SetMinRow( min(Selection.GetMinRow(), cell.m_nRow) );
		Selection.SetMinCol( min(Selection.GetMinCol(), cell.m_nCol) );
		Selection.SetMaxRow( __max(Selection.GetMaxRow(), cell.m_nRow) );
		Selection.SetMaxCol( __max(Selection.GetMaxCol(), cell.m_nCol) );
	}
...
CAsCoolGridCellRange CAsCoolGridCtrl::GetCellRange() const
{
	return CAsCoolGridCellRange(0, 0, GetRowCount() - 1, GetColumnCount() - 1);
}

// Resets the selected cell range to the empty set.
void CAsCoolGridCtrl::ResetSelectedRange()
{
	m_PrevSelectedCellMap.RemoveAll();
...
int CAsCoolGridCtrl::GetSelectedCells(CArray<CAsCoolGridCellID> &arraySelCells) const
{
	arraySelCells.RemoveAll();

	int ncount = m_SelectedCellMap.GetCount();
	POSITION pos = NULL;
	pos = m_SelectedCellMap.GetStartPosition();
	while (pos)
	{
		DWORD key;
		CAsCoolGridCellID cell;
		...
		if (!IsValid(cell))
			continue;
		arraySelCells.Add(cell);
	}
	return arraySelCells.GetCount();
}

// Get all selected rows
int CAsCoolGridCtrl::GetSelectedRows(CUIntArray &naRowsIndex) const
{
	naRowsIndex.RemoveAll();

	CArray<CAsCoolGridCellID> arraySelectedCells;
	int nSelCellsCount = GetSelectedCells(arraySelectedCells);
	
	for (int i=0; i < nSelCellsCount; i++)
	{
		int nCurRow = arraySelectedCells[i].m_nRow;
		if (IsValid(arraySelectedCells[i]))
		{
			BOOL bFindEqualRow = FALSE;
			for (int t=0; t < naRowsIndex.GetCount(); t++)
			{
				if (nCurRow == naRowsIndex[t])
				{
					bFindEqualRow = TRUE;
					break;
				}
			...
			if (!bFindEqualRow)
			{	
				naRowsIndex.Add(nCurRow);
			}
		}
	}
	return naRowsIndex.GetCount();
}
// Get all selected columns
int CAsCoolGridCtrl::GetSelectedColumns(CUIntArray &naColsIndex) const
{
	naColsIndex.RemoveAll();
	...
	for (int i=0; i < nSelCellsCount; i++)
	{
		int nCurCol = arraySelectedCells[i].m_nCol;
		if (IsValid(arraySelectedCells[i]))
		{
			BOOL bFindEqualRow = FALSE;
			for (int t=0; t < naColsIndex.GetCount(); t++)
			{
				if (nCurCol == naColsIndex[t])
				{
					bFindEqualRow = TRUE;
					break;
				}
			...
			if (!bFindEqualRow)
			{	
				naColsIndex.Add(nCurCol);
			}
		}
	}
	return naColsIndex.GetCount();
}

void CAsCoolGridCtrl::SetSelectedRow(int nRow, BOOL bAddSelRow /*= FALSE*/)
{
	if (!IsSelectable() || GetFixedRowCount() > nRow)
	{			
		return;
	}

	if (bAddSelRow && !GetSingleRowSelection())
	{
		int nColCount = GetColumnCount();
		for (int i=GetFixedColumnCount(); i < nColCount; i++)
		{	
			SetItemState(nRow, i, GetItemState(nRow, i) | GVIS_SELECTED);
		}
	}
	else
	{
...
void CAsCoolGridCtrl::SetSelectedColumn(int nCol, BOOL bAddSelCol/* = FALSE*/)
{
	if (!IsSelectable() || GetFixedColumnCount() > nCol)
	{			
		return;
	}

	if (bAddSelCol && !GetSingleColSelection())
	{
		int nRowsCount = GetColumnCount();
		for (int i=GetFixedRowCount(); i < nRowsCount; i++)
		{	
			SetItemState(i, nCol, GetItemState(i, nCol) | GVIS_SELECTED);
		}
	}
	else
	{
		SetSelectedRange(GetFixedRowCount(), nCol, GetRowCount()-1, nCol, TRUE);
	}
}

BOOL CAsCoolGridCtrl::SetColumnBkColor(int nCol, COLORREF clr /*= CLR_DEFAULT*/)
{
	int nRowCount = GetRowCount();
	for (int i=GetFixedRowCount(); i < nRowCount; i++)
	{
		if (!SetItemBkColour(i, nCol, clr))
		{
			return FALSE;
		}
	}
...
BOOL CAsCoolGridCtrl::SetRowBkColor(int nRow, COLORREF clr /*= CLR_DEFAULT*/)
{
	int nColumnCount = GetColumnCount();
	for (int i=GetFixedColumnCount(); i < nColumnCount; i++)
	{
		if (!SetItemBkColour(nRow, i, clr))
		{
			return FALSE;
		}
	}
...
BOOL CAsCoolGridCtrl::SetColumnFgColor(int nCol, COLORREF clr /*= CLR_DEFAULT*/)
{
	int nRowCount = GetRowCount();
	for (int i=GetFixedRowCount(); i < nRowCount; i++)
	{
		if (!SetItemFgColour(i, nCol, clr))
		{
			return FALSE;
		}
	}
...
BOOL CAsCoolGridCtrl::SetRowFgColor(int nRow, COLORREF clr /*= CLR_DEFAULT*/)
{
	int nColumnCount = GetColumnCount();
	for (int i=GetFixedColumnCount(); i < nColumnCount; i++)
	{
		if (!SetItemFgColour(nRow, i, clr))
		{
			return FALSE;
		}
	}
...
BOOL CAsCoolGridCtrl::SetColEditable(int nCol, BOOL bEnable)
{
	int nRowCount = GetRowCount();
	for (int i=GetFixedRowCount(); i < nRowCount; i++)
	{
		if (!SetCellEditalbe(i, nCol, bEnable))
		{
			return FALSE;
		}
	}
...
BOOL CAsCoolGridCtrl::SetRowEditable(int nRow, BOOL bEnable)
{
	int nColumnCount = GetColumnCount();
	for (int i=GetFixedColumnCount(); i < nColumnCount; i++)
	{
		if (!SetCellEditalbe(nRow, i, bEnable))
		{
			return FALSE;
		}
	}
...
int CAsCoolGridCtrl::GetScrollPos32(int nBar, BOOL bGetTrackPos /* = FALSE */)
{
	SCROLLINFO si;
...
BOOL CAsCoolGridCtrl::SetScrollPos32(int nBar, int nPos, BOOL bRedraw /* = TRUE */)
{
	m_idTopLeftCell.m_nRow = -1;
...
void CAsCoolGridCtrl::EnableScrollBars(int nBar, BOOL bEnable /*=TRUE*/)
{
	if (bEnable)
	{
...
void CAsCoolGridCtrl::ResetScrollBars()
{
	// Force a refresh. 
	m_idTopLeftCell.m_nRow = -1;
...
BOOL CAsCoolGridCtrl::GetCellOrigin(int nRow, int nCol, LPPOINT p)
{
	int i;
	...
	{                                        // is a scrollable data column
		for (i = 0; i < m_nFixedCols; i++)
			p->x += GetColumnWidth(i);
		for (i = idTopLeft.m_nCol; i < nCol; i++)
			p->x += GetColumnWidth(i);
	}
	...
	{                                        // is a scrollable data row
		for (i = 0; i < m_nFixedRows; i++)
			p->y += GetRowHeight(i);
		for (i = idTopLeft.m_nRow; i < nRow; i++)
			p->y += GetRowHeight(i);
	}
...
BOOL CAsCoolGridCtrl::GetCellOrigin(const CAsCoolGridCellID& cell, LPPOINT p)
{
	return GetCellOrigin(cell.m_nRow, cell.m_nCol, p);
}

// Returns the bounding box of the cell
BOOL CAsCoolGridCtrl::GetCellRect(const CAsCoolGridCellID& cell, LPRECT pRect)
{
	return GetCellRect(cell.m_nRow, cell.m_nCol, pRect);
}

BOOL CAsCoolGridCtrl::GetCellRect(int nRow, int nCol, LPRECT pRect)
{
	CPoint CellOrigin;
...
BOOL CAsCoolGridCtrl::GetTextRect(const CAsCoolGridCellID& cell, LPRECT pRect)
{
	return GetTextRect(cell.m_nRow, cell.m_nCol, pRect);
}

BOOL CAsCoolGridCtrl::GetTextRect(int nRow, int nCol, LPRECT pRect)
{
	CAsCoolGridCellBase* pCell = GetCell( nRow, nCol);
	if( pCell == NULL)
		return FALSE;
...
BOOL CAsCoolGridCtrl::GetCellRangeRect(const CAsCoolGridCellRange& cellRange, LPRECT lpRect)
{
...
LRESULT CAsCoolGridCtrl::OnSetFont(WPARAM hFont, LPARAM /*lParam */)
{
...
LRESULT CAsCoolGridCtrl::OnGetFont(WPARAM /*wParam*/, LPARAM /*lParam*/)
{
	//LOGFONT    lf;
...
BOOL CAsCoolGridCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
	if (nHitTest == HTCLIENT)
	{
		...
		default:
			if (!GetVirtualMode())
			{
				...
				ScreenToClient(&pt);
				CAsCoolGridCellID cell = GetCellFromPt(pt);
				if (IsValid(cell))
				{
					CAsCoolGridCellBase* pCell = GetCell(cell.m_nRow, cell.m_nCol);
					if (pCell)
...
BOOL CAsCoolGridCtrl::SetFixedRowCount(int nFixedRows)
{
	...
	// Force recalculation
	m_idTopLeftCell.m_nCol = -1;

	if (nFixedRows > GetRowCount())
		if (!SetRowCount(nFixedRows))
			return FALSE;

	if (m_idCurrentCell.m_nRow < nFixedRows)
		SetFocusCell(-1, - 1);
...
BOOL CAsCoolGridCtrl::SetFixedColumnCount(int nFixedCols)
{
	...
	if (m_idCurrentCell.m_nCol < nFixedCols)
		SetFocusCell(-1, - 1);
...
BOOL CAsCoolGridCtrl::SetRowCount(int nRows)
{
	BOOL bResult = TRUE;
	...
	if (m_idCurrentCell.m_nRow >= nRows)
		SetFocusCell(-1, - 1);
...
BOOL CAsCoolGridCtrl::SetColumnCount(int nCols)
{
	BOOL bResult = TRUE;
	...
	if (m_idCurrentCell.m_nCol >= nCols)
		SetFocusCell(-1, - 1);
...
int CAsCoolGridCtrl::InsertColumn(LPCTSTR strHeading,
								  ...
								  int nColumn  /* = -1 */)
{
	...
	//ResetSelectedRange(); // Shouldn't affect the current status [12/25/2008 GUAN]
	...
	//////////////////////////////////////////////////////////////////////////
	// [12/27/2008 GUAN changed]
	// Adjust the selected columns after the nColumn, 
	// because the indexes of cols have been affected by delete column
	CArray<CAsCoolGridCellID> arCellIDBeAdjusted;
	for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
	{
		DWORD key;
		CAsCoolGridCellID cell;
		...
		if (cell.m_nCol > nColumn)
		{
			cell.m_nCol++; // Increase col index
			arCellIDBeAdjusted.Add(cell);
			m_SelectedCellMap.RemoveKey(key);
		}
	}
	for (int i=0; i < arCellIDBeAdjusted.GetCount(); i++)
	{
		CAsCoolGridCellID cell = arCellIDBeAdjusted[i];
		m_SelectedCellMap.SetAt(MAKELONG(cell.m_nRow, cell.m_nCol), cell);
	}
	//////////////////////////////////////////////////////////////////////////
	...
	if (m_idCurrentCell.m_nCol != -1 && nColumn < m_idCurrentCell.m_nCol)
		m_idCurrentCell.m_nCol++;
...
int CAsCoolGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
	...
	//ResetSelectedRange(); // Shouldn't affect the current status [12/25/2008 GUAN changed]
	...
	//////////////////////////////////////////////////////////////////////////
	// [12/27/2008 GUAN changed]
	// Adjust the selected rows after the nRow, 
	// because the indexes of rows have been affected by delete row
	CArray<CAsCoolGridCellID> arCellIDBeAdjusted;
	for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
	{
		DWORD key;
		...
		if (cell.m_nRow > nRow)
		{
			cell.m_nRow++; // Increase row index
			arCellIDBeAdjusted.Add(cell);
			m_SelectedCellMap.RemoveKey(key);
		}
	}
	...
	if (m_idCurrentCell.m_nRow != -1 && nRow < m_idCurrentCell.m_nRow)
		m_idCurrentCell.m_nRow++;
...
///////////////////////////////////////////////////////////////////////////////
// Cell creation stuff
BOOL CAsCoolGridCtrl::SetCellType(int nRow, int nCol, CRuntimeClass* pRuntimeClass)
{
	if (GetVirtualMode())
		return FALSE;
	...
	if (!pRuntimeClass->IsDerivedFrom(RUNTIME_CLASS(CAsCoolGridCellBase)))
	{
		ASSERT( FALSE);
		return FALSE;
	}

	CAsCoolGridCellBase* pNewCell = (CAsCoolGridCellBase*) pRuntimeClass->CreateObject();

	CAsCoolGridCellBase* pCurrCell = GetCell(nRow, nCol);
	if (pCurrCell)
		*pNewCell = *pCurrCell;
...
BOOL CAsCoolGridCtrl::SetCellEditalbe(int nRow, int nCol, BOOL bEnable)
{
	if (!IsValid(nRow, nCol)) return FALSE;

	DWORD dwState = GetItemData(nRow, nCol);

	return SetItemState(nRow, nCol, bEnable ? (dwState&~GVIS_READONLY) : (dwState|GVIS_READONLY) );		
}

BOOL CAsCoolGridCtrl::SetDefaultCellType( CRuntimeClass* pRuntimeClass)
{
...
/*virtual*/ CAsCoolGridCellBase* CAsCoolGridCtrl::CreateCell(int nRow, int nCol)
{
	ASSERT(!GetVirtualMode());

	if (!m_pRtcDefault || !m_pRtcDefault->IsDerivedFrom(RUNTIME_CLASS(CAsCoolGridCellBase)))
	{
		ASSERT( FALSE);
		return NULL;
	}
	CAsCoolGridCellBase* pCell = (CAsCoolGridCellBase*) m_pRtcDefault->CreateObject();
	if (!pCell)
		return NULL;
	...
	// GUAN: Validate the Default State, exp. GVIS_READONLY
	pCell->SetState(pCell->GetDefaultCell()->GetState());

	if (nCol < m_nFixedCols)
		pCell->SetState(pCell->GetState() |GVIS_READONLY | GVIS_FIXED | GVIS_FIXEDCOL);
	if (nRow < m_nFixedRows)
		pCell->SetState(pCell->GetState() |GVIS_READONLY | GVIS_FIXED | GVIS_FIXEDROW);
...
/*virtual*/ void CAsCoolGridCtrl::DestroyCell(int nRow, int nCol)
{
...
BOOL CAsCoolGridCtrl::DeleteColumn(int nColumn)
{
	if (nColumn < 0 || nColumn >= GetColumnCount())
		return FALSE;
	...
	for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
	{
		...
		if (cell.m_nCol > nColumn)
		{
			cell.m_nCol--; // Decrease col index
			arCellIDBeAdjusted.Add(cell);
			m_SelectedCellMap.RemoveKey(key);
		}
	}
	...
	if (nColumn == m_idCurrentCell.m_nCol)
		m_idCurrentCell.m_nRow = m_idCurrentCell.m_nCol = -1;
	else if (nColumn < m_idCurrentCell.m_nCol)
		m_idCurrentCell.m_nCol--;
...
BOOL CAsCoolGridCtrl::DeleteRow(int nRow)
{
	if (nRow < 0 || nRow >= GetRowCount())
		return FALSE;
	...
	for (POSITION pos = m_SelectedCellMap.GetStartPosition(); pos != NULL; )
	{
		...
		if (cell.m_nRow > nRow)
		{
			cell.m_nRow--; // Decrease row index
			arCellIDBeAdjusted.Add(cell);
			m_SelectedCellMap.RemoveKey(key);
		}
	}
	...
	if (nRow == m_idCurrentCell.m_nRow)
		m_idCurrentCell.m_nRow = m_idCurrentCell.m_nCol = -1;
	else if (nRow < m_idCurrentCell.m_nRow)
		m_idCurrentCell.m_nRow--;
...
BOOL CAsCoolGridCtrl::DeleteNonFixedRows()
{
	ResetSelectedRange();
...
BOOL CAsCoolGridCtrl::DeleteAllItems()
{
	ResetSelectedRange();
...
/////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl data functions

// Set CListCtrl::GetNextItem for details
CAsCoolGridCellID CAsCoolGridCtrl::GetNextItem(CAsCoolGridCellID& cell, int nFlags) const
{
	if ((nFlags & GVNI_ALL) == GVNI_ALL)
	{    // GVNI_ALL Search whole Grid beginning from cell
		//          First row (cell.m_nRow) -- ONLY Columns to the right of cell
		//          following rows       -- ALL  Columns
		int row = cell.m_nRow , col = cell.m_nCol + 1;
		if (row <= 0)
		...
		for (; row < GetRowCount(); row++)
		{
			...
			for (; col < GetColumnCount(); col++)
			{
				...
				if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) || 
					...
					return CAsCoolGridCellID(row, col);
			}
			// go to First Column
			col = GetFixedColumnCount();
		}
	...
	{   // GVNI_AREA Search Grid beginning from cell to Lower-Right of Grid
		//           Only rows starting with  cell.m_nRow and below
	...
	{
		for (int row = cell.m_nRow - 1; row >= GetFixedRowCount(); row--) 
		{
			int nState = GetItemState(row, cell.m_nCol);
			if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) || 
				(nFlags & GVNI_FOCUSED     && nState & GVIS_FOCUSED)     ||
				(nFlags & GVNI_SELECTED    && nState & GVIS_SELECTED)    ||
				(nFlags & GVNI_READONLY    && nState & GVIS_READONLY)    ||
				(nFlags & GVNI_FIXED       && nState & GVIS_FIXED)       ||
				(nFlags & GVNI_MODIFIED    && nState & GVIS_MODIFIED))
				return CAsCoolGridCellID(row, cell.m_nCol);
		}
	}
	else if ((nFlags & GVNI_BELOW) == GVNI_BELOW)
	{
		for (int row = cell.m_nRow + 1; row < GetRowCount(); row++) 
		{
			int nState = GetItemState(row, cell.m_nCol);
			if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) || 
				(nFlags & GVNI_FOCUSED     && nState & GVIS_FOCUSED)     ||
	...
	{
		for (int col = cell.m_nCol - 1; col >= GetFixedColumnCount(); col--) 
		{
			int nState = GetItemState(cell.m_nRow, col);
			if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) || 
				(nFlags & GVNI_FOCUSED     && nState & GVIS_FOCUSED)     ||
				(nFlags & GVNI_SELECTED    && nState & GVIS_SELECTED)    ||
				(nFlags & GVNI_READONLY    && nState & GVIS_READONLY)    ||
				(nFlags & GVNI_FIXED       && nState & GVIS_FIXED)       ||
				(nFlags & GVNI_MODIFIED    && nState & GVIS_MODIFIED))
				return CAsCoolGridCellID(cell.m_nRow, col);
		}
	}
	else if ((nFlags & GVNI_TORIGHT) == GVNI_TORIGHT)
	{
		for (int col = cell.m_nCol + 1; col < GetColumnCount(); col++) 
		{
			int nState = GetItemState(cell.m_nRow, col);
			if ((nFlags & GVNI_DROPHILITED && nState & GVIS_DROPHILITED) || 
				(nFlags & GVNI_FOCUSED     && nState & GVIS_FOCUSED)     ||
	...
	return CAsCoolGridCellID(-1, -1);
}

// Sorts on a given column using the cell text
BOOL CAsCoolGridCtrl::SortTextItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
	return CAsCoolGridCtrl::SortItems(pfnCellTextCompare, nCol, bAscending, data);
}

void CAsCoolGridCtrl::SetCompareFunction(PFNLVCOMPARE pfnCompare)
{
...
// function
BOOL CAsCoolGridCtrl::SortItems(int nCol, BOOL bAscending, LPARAM data /* = 0 */)
{
	SetSortColumn(nCol);
	...
	if (m_pfnCompare == NULL)
		...
		return CAsCoolGridCtrl::SortItems(m_pfnCompare, nCol, bAscending, data);
}

// Sorts on a given column using the supplied compare function (see CListCtrl::SortItems)
BOOL CAsCoolGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending,
								LPARAM data /* = 0 */)
{
	SetSortColumn(nCol);
	SetSortAscending(bAscending);
...
int CALLBACK CAsCoolGridCtrl::pfnCellTextCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);

	CAsCoolGridCellBase* pCell1 = (CAsCoolGridCellBase*) lParam1;
	CAsCoolGridCellBase* pCell2 = (CAsCoolGridCellBase*) lParam2;
	if (!pCell1 || !pCell2) return 0;

	return _tcscmp(pCell1->GetText(), pCell2->GetText());
}

int CALLBACK CAsCoolGridCtrl::pfnCellNumericCompare(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
	UNUSED_ALWAYS(lParamSort);
...
BOOL CAsCoolGridCtrl::SortItems(PFNLVCOMPARE pfnCompare, int nCol, BOOL bAscending, LPARAM data,
								int low, int high)
{
	...
	while (lo <= hi)
	{
		...
		if (lo <= hi)
		{
			...
			if (pfnCompare((LPARAM)GetCell(lo, nCol), (LPARAM)GetCell(hi, nCol), data) != 0)
			{
				for (int col = 0; col < GetColumnCount(); col++)
				{
					CAsCoolGridCellBase *pCell = GetCell(lo, col);
...
BOOL CAsCoolGridCtrl::SetItem(const GV_ITEM* pItem)
{
	if (!pItem || GetVirtualMode())
		return FALSE;

	CAsCoolGridCellBase* pCell = GetCell(pItem->row, pItem->col);
	if (!pCell)
		return FALSE;
...
BOOL CAsCoolGridCtrl::GetItem(GV_ITEM* pItem)
{
	if (!pItem)
		return FALSE;
	CAsCoolGridCellBase* pCell = GetCell(pItem->row, pItem->col);
...
BOOL CAsCoolGridCtrl::SetItemText(int nRow, int nCol, LPCTSTR str)
{
	if (GetVirtualMode())
		return FALSE;
...
BOOL CAsCoolGridCtrl::SetItemTextFmt(int nRow, int nCol, LPCTSTR szFmt, ...)
{
	if (GetVirtualMode())
		return FALSE;
...
BOOL CAsCoolGridCtrl::SetItemTextFmtID(int nRow, int nCol, UINT nID, ...)
{
	if (GetVirtualMode())
		return FALSE;
...
BOOL CAsCoolGridCtrl::SetItemData(int nRow, int nCol, LPARAM lParam)
{
	if (GetVirtualMode())
		return FALSE;
...
LPARAM CAsCoolGridCtrl::GetItemData(int nRow, int nCol) const
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell)
		return (LPARAM) 0;
...
BOOL CAsCoolGridCtrl::SetItemImage(int nRow, int nCol, int iImage)
{
	if (GetVirtualMode())
		return FALSE;
...
int CAsCoolGridCtrl::GetItemImage(int nRow, int nCol) const
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CAsCoolGridCtrl::SetItemState(int nRow, int nCol, UINT state)
{
	...
	if (bSelected && !(state & GVIS_SELECTED))
	{
		...
		if (m_SelectedCellMap.Lookup(key, (CAsCoolGridCellID&)cell))
			m_SelectedCellMap.RemoveKey(key);
	}
	...
	{
		CAsCoolGridCellID cell(nRow, nCol);
...
UINT CAsCoolGridCtrl::GetItemState(int nRow, int nCol) const
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CAsCoolGridCtrl::SetItemFormat(int nRow, int nCol, UINT nFormat)
{
	if (GetVirtualMode())
		return FALSE;
...
UINT CAsCoolGridCtrl::GetItemFormat(int nRow, int nCol) const
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CAsCoolGridCtrl::SetItemBkColour(int nRow, int nCol, COLORREF cr /* = CLR_DEFAULT */)
{
	if (GetVirtualMode())
		return FALSE;
...
COLORREF CAsCoolGridCtrl::GetItemBkColour(int nRow, int nCol) const
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CAsCoolGridCtrl::SetItemFgColour(int nRow, int nCol, COLORREF cr /* = CLR_DEFAULT */)
{
	if (GetVirtualMode())
		return FALSE;
...
COLORREF CAsCoolGridCtrl::GetItemFgColour(int nRow, int nCol) const
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
BOOL CAsCoolGridCtrl::SetItemFont(int nRow, int nCol, const LOGFONT* plf)
{
	if (GetVirtualMode())
		return FALSE;
...
const LOGFONT* CAsCoolGridCtrl::GetItemFont(int nRow, int nCol)
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell) 
...
BOOL CAsCoolGridCtrl::IsItemEditing(int nRow, int nCol)
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
long CAsCoolGridCtrl::GetVirtualWidth() const
{
...
long CAsCoolGridCtrl::GetVirtualHeight() const
{
...
int CAsCoolGridCtrl::GetRowHeight(int nRow) const
{
...
int CAsCoolGridCtrl::GetColumnWidth(int nCol) const
{
...
BOOL CAsCoolGridCtrl::SetRowHeight(int nRow, int height)
{
	ASSERT(nRow >= 0 && nRow < m_nRows && height >= 0);
	if (nRow < 0 || nRow >= m_nRows || height < 0)
		return FALSE;

	// Ensure the cell's Height little than the grid's Height-30
	CRect rectClient;
	GetClientRect(&rectClient);
	int nClientHeight = rectClient.Height();
	m_arRowHeights[nRow] = height < nClientHeight -30 ? height : nClientHeight -30;
...
BOOL CAsCoolGridCtrl::SetColumnWidth(int nCol, int width)
{
	ASSERT(nCol >= 0 && nCol < m_nCols && width >= 0);
	if (nCol < 0 || nCol >= m_nCols || width < 0)
		return FALSE;

	// Ensure the cell's Width little than the grid's Width-30
	CRect rectClient;
	GetClientRect(&rectClient);
	int nClientWidth = rectClient.Width();
	m_arColWidths[nCol] = width < nClientWidth -30 ? width : nClientWidth -30;
...
int CAsCoolGridCtrl::GetFixedRowHeight() const
{
	int nHeight = 0;
...
int CAsCoolGridCtrl::GetFixedColumnWidth() const
{
	int nWidth = 0;
...
BOOL CAsCoolGridCtrl::AutoSizeColumn(int nCol, UINT nAutoSizeStyle /*=GVS_DEFAULT*/, 
									 BOOL bResetScroll /*=TRUE*/)
{
	...
	if (GetVirtualMode())
		SendCacheHintToParent(CAsCoolGridCellRange(nStartRow, nCol, nEndRow, nCol));
	...
	GetClientRect(&rectClient);
	int nClientWidth = rectClient.Width();
	int nAdjustWidth = nWidth;
	int nLimitWidth = nClientWidth -30; 
	if (nLimitWidth > 0 )
		nAdjustWidth = nWidth < nLimitWidth ? nWidth : nLimitWidth;
	m_arColWidths[nCol] = nAdjustWidth;
...
BOOL CAsCoolGridCtrl::AutoSizeRow(int nRow, BOOL bResetScroll /*=TRUE*/)
{
	ASSERT(nRow >= 0 && nRow < m_nRows);
	if (nRow < 0 || nRow >= m_nRows)
		return FALSE;
	...
	if (GetVirtualMode())
		SendCacheHintToParent(CAsCoolGridCellRange(nRow, 0, nRow, nNumColumns));
	...
	GetClientRect(&rectClient);
	int nClientHeight = rectClient.Height();
	int nAdjustHeight = nHeight;
	int nLimitWidth = nClientHeight -30; 
	if (nLimitWidth > 0 )
		nAdjustHeight = nHeight < nLimitWidth ? nHeight : nLimitWidth;
	m_arRowHeights[nRow] = nAdjustHeight;
...
void CAsCoolGridCtrl::AutoSizeColumns(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
	int nNumColumns = GetColumnCount();
	for (int nCol = 0; nCol < nNumColumns; nCol++)
	{
...
void CAsCoolGridCtrl::AutoSizeRows()
{
...
void CAsCoolGridCtrl::AutoSize(UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
	CDC* pDC = GetDC();
	if (!pDC)
		return;
	...
	if (GetVirtualMode())
		SendCacheHintToParent(CAsCoolGridCellRange(nStartRow, 0, nEndRow, nNumColumns));
...
void CAsCoolGridCtrl::ExpandColumnsToFit(BOOL bExpandFixed /*=TRUE*/)
{
	if (bExpandFixed)
	{
...
void CAsCoolGridCtrl::ExpandLastColumn()
{
	if (GetColumnCount() <= 0)
		return;
...
void CAsCoolGridCtrl::ExpandRowsToFit(BOOL bExpandFixed /*=TRUE*/)
{
	if (bExpandFixed)
	{
...
void CAsCoolGridCtrl::ExpandToFit(BOOL bExpandFixed /*=TRUE*/)
{
...
void CAsCoolGridCtrl::SetVirtualMode(BOOL bVirtual)
{
	DeleteAllItems();
...
void CAsCoolGridCtrl::SetGridLines(int nWhichLines /*=GVL_BOTH*/) 
{
	m_nGridLines = nWhichLines;
	Refresh();
}

void CAsCoolGridCtrl::SetListMode(BOOL bEnableListMode /*=TRUE*/)
{
	ResetSelectedRange();
	SetSortColumn(-1);
...
void CAsCoolGridCtrl::SetSortColumn(int nCol)
{
	if (m_nSortColumn >= 0)
...
BOOL CAsCoolGridCtrl::IsCellFixed(int nRow, int nCol)
{
	return (nRow < GetFixedRowCount() || nCol < GetFixedColumnCount());
}

void CAsCoolGridCtrl::SetModified(BOOL bModified /*=TRUE*/, int nRow /*=-1*/, int nCol /*=-1*/)
{
...
BOOL CAsCoolGridCtrl::GetModified(int nRow /*=-1*/, int nCol /*=-1*/)
{
	if (nRow >= 0 && nCol >= 0)
...
void CAsCoolGridCtrl::EnsureVisible(int nRow, int nCol)
{
	if (!m_bAllowDraw)
		return;
	...
	CAsCoolGridCellID idTopLeft = GetTopleftNonFixedCell(FALSE);
	CAsCoolGridCellID idNewTopLeft = GetTopleftNonFixedCell(TRUE);
	if (idNewTopLeft != idTopLeft)
	{
	...
	*/
	...
	CAsCoolGridCellRange VisibleCells = GetVisibleNonFixedCellRange();
...
BOOL CAsCoolGridCtrl::IsCellEditable(CAsCoolGridCellID &cell) const
{
	return IsCellEditable(cell.m_nRow, cell.m_nCol);
}

BOOL CAsCoolGridCtrl::IsCellEditable(int nRow, int nCol) const
{
	return IsEditable() && ((GetItemState(nRow, nCol) & GVIS_READONLY) != GVIS_READONLY);
}

BOOL CAsCoolGridCtrl::IsCellSelected(CAsCoolGridCellID &cell) const
{
	return IsCellSelected(cell.m_nRow, cell.m_nCol);
}

BOOL CAsCoolGridCtrl::IsCellSelected(int nRow, int nCol) const
{
	if (GetVirtualMode())
	{   
		if (!IsSelectable())
		...
		return (m_SelectedCellMap.Lookup(key, (CAsCoolGridCellID&)cell));       
	}
	else
		return IsSelectable() && ((GetItemState(nRow, nCol) & GVIS_SELECTED) == GVIS_SELECTED);
}

BOOL CAsCoolGridCtrl::IsCellVisible(CAsCoolGridCellID cell) 
{
	return IsCellVisible(cell.m_nRow, cell.m_nCol);
}

BOOL CAsCoolGridCtrl::IsCellVisible(int nRow, int nCol)
{
	if (!IsWindow(m_hWnd))
		return FALSE;
	...
	CAsCoolGridCellID TopLeft;
	if (nCol >= GetFixedColumnCount() || nRow >= GetFixedRowCount())
	{
		TopLeft = GetTopleftNonFixedCell();
		if (nCol >= GetFixedColumnCount() && nCol < TopLeft.m_nCol)
			return FALSE;
		if (nRow >= GetFixedRowCount() && nRow < TopLeft.m_nRow)
			return FALSE;
	}
	...
	else 
	{
		...
		for (int i = TopLeft.m_nCol; i <= nCol; i++) 
		{
			if (x >= rect.right)
				return FALSE;
			x += GetColumnWidth(i);    
	...
	{
		if (nRow < TopLeft.m_nRow)
			return FALSE;
		y = GetFixedRowHeight();
		for (int i = TopLeft.m_nRow; i <= nRow; i++) 
		{
			if (y >= rect.bottom)
				return FALSE;
			y += GetRowHeight(i);    
...
BOOL CAsCoolGridCtrl::InvalidateCellRect(const CAsCoolGridCellID& cell)
{
	return InvalidateCellRect(cell.m_nRow, cell.m_nCol);
}

BOOL CAsCoolGridCtrl::InvalidateCellRect(const int row, const int col)
{
...
BOOL CAsCoolGridCtrl::InvalidateCellRect(const CAsCoolGridCellRange& cellRange)
{
	ASSERT(IsValid(cellRange));
	if (!::IsWindow(GetSafeHwnd()) || !m_bAllowDraw)
		return FALSE;

	CAsCoolGridCellRange visibleCellRange = GetVisibleNonFixedCellRange().Intersect(cellRange);
...
void CAsCoolGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
	CRect rect;
	GetClientRect(rect);
		...
		(m_bLMouseButtonDown && m_MouseMode == MOUSE_NOTHING))
	{
		...
		if (m_MouseMode == MOUSE_NOTHING)
		{
			CAsCoolGridCellBase* pCell = NULL;
			CAsCoolGridCellID idCurrentCell;
			if (!GetVirtualMode() || m_bTitleTips)
			{
				// Let the cell know that a big fat cursor is currently hovering over it.
				idCurrentCell = GetCellFromPt(point);
				pCell = GetCell(idCurrentCell.m_nRow, idCurrentCell.m_nCol);
				if (pCell)
					pCell->OnMouseOver();
			}
			...
			if (m_bTitleTips)
			{
				...
				if (pCell)
				{
					if (!m_bRMouseButtonDown && !pCell->IsEditing()
						&& GetCellRect(idCurrentCell.m_nRow, idCurrentCell.m_nCol, CellRect) )
					{
						m_TitleTip.Show(CellRect, CString(pCell->GetTipText()), pCell->GetFont(),
							GetTitleTipTextClr(), GetTitleTipBackClr() );
					}
				}
				else
				{
		...
		case MOUSE_SELECT_CELLS:
			{
				...
				if (idCurrentCell != GetFocusCell())
				{
					...
					if((idCurrentCell.m_nRow >= m_nFixedRows &&
						idCurrentCell.m_nCol >= m_nFixedCols) ||
...
CPoint CAsCoolGridCtrl::GetPointClicked(int nRow, int nCol, const CPoint& point)
{
...
void CAsCoolGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	TRACE0("CAsCoolGridCtrl::OnLButtonDblClk\n");

	CAsCoolGridCellID cell = GetCellFromPt(point);
	if( !IsValid( cell) )
	{
		//ASSERT(FALSE);
		return;
	...
	{
		CPoint start;
		if (!GetCellOrigin(0, cell.m_nCol, &start))
			return;
		...
		while( cell.m_nCol >= 0)
		{
			if( GetColumnWidth( cell.m_nCol) > 0)
			{
				bFoundVisible = TRUE;
				break;
			}
		...
		AutoSizeColumn(cell.m_nCol, GetAutoSizeStyle());
		Invalidate();
	}
	else if (m_MouseMode == MOUSE_OVER_ROW_DIVIDE)
	{
		...
		AutoSizeRow(cell.m_nRow);
		Invalidate();
	}
	else if (m_MouseMode == MOUSE_NOTHING)
	{
		CPoint pointClickedRel;
		pointClickedRel = GetPointClicked( cell.m_nRow, cell.m_nCol, point);

		CAsCoolGridCellBase* pCell = NULL;
		if (IsValid(cell))
			pCell = GetCell(cell.m_nRow, cell.m_nCol);
		...
		{
			CRect rectCell;
			if (GetCellRect(cell.m_nRow, cell.m_nCol, rectCell) && pCell->GetTextRect(rectCell))
				bInTextArea = rectCell.PtInRect(point);
		}

		if (cell.m_nRow >= m_nFixedRows && IsValid(m_LeftClickDownCell) && 
			cell.m_nCol >= m_nFixedCols && bInTextArea)
		{
			OnEditCell(cell.m_nRow, cell.m_nCol, pointClickedRel, VK_LBUTTON);
		}
		else if (m_bListMode)
		{
			if (!IsValid(cell))
				return;
			if (cell.m_nRow >= m_nFixedRows && cell.m_nCol >= m_nFixedCols && bInTextArea)
				OnEditCell(cell.m_nRow, cell.m_nCol, pointClickedRel, VK_LBUTTON);
		}
		...
		if (IsValid(cell))
		{
			...
			SendMessageToParent(cell.m_nRow, cell.m_nCol, NM_DBLCLK);
		}
	}
...
void CAsCoolGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	CAsCoolGridCellBase* pCell = GetCell(m_LeftClickDownCell.m_nRow, m_LeftClickDownCell.m_nCol);
	if (pCell)
		pCell->OnClickDown(GetPointClicked( m_LeftClickDownCell.m_nRow, m_LeftClickDownCell.m_nCol, point));
	...
	{
		CRect rectCell;
		if (GetCellRect(m_LeftClickDownCell.m_nRow, m_LeftClickDownCell.m_nCol, rectCell) &&
	...
	{
		SetFocusCell(m_LeftClickDownCell.m_nRow, m_LeftClickDownCell.m_nCol);
		...
		m_MouseMode != MOUSE_OVER_ROW_DIVIDE)        
	{
		if (m_LeftClickDownCell.m_nRow >= GetFixedRowCount() && 
			m_LeftClickDownCell.m_nCol >= GetFixedColumnCount())
		{
			SetFocusCell(m_LeftClickDownCell.m_nRow, m_LeftClickDownCell.m_nCol);
		}
		else
		...
		//    SetFocusCell(max(m_LeftClickDownCell.m_nRow, m_nFixedRows),
		//                 max(m_LeftClickDownCell.m_nCol, m_nFixedCols));
	}
	...
	if (m_MouseMode == MOUSE_OVER_COL_DIVIDE) // sizing column
	{
		...
		if (GetColumnWidth(GetColumnCount()-1) < m_nResizeCaptureRange)
		{
			...
			if (abs(point.x - VisRect.right) < m_nResizeCaptureRange)
				m_LeftClickDownCell.m_nCol = GetColumnCount()-1;
		}

		CPoint start;
		if (!GetCellOrigin(0, m_LeftClickDownCell.m_nCol, &start))
			return;
		...
		if( !m_bHiddenColUnhide)
		{
			...
			if( bLookForVisible)
			{
				...
				while( m_LeftClickDownCell.m_nCol - iOffset >= 0)
				{
					if( GetColumnWidth( m_LeftClickDownCell.m_nCol - iOffset) > 0)
					{
						bFoundVisible = TRUE;
						break;
					}
					m_LeftClickDownCell.m_nCol--;
				}
				if( !bFoundVisible)
					return;
			}
		...
		{
			if (m_LeftClickDownCell.m_nCol < GetColumnCount()-1 || 
	...
	else if (m_MouseMode == MOUSE_OVER_ROW_DIVIDE) // sizing row
	{
		...
		if (GetRowHeight(GetRowCount()-1) < m_nResizeCaptureRange)
		{
			...
			if (abs(point.y - VisRect.bottom) < m_nResizeCaptureRange)
				m_LeftClickDownCell.m_nRow = GetRowCount()-1;
		}
		...
		if( !m_bHiddenRowUnhide)
		{
			...
			if( bLookForVisible)
			{
				...
				while( m_LeftClickDownCell.m_nRow - iOffset >= 0)
				{
					if( GetRowHeight( m_LeftClickDownCell.m_nRow - iOffset) > 0)
					{
						bFoundVisible = TRUE;
						break;
					}
					m_LeftClickDownCell.m_nRow--;
				}
				if( !bFoundVisible)
					return;
			}
		...
		if (point.y - start.y < m_nResizeCaptureRange)            // clicked below border
		{
			if (m_LeftClickDownCell.m_nRow < GetRowCount()-1 || 
				GetRowHeight(GetRowCount()-1) >= m_nResizeCaptureRange)
			{
				if (!GetCellOrigin(--m_LeftClickDownCell.m_nRow, 0, &start))
					return;
			}
		}
		...
		if (m_MouseMode != MOUSE_PREPARE_DRAG) // not sizing or editing -- selecting
		{
			...
			if (m_LeftClickDownCell.m_nRow < GetFixedRowCount())
			{
				if ( 1 != SendMessageToParent(m_LeftClickDownCell.m_nRow,
					m_LeftClickDownCell.m_nCol, GVN_COLUMNCLICK) )
				{
					OnFixedRowClick(m_LeftClickDownCell);
				}
			}
			else if (m_LeftClickDownCell.m_nCol < GetFixedColumnCount())
...
void CAsCoolGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TRACE0("CAsCoolGridCtrl::OnLButtonUp\n");
	...
	CPoint pointClickedRel;
	pointClickedRel = GetPointClicked( m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol, point);
	...
	{
		OnEditCell(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol, pointClickedRel, VK_LBUTTON);
	}
	// m_MouseMode == MOUSE_PREPARE_DRAG only if user clicked down on a selected cell
	// and then didn't move mouse before clicking up (releasing button)
	else if (m_MouseMode == MOUSE_PREPARE_DRAG) 
	{
		CAsCoolGridCellBase* pCell = GetCell(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol);
		if (pCell)
			pCell->OnClick( GetPointClicked( m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol, point) );
		SendMessageToParent(m_LeftClickDownCell.m_nRow, m_LeftClickDownCell.m_nCol, NM_CLICK);
		SendMessageToParent(m_LeftClickDownCell.m_nRow, m_LeftClickDownCell.m_nCol, GVN_SELCHANGING);
		ResetSelectedRange();
		SendMessageToParent(m_idCurrentCell.m_nRow, m_idCurrentCell.m_nCol, GVN_SELCHANGED);
	}
	else if (m_MouseMode == MOUSE_SIZING_COL)
	{
		...
		{   
			...
			SetColumnWidth(m_LeftClickDownCell.m_nCol, nColumnWidth);
			ResetScrollBars();
			Invalidate();
		}
	}
	else if (m_MouseMode == MOUSE_SIZING_ROW)
	{
		...
		if (m_LeftClickDownPoint != point  && (point.x != 0 || point.y != 0)) // 0 pt fix by email1@bierling.net
		{
			...
			SetRowHeight(m_LeftClickDownCell.m_nRow, nRowHeight);
			ResetScrollBars();
			Invalidate();
		}
	}
...
void CAsCoolGridCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
...
void CAsCoolGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
	CWnd::OnRButtonUp(nFlags, point);

	m_bRMouseButtonDown = FALSE;

	CAsCoolGridCellID FocusCell;
	...
	{
		SetFocusCell(-1,-1);
		SetFocusCell(__max(FocusCell.m_nRow, m_nFixedRows),
			__max(FocusCell.m_nCol, m_nFixedCols));

		// tell the cell about it
		CAsCoolGridCellBase* pCell = GetCell(FocusCell.m_nRow, FocusCell.m_nCol);
		if (pCell)
			pCell->OnRClick( GetPointClicked( FocusCell.m_nRow, FocusCell.m_nCol, point) );

		SendMessageToParent(FocusCell.m_nRow, FocusCell.m_nCol, NM_RCLICK);
	}
}

/////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl printing

// EFW - New print margin support functions
void CAsCoolGridCtrl::SetPrintMarginInfo(int nHeaderHeight, int nFooterHeight,
...
void CAsCoolGridCtrl::GetPrintMarginInfo(int &nHeaderHeight, int &nFooterHeight,
										 int &nLeftMargin, int &nRightMargin, int &nTopMargin,
										 int &nBottomMargin, int &nGap)
{
...
void CAsCoolGridCtrl::Print(CPrintDialog* pPrntDialog /*=NULL*/)
{
	CDC dc;
...
/////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl printing overridables - for Doc/View print/print preview framework

// EFW - Various changes in the next few functions to support the
// new print margins and a few other adjustments.
void CAsCoolGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
...
void CAsCoolGridCtrl::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
	if (!pDC || !pInfo)
		return;
	...
	while (m_nCurrPrintRow < GetRowCount())
	{
		...
		for (int col = m_nPrintColumn; col < GetColumnCount(); col++)
		{
			...
			CAsCoolGridCellBase* pCell = GetCell(m_nCurrPrintRow, col);
			if (pCell)
...
void CAsCoolGridCtrl::PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect,
										 CDC *pDC, BOOL& bFirst)
{
	...
	for (int col =nStartColumn; col < nStopColumn; col++)
	{
		...
		CAsCoolGridCellBase* pCell = GetCell(row, col);
		if (pCell)
...
} // end of CAsCoolGridCtrl::PrintFixedRowCells

void CAsCoolGridCtrl::PrintColumnHeadings(CDC *pDC, CPrintInfo* /*pInfo*/)
{
	CFont *pOldFont = pDC->SelectObject(&m_PrinterFont);
	...
	pDC->SelectObject(pOldFont);
} // end of CAsCoolGridCtrl::PrintColumnHeadings
...
*****************************************************************************/
void CAsCoolGridCtrl::PrintRowButtons(CDC *pDC, CPrintInfo* /*pInfo*/)
{
	CFont *pOldFont = pDC->SelectObject(&m_PrinterFont);
	...
	for( int iCol = 0; iCol < GetFixedColumnCount(); iCol++)
	{
		...
		for( int iRow = m_nCurrPrintRow; iRow < GetRowCount(); iRow++)
		{
			...
			CAsCoolGridCellBase* pCell = GetCell(iRow, iCol);
			if (pCell)
...
void CAsCoolGridCtrl::PrintHeader(CDC *pDC, CPrintInfo *pInfo)
{
...
void CAsCoolGridCtrl::PrintFooter(CDC *pDC, CPrintInfo *pInfo)
{
...
void CAsCoolGridCtrl::OnEndPrinting(CDC* /*pDC*/, CPrintInfo* /*pInfo*/)
{
	m_PrinterFont.DeleteObject();
}

/////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl persistance

BOOL CAsCoolGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
	CStdioFile File;
	CFileException ex;
...
BOOL CAsCoolGridCtrl::Load(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
	if (GetVirtualMode())
		return FALSE;
...
/////////////////////////////////////////////////////////////////////////////
// CAsCoolGridCtrl overrideables

// This is no longer needed since I've changed to OLE drag and drop - but it's
// still cool code. :)
CImageList* CAsCoolGridCtrl::CreateDragImage(CPoint *pHotSpot)
{
	CDC* pDC = GetDC();
	if (!pDC)
		return NULL;
	...
	CRect rect;
	CAsCoolGridCellID cell = GetFocusCell();
	if (!GetCellRect(cell.m_nRow, cell.m_nCol, rect))
		return NULL;
	...
	*pHotSpot = rect.BottomRight(); 
	...
	CAsCoolGridCellBase* pCell = GetCell(cell.m_nRow, cell.m_nCol);
	if (pCell)
		pCell->Draw(&MemDC, cell.m_nRow, cell.m_nCol, rect, FALSE);
...
void CAsCoolGridCtrl::OnFixedRowClick(CAsCoolGridCellID& cell)
{
	if (!IsValid(cell))
		return;
	...
	{
		CWaitCursor waiter;
		if (cell.m_nCol == GetSortColumn())
			SortItems(cell.m_nCol, !GetSortAscending());
		else
			SortItems(cell.m_nCol, TRUE);
		Invalidate();
	}
	...
	{
		if (cell.m_nCol < GetFixedColumnCount())
		{
...
void CAsCoolGridCtrl::OnFixedColumnClick(CAsCoolGridCellID& cell)
{
	if (!IsValid(cell))
		return;
	...
	{
		if (cell.m_nRow < GetFixedRowCount())
		{
			m_MouseMode = MOUSE_SELECT_ALL;
			OnSelecting(cell);
		}
...
CSize CAsCoolGridCtrl::GetTextExtent(int nRow, int nCol, LPCTSTR str)
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell)
		return CSize(0, 0);
...
// virtual
void CAsCoolGridCtrl::OnEditCell(int nRow, int nCol, CPoint point, UINT nChar)
{
	m_TitleTip.Hide();  // hide any titletips
...
// virtual
void CAsCoolGridCtrl::EndEditing()
{
	CAsCoolGridCellID cell = GetFocusCell();
...
// virtual
void CAsCoolGridCtrl::OnEndEditCell(int nRow, int nCol, CString str)
{
	CString strCurrentText = GetItemText(nRow, nCol);
...
// virtual
BOOL CAsCoolGridCtrl::ValidateEdit(int nRow, int nCol, LPCTSTR str)
{
	CAsCoolGridCellBase* pCell = GetCell(nRow, nCol);
	ASSERT(pCell);
	if (!pCell)
...
// virtual
CString CAsCoolGridCtrl::GetItemText(int nRow, int nCol) const
{
